...
void routine_8a7e_impl(StarWarsCPU& cpu) {
    ...
    // 8B30: BCS $00B5
    if (cpu.carry_flag()) cpu.state_.pc = 0x8AE7;
    ...
    // 8B42: BCS $00C7
    if (cpu.carry_flag()) cpu.state_.pc = 0x8B0B;
    ...
    // 8B51: BPL $009A
    if (!cpu.negative_flag()) cpu.state_.pc = 0x8AED;
    ...
    // 8B61: BNE $00E8
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8B4B;
    ...
    // 8B76: BEQ $00FD
    if (cpu.zero_flag()) cpu.state_.pc = 0x8B75;
    ...
    // 8B83: BCS $00F2
    if (cpu.carry_flag()) cpu.state_.pc = 0x8B77;
    ...
    // 8B91: BNE $0158
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8BEB;
    ...
    // 8B97: BLT $011D
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8BB6;
    ...
    // 8B9D: BGT $0123
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8BC2;
    ...
    // 8BA7: BLT $012D
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8BD6;
    ...
    // 8BAD: BGT $0133
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8BE2;
    ...
    // 8BBC: BRA $0158
    cpu.state_.pc = 0x8C16;

    // 8BBE: LDA $A,U
    // TODO: Fix comma operator: LDA $A,U

    // 8BC1: BPL $0146
    if (!cpu.negative_flag()) cpu.state_.pc = 0x8C09;

    // 8BC4: CMPA #$08
    cpu.compare_a(0x08);

    // 8BC6: BGT $0158
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8C20;

    // 8BC8: LDA $C,U
    // TODO: Fix comma operator: LDA $C,U

    // 8BCB: BPL $0150
    if (!cpu.negative_flag()) cpu.state_.pc = 0x8C1D;

    // 8BCE: CMPA #$08
    cpu.compare_a(0x08);

    // 8BD0: BGT $0158
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8C2A;
    ...
    // 8BDE: BCS $010B
    if (cpu.carry_flag()) cpu.state_.pc = 0x8BEB;
    ...
    // 8BFF: BMI $0197
    if (cpu.negative_flag()) cpu.state_.pc = 0x8B98;
    ...
    // 8C2A: BCC $01B1
    if (!cpu.carry_flag()) cpu.state_.pc = 0x8BDD;
    ...
    // 8C38: BCC $01BF
    if (!cpu.carry_flag()) cpu.state_.pc = 0x8BF9;
    ...
    // 8C3F: BCC $01C6
    if (!cpu.carry_flag()) cpu.state_.pc = 0x8C07;
    ...
    // 8C66: BEQ $0203
    if (cpu.zero_flag()) cpu.state_.pc = 0x8C6B;
