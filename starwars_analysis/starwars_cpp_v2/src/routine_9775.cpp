...
void routine_9775_impl(StarWarsCPU& cpu) {
    ...
    // 9777: BEQ $0036
    if (cpu.zero_flag()) cpu.state_.pc = 0x97AF;
    ...
    // 97A3: BNE $0012
    if (!cpu.zero_flag()) cpu.state_.pc = 0x97B7;
    ...
    // 97B1: BCS $0043
    if (cpu.carry_flag()) cpu.state_.pc = 0x97F6;

    // 97B3: LDU #$9847
    cpu.state_.u = 0x9847;

    // 97B6: BRA $004C
    cpu.state_.pc = 0x9804;
    ...
    // 97E6: BRA $009B
    cpu.state_.pc = 0x9783;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $009B
    cpu.state_.pc = 0x9788;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $009B
    cpu.state_.pc = 0x978D;

    // 97F2: LDU #$9850
    cpu.state_.u = 0x9850;

    // 97F5: BRA $009B
    cpu.state_.pc = 0x9792;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $009B
    cpu.state_.pc = 0x9797;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $009B
    cpu.state_.pc = 0x979C;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $009B
    cpu.state_.pc = 0x97A1;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $009B
    cpu.state_.pc = 0x97A6;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $009B
    cpu.state_.pc = 0x97AB;
    ...
    // 9876: BNE $0109
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9881;
    ...
    // 9892: BEQ $0122
    if (cpu.zero_flag()) cpu.state_.pc = 0x98B6;
    ...
    // 989C: BNE $013A
    if (!cpu.zero_flag()) cpu.state_.pc = 0x98D8;
    ...
    // 98A3: BPL $0132
    if (!cpu.negative_flag()) cpu.state_.pc = 0x98D7;
    ...
    // 98AD: BCS $012C
    if (cpu.carry_flag()) cpu.state_.pc = 0x98DB;
    ...
    // 98B4: BEQ $0155
    if (cpu.zero_flag()) cpu.state_.pc = 0x990B;
    ...
    // 98D6: BRA $0187
    cpu.state_.pc = 0x985F;

    // 98D8: BCC $0189
    if (!cpu.carry_flag()) cpu.state_.pc = 0x9863;

    // 98DA: BCS $018C
    if (cpu.carry_flag()) cpu.state_.pc = 0x9868;
