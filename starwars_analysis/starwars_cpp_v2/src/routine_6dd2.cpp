...
void routine_6dd2_impl(CPU6809& cpu) {
    ...
    // 6DD4: BEQ $001E
    if (cpu.zero_flag()) cpu.m_pc = 0x6DF4;

    // 6DD6: BLE $000D
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6DE5;
    ...
    // 6DFC: BEQ $0046
    if (cpu.zero_flag()) cpu.m_pc = 0x6E44;

    // 6DFE: BLE $0035
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E35;
    ...
    // 6E24: BEQ $005C
    if (cpu.zero_flag()) cpu.m_pc = 0x6E82;

    // 6E26: BLE $0059
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E81;
    ...
    // 6E30: BPL $0061
    if (!cpu.negative_flag()) cpu.m_pc = 0x6E93;
    ...
    // 6E38: BPL $006C
    if (!cpu.negative_flag()) cpu.m_pc = 0x6EA6;
    ...
    // 6E42: BPL $0073
    if (!cpu.negative_flag()) cpu.m_pc = 0x6EB7;
    ...
    // 6E4A: BPL $007E
    if (!cpu.negative_flag()) cpu.m_pc = 0x6ECA;
    ...
    // 6E58: BLE $0093
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6DED;

    // 6E5A: CMPD #$0010
    // TODO: Convert CMPD #$0010

    // 6E5E: BLE $0091
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6DF1;
    ...
    // 6E69: BGE $009C
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6E07;
    ...
    // 6E72: BLE $00AD
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E21;

    // 6E74: CMPD #$0032
    // TODO: Convert CMPD #$0032

    // 6E78: BLE $00AB
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E25;
    ...
    // 6E83: BGE $00B6
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6E3B;
    ...
    // 6E9B: BEQ $00CE
    if (cpu.zero_flag()) cpu.m_pc = 0x6E6B;
    ...
    // 6EA5: BNE $00D8
    if (!cpu.zero_flag()) cpu.m_pc = 0x6E7F;
    ...
    // 6EAE: BNE $00E7
    if (!cpu.zero_flag()) cpu.m_pc = 0x6E97;

    // 6EB0: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 6EB2: BNE $00E7
    if (!cpu.zero_flag()) cpu.m_pc = 0x6E9B;
    ...
    // 6EBC: BLE $00F1
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6EAF;
    ...
    // 6EC9: BCS $00D8
    if (cpu.carry_flag()) cpu.m_pc = 0x6EA3;
    ...
    // 6ED4: BMI $0109
    if (cpu.negative_flag()) cpu.m_pc = 0x6EDF;
    ...
    // 6EE6: BMI $011B
    if (cpu.negative_flag()) cpu.m_pc = 0x6F03;
    ...
    // 6F14: BLE $0167
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6F7D;
    ...
    // 6F8B: BPL $01BC
    if (!cpu.negative_flag()) cpu.m_pc = 0x6F49;

    // 6F90: TST <$7D
    // TODO: Convert TST <$7D

    // 6F92: BGE $01C6
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6F5A;
    ...
    // 6FAF: BPL $01E0
    if (!cpu.negative_flag()) cpu.m_pc = 0x6F91;

    // 6FB4: TST <$7F
    // TODO: Convert TST <$7F

    // 6FB6: BPL $01EA
    if (!cpu.negative_flag()) cpu.m_pc = 0x6FA2;
    ...
    // 6FC9: BLE $01FE
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6FC9;
