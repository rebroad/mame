...
void routine_a2f8_impl(StarWarsCPU& cpu) {
    ...
    // A36B: BEQ $00AF
    if (cpu.zero_flag()) cpu.state_.pc = 0xA31C;
    ...
    // A37D: BGT $00AD
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xA32C;
    ...
    // A38D: BLT $009D
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xA32C;
    ...
    // A397: BNE $00A5
    if (!cpu.zero_flag()) cpu.state_.pc = 0xA33E;
    ...
    // A3A0: BPL $00AB
    if (!cpu.negative_flag()) cpu.state_.pc = 0xA34D;

    // A3A3: STB <$63
    cpu.write_memory(0x63, cpu.state_.b);

    // A3A5: BRA $00FC
    cpu.state_.pc = 0xA3A3;
    ...
    // A3AD: BGT $00FC
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xA3AB;
    ...
    // A3BF: BNE $00FC
    if (!cpu.zero_flag()) cpu.state_.pc = 0xA3BD;
    ...
    // A3C7: BGE $00FC
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xA3C5;
    ...
    // A3D2: BGT $00FC
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xA3D0;
    ...
    // A3DC: BLT $00EC
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xA3CA;
    ...
    // A3E6: BNE $00FC
    if (!cpu.zero_flag()) cpu.state_.pc = 0xA3E4;
    ...
    // A3ED: BMI $00F8
    if (cpu.negative_flag()) cpu.state_.pc = 0xA3E7;
    ...
    // A3F9: BGT $0108
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xA403;

    // A3FB: JSR $CD68
    cpu.call_function(0xCD68);

    // A3FE: BRA $010B
    cpu.state_.pc = 0xA40B;
    ...
    // A434: BPL $015D
    if (!cpu.negative_flag()) cpu.state_.pc = 0xA493;

    // A436: LDA <$BC
    cpu.state_.a = cpu.read_memory(0xBC);

    // A438: BEQ $015D
    if (cpu.zero_flag()) cpu.state_.pc = 0xA497;
    ...
    // A48F: BEQ $01A7
    if (cpu.zero_flag()) cpu.state_.pc = 0xA438;
    ...
    // A4C0: BPL $01CD
    if (!cpu.negative_flag()) cpu.state_.pc = 0xA48F;
    ...
    // A4EF: BMI $0252
    if (cpu.negative_flag()) cpu.state_.pc = 0xA543;

    // A4F1: SUBD $5004
    cpu.state_.d -= 0x5004;

    // A4F4: BGT $0252
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xA548;
