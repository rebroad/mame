...
void routine_8a59_impl(CPU6809& cpu) {
    ...
    // 8AD5: BNE $0082
    if (!cpu.zero_flag()) cpu.m_pc = 0x8A59;

    // 8AD7: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 8AD9: BEQ $0083
    if (cpu.zero_flag()) cpu.m_pc = 0x8A5E;
    ...
    // 8AE3: BNE $0094
    if (!cpu.zero_flag()) cpu.m_pc = 0x8A79;
    ...
    // 8B30: BCS $00DA
    if (cpu.carry_flag()) cpu.m_pc = 0x8B0C;
    ...
    // 8B42: BCS $00EC
    if (cpu.carry_flag()) cpu.m_pc = 0x8B30;
    ...
    // 8B51: BPL $00BF
    if (!cpu.negative_flag()) cpu.m_pc = 0x8B12;
    ...
    // 8B61: BNE $010D
    if (!cpu.zero_flag()) cpu.m_pc = 0x8B70;
    ...
    // 8B83: BCS $0117
    if (cpu.carry_flag()) cpu.m_pc = 0x8B9C;
    ...
    // 8B91: BNE $017D
    if (!cpu.zero_flag()) cpu.m_pc = 0x8C10;
    ...
    // 8B97: BLT $0142
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8BDB;
    ...
    // 8B9D: BGT $0148
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8BE7;
    ...
    // 8BA7: BLT $0152
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8BFB;
    ...
    // 8BAD: BGT $0158
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8C07;
    ...
    // 8BC6: BGT $017D
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8C45;

    // 8BC8: LDA $C,U
    // TODO: Fix comma operator: LDA $C,U

    // 8BCB: BPL $0175
    if (!cpu.negative_flag()) cpu.m_pc = 0x8C42;

    // 8BCE: CMPA #$08
    cpu.compare_a(0x08);

    // 8BD0: BGT $017D
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8C4F;
    ...
    // 8BFF: BMI $01BC
    if (cpu.negative_flag()) cpu.m_pc = 0x8BBD;
    ...
    // 8C1C: BCC $01C8
    if (!cpu.carry_flag()) cpu.m_pc = 0x8BE6;
    ...
    // 8C23: BCC $01CF
    if (!cpu.carry_flag()) cpu.m_pc = 0x8BF4;
