...
void routine_77a4_impl(CPU6809& cpu) {
    ...
    // 77A7: BLE $002F
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x77D8;
    ...
    // 77B3: BPL $0015
    if (!cpu.negative_flag()) cpu.m_pc = 0x77CA;
    ...
    // 77BC: BGE $002F
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x77ED;
    ...
    // 77C5: BPL $0027
    if (!cpu.negative_flag()) cpu.m_pc = 0x77EE;
    ...
    // 77CE: BGE $002F
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x77FF;
    ...
    // 7809: BCC $006A
    if (!cpu.carry_flag()) cpu.m_pc = 0x7875;
    ...
    // 7812: BNE $0075
    if (!cpu.zero_flag()) cpu.m_pc = 0x7889;
    ...
    // 7823: BNE $0086
    if (!cpu.zero_flag()) cpu.m_pc = 0x77AB;
    ...
    // 7841: BNE $00A4
    if (!cpu.zero_flag()) cpu.m_pc = 0x77E7;
    ...
    // 7871: BEQ $00D2
    if (cpu.zero_flag()) cpu.m_pc = 0x7845;
    ...
    // 787E: BCS $00C9
    if (cpu.carry_flag()) cpu.m_pc = 0x7849;
    ...
    // 78D2: BNE $0140
    if (!cpu.zero_flag()) cpu.m_pc = 0x7914;
    ...
    // 78E6: BNE $0159
    if (!cpu.zero_flag()) cpu.m_pc = 0x7941;

    // 78E8: CMPU $4B39
    // TODO: Convert CMPU $4B39

    // 78EC: BGT $014F
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x793D;
    ...
    // 7904: BNE $0165
    if (!cpu.zero_flag()) cpu.m_pc = 0x796B;
    ...
    // 794F: BNE $01CE
    if (!cpu.zero_flag()) cpu.m_pc = 0x791F;
    ...
    // 7955: BNE $01CE
    if (!cpu.zero_flag()) cpu.m_pc = 0x7925;
    ...
    // 795E: BCS $01CB
    if (cpu.carry_flag()) cpu.m_pc = 0x792B;

    // 7960: LDA $4703
    cpu.m_a = cpu.read_memory(0x4703);

    // 7963: BPL $01C6
    if (!cpu.negative_flag()) cpu.m_pc = 0x792B;
    ...
    // 7991: BPL $01F3
    if (!cpu.negative_flag()) cpu.m_pc = 0x7986;
    ...
    // 799F: BPL $0201
    if (!cpu.negative_flag()) cpu.m_pc = 0x79A2;
