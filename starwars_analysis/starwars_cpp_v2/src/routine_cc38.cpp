...
void routine_cc38_impl(CPU6809& cpu) {
    // Converted from rom_disasm_cc38.md
    // Address: 0xCC38
    ...
    // CC3F: BCC $0023
    if (!cpu.carry_flag()) cpu.m_pc = 0xCC64;
    ...
    // CC4D: BCC $0023
    if (!cpu.carry_flag()) cpu.m_pc = 0xCC72;
    ...
    // CC53: BCC $0023
    if (!cpu.carry_flag()) cpu.m_pc = 0xCC78;

    // CC55: CMPX #$4AB6
    cpu.compare_x(0x4AB6);

    // CC58: BCS $0011
    if (cpu.carry_flag()) cpu.m_pc = 0xCC6B;
    ...
    // CC68: BCS $0029
    if (cpu.carry_flag()) cpu.m_pc = 0xCC93;
    ...
    // CC77: BCS $0038
    if (cpu.carry_flag()) cpu.m_pc = 0xCCB1;
    ...
    // CDC0: BMI $0185
    if (cpu.negative_flag()) cpu.m_pc = 0xCD47;
