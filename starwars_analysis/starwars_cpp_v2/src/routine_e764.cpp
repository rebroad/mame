...
void routine_e764_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_e764.md
    // Address: 0xE764
    ...
    // E76B: BGT $000E
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xE77B;
    ...
    // E782: BGT $0025
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xE7A9;
    ...
    // E797: BNE $003E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE7D7;

    // E799: TST <$AD
    // TODO: Convert TST <$AD

    // E79B: BLE $003E
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xE7DB;
    ...
    // E7B0: BNE $0057
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE809;
    ...
    // E7CE: BSR $0079
    // TODO: Convert BSR $0079

    // E7D0: BSR $0086
    // TODO: Convert BSR $0086

    // E7D3: CMPB #$D6
    cpu.compare_b(0xD6);

    // E7D5: BCC $0078
    if (!cpu.carry_flag()) cpu.state_.pc = 0xE84F;
    ...
    // E833: BCS $00D8
    if (cpu.carry_flag()) cpu.state_.pc = 0xE90D;
    ...
    // E83A: BRA $0124
    cpu.state_.pc = 0xE960;

    // E83C: CMPB #$74
    cpu.compare_b(0x74);

    // E83E: BNE $00E1
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE921;

    // E840: LDX #$2FDE
    cpu.state_.x = 0x2FDE;

    // E843: BRA $0124
    cpu.state_.pc = 0xE969;

    // E845: CMPB #$80
    cpu.compare_b(0x80);

    // E847: BNE $00EB
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE934;

    // E849: LDX #$3058
    cpu.state_.x = 0x3058;

    // E84D: BRA $0124
    cpu.state_.pc = 0xE973;

    // E84F: CMPB #$60
    cpu.compare_b(0x60);

    // E851: BCS $00F4
    if (cpu.carry_flag()) cpu.state_.pc = 0xE947;

    // E853: LDX #$2FA4
    cpu.state_.x = 0x2FA4;

    // E856: BRA $0124
    cpu.state_.pc = 0xE97C;

    // E858: CMPB #$40
    cpu.compare_b(0x40);

    // E85A: BNE $00FD
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE959;

    // E85C: LDX #$2FC2
    cpu.state_.x = 0x2FC2;

    // E85F: BRA $0124
    cpu.state_.pc = 0xE985;

    // E861: CMPB #$4E
    cpu.compare_b(0x4E);

    // E863: BNE $0106
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE96B;

    // E865: LDX #$2FFE
    cpu.state_.x = 0x2FFE;

    // E868: BRA $0124
    cpu.state_.pc = 0xE98E;

    // E86A: CMPB #$58
    cpu.compare_b(0x58);

    // E86C: BNE $010F
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE97D;

    // E86E: LDX #$2FF6
    cpu.state_.x = 0x2FF6;

    // E871: BRA $0124
    cpu.state_.pc = 0xE997;

    // E873: CMPB #$5A
    cpu.compare_b(0x5A);

    // E875: BNE $0118
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE98F;

    // E877: LDX #$3000
    cpu.state_.x = 0x3000;

    // E87A: BRA $0124
    cpu.state_.pc = 0xE9A0;

    // E87C: CMPB #$4A
    cpu.compare_b(0x4A);

    // E87E: BNE $0121
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE9A1;

    // E880: LDX #$300C
    cpu.state_.x = 0x300C;

    // E883: BRA $0124
    cpu.state_.pc = 0xE9A9;
    ...
    // E891: BPL $00C7
    if (!cpu.negative_flag()) cpu.state_.pc = 0xE95A;
    ...
    // E945: BRA $01C5
    cpu.state_.pc = 0xEB0C;

    // E947: BNE $01C7
    if (!cpu.zero_flag()) cpu.state_.pc = 0xEB10;
