...
void routine_a80b_impl(CPU6809& cpu) {
    ...
    // A80D: BEQ $0009
    if (cpu.zero_flag()) cpu.m_pc = 0xA818;
    ...
    // A83F: BLT $0038
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA879;
    ...
    // A850: BEQ $0056
    if (cpu.zero_flag()) cpu.m_pc = 0xA8A8;
    ...
    // A857: BCC $0055
    if (!cpu.carry_flag()) cpu.m_pc = 0xA8AE;
    ...
    // A868: BCS $0041
    if (cpu.carry_flag()) cpu.m_pc = 0xA8AB;

    // A86B: DEC $5,X
    // TODO: Convert DEC $5,X

    // A86D: BGT $0069
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA8D8;
    ...
    // A877: BGT $0075
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA8EE;
    ...
    // A8A9: BGT $00A7
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA852;
    ...
    // A911: BMI $0114
    if (cpu.negative_flag()) cpu.m_pc = 0xA927;
    ...
    // A92C: BPL $0127
    if (!cpu.negative_flag()) cpu.m_pc = 0xA955;
    ...
    // A93F: BMI $013A
    if (cpu.negative_flag()) cpu.m_pc = 0xA97B;
    ...
    // A958: BNE $0153
    if (!cpu.zero_flag()) cpu.m_pc = 0xA9AD;
    ...
    // A96F: BLE $0169
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA9DA;
    ...
    // A97F: BGE $0179
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA9FA;
    ...
    // A990: BMI $0197
    if (cpu.negative_flag()) cpu.m_pc = 0xA929;
    ...
    // A999: BLE $0193
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA92E;
    ...
    // A9B5: BEQ $01B0
    if (cpu.zero_flag()) cpu.m_pc = 0xA967;
    ...
    // A9CC: BLE $01C6
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA994;
    ...
    // A9DC: BGE $01D6
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA9B4;
    ...
    // A9ED: BMI $01F4
    if (cpu.negative_flag()) cpu.m_pc = 0xA9E3;
    ...
    // A9F6: BLE $01F0
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA9E8;
