...
void routine_8951_impl(StarWarsCPU& cpu) {
    ...
    // 8AD5: BNE $018A
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8A61;

    // 8AD7: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 8AD9: BEQ $018B
    if (cpu.zero_flag()) cpu.state_.pc = 0x8A66;
    ...
    // 8AE3: BNE $019C
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8A81;
    ...
    // 8B30: BCS $01E2
    if (cpu.carry_flag()) cpu.state_.pc = 0x8B14;
    ...
    // 8B42: BCS $01F4
    if (cpu.carry_flag()) cpu.state_.pc = 0x8B38;
