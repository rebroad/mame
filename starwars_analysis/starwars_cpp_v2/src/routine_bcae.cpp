...
void routine_bcae_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_bcae.md
    // Address: 0xBCAE
    ...
    // BCC5: BNE $0002
    if (!cpu.zero_flag()) cpu.state_.pc = 0xBCC9;
    ...
    // BCE6: BNE $001C
    if (!cpu.zero_flag()) cpu.state_.pc = 0xBD04;
    ...
    // BCEE: BPL $0047
    if (!cpu.negative_flag()) cpu.state_.pc = 0xBD37;

    // BCF1: BNE $003D
    if (!cpu.zero_flag()) cpu.state_.pc = 0xBD30;
    ...
    // BE28: BCS $0174
    if (cpu.carry_flag()) cpu.state_.pc = 0xBF9E;
    ...
    // BE52: BPL $019E
    if (!cpu.negative_flag()) cpu.state_.pc = 0xBFF2;
    ...
    // BE5F: BCS $018C
    if (cpu.carry_flag()) cpu.state_.pc = 0xBFED;
