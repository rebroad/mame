...
void routine_98b0_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_98b0.md
    // Address: 0x98B0
    ...
    // 98D6: BRA $004C
    cpu.state_.pc = 0x9924;

    // 98D8: BCC $004E
    if (!cpu.carry_flag()) cpu.state_.pc = 0x9928;

    // 98DA: BCS $0051
    if (cpu.carry_flag()) cpu.state_.pc = 0x992D;
