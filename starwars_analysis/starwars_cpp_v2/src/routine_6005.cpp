...
void routine_6005_impl(StarWarsCPU& cpu) {
    ...
    // 6011: BCC $0000
    if (!cpu.carry_flag()) cpu.state_.pc = 0x6013;

    // 6013: CMPS #$4FFF
    // TODO: Convert CMPS #$4FFF

    // 6017: BEQ $0015
    if (cpu.zero_flag()) cpu.state_.pc = 0x602E;

    // 601A: LDA <$28
    cpu.state_.a = cpu.read_memory(0x28);

    // 601C: BNE $002A
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6048;

    // 601E: LDA $4824
    cpu.state_.a = cpu.read_memory(0x4824);

    // 6021: ANDA #$80
    cpu.state_.a &= 0x80;

    // 6023: BNE $002A
    if (!cpu.zero_flag()) cpu.state_.pc = 0x604F;
    ...
    // 6029: BEQ $0000
    if (cpu.zero_flag()) cpu.state_.pc = 0x602B;
    ...
    // 6034: BMI $002D
    if (cpu.negative_flag()) cpu.state_.pc = 0x6063;

    // 6036: LDA <$41
    cpu.state_.a = cpu.read_memory(0x41);

    // 6038: CMPA #$3D
    cpu.compare_a(0x3D);

    // 603A: BCC $0035
    if (!cpu.carry_flag()) cpu.state_.pc = 0x6071;
    ...
    // 607D: STD $6802
    cpu.write_memory(0x6802, cpu.state_.d);
    ...
    // 60C0: BNE $00C5
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6087;

    // 60C2: INC <$42
    // TODO: Convert INC <$42

    // 60C4: BNE $00C5
    if (!cpu.zero_flag()) cpu.state_.pc = 0x608B;
    ...
    // 60CD: BNE $00CF
    if (!cpu.zero_flag()) cpu.state_.pc = 0x609E;

    // 60CF: STA $4B31
    cpu.write_memory(0x4B31, cpu.state_.a);

    // 60D2: BRA $00EF
    cpu.state_.pc = 0x60C3;

    // 60D4: LDA $4B31
    cpu.state_.a = cpu.read_memory(0x4B31);

    // 60D7: BNE $00DF
    if (!cpu.zero_flag()) cpu.state_.pc = 0x60B8;
    ...
    // 60EA: BCC $00EF
    if (!cpu.carry_flag()) cpu.state_.pc = 0x60DB;
    ...
    // 612C: BCS $011F
    if (cpu.carry_flag()) cpu.state_.pc = 0x614D;
    ...
    // 6141: BCS $0150
    if (cpu.carry_flag()) cpu.state_.pc = 0x6193;
    ...
    // 6167: BNE $016C
    if (!cpu.zero_flag()) cpu.state_.pc = 0x61D5;
    ...
    // 618C: BCS $0178
    if (cpu.carry_flag()) cpu.state_.pc = 0x6206;
    ...
    // 61A5: BCS $0191
    if (cpu.carry_flag()) cpu.state_.pc = 0x6138;
