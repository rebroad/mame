...
void routine_c6d4_impl(CPU6809& cpu) {
    ...
    // C6EF: BPL $0008
    if (!cpu.negative_flag()) cpu.m_pc = 0xC6F9;
    ...
    // C709: BPL $0028
    if (!cpu.negative_flag()) cpu.m_pc = 0xC733;
    ...
    // C71A: BCS $004A
    if (cpu.carry_flag()) cpu.m_pc = 0xC766;
    ...
    // C72C: BEQ $0060
    if (cpu.zero_flag()) cpu.m_pc = 0xC78E;
    ...
    // C7AA: BCS $0102
    if (cpu.carry_flag()) cpu.m_pc = 0xC7AE;
    ...
    // C840: BNE $0173
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8B5;
    ...
    // C854: BMI $019D
    if (cpu.negative_flag()) cpu.m_pc = 0xC7F3;
    ...
    // C85B: BNE $0199
    if (!cpu.zero_flag()) cpu.m_pc = 0xC7F6;
    ...
    // C862: BNE $0195
    if (!cpu.zero_flag()) cpu.m_pc = 0xC7F9;
    ...
    // C873: BNE $01B0
    if (!cpu.zero_flag()) cpu.m_pc = 0xC825;

    // C875: TST $4AEC
    // TODO: Convert TST $4AEC

    // C878: BMI $01AB
    if (cpu.negative_flag()) cpu.m_pc = 0xC825;
    ...
    // C88C: BMI $01D5
    if (cpu.negative_flag()) cpu.m_pc = 0xC863;
    ...
    // C893: BNE $01D1
    if (!cpu.zero_flag()) cpu.m_pc = 0xC866;
    ...
    // C8B0: BMI $01E3
    if (cpu.negative_flag()) cpu.m_pc = 0xC895;
