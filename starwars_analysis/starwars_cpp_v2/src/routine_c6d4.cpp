...
void routine_c6d4_impl(StarWarsCPU& cpu) {
    ...
    // C6EF: BPL $0008
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0008;
    ...
    // C709: BPL $0028
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0028;
    ...
    // C71A: BCS $004A
    if (cpu.carry_flag()) cpu.state_.pc += 0x004A;
    ...
    // C732: BNE $005A
    if (!cpu.zero_flag()) cpu.state_.pc += 0x005A;
    ...
    // C7AA: BCS $0102
    if (cpu.carry_flag()) cpu.state_.pc += 0x0102;
    ...
    // C85B: BNE $0199
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0199;
    ...
    // C862: BNE $0195
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0195;

    // C864: LDD $4B10
    cpu.state_.d = cpu.read_memory_word(0x4B10);

    // C867: BRA $0197
    cpu.state_.pc += 0x0197;

    // C869: LDD <$01
    cpu.state_.d = cpu.read_memory_word(0x01);

    // C86B: BRA $019B
    cpu.state_.pc += 0x019B;
    ...
    // C878: BMI $01AB
    if (cpu.negative_flag()) cpu.state_.pc += 0x01AB;

    // C87A: LDD $3054
    cpu.state_.d = cpu.read_memory_word(0x3054);

    // C87D: BRA $01AE
    cpu.state_.pc += 0x01AE;

    // C87F: LDD $3002
    cpu.state_.d = cpu.read_memory_word(0x3002);

    // C882: BRA $01B3
    cpu.state_.pc += 0x01B3;
    ...
    // C893: BNE $01D1
    if (!cpu.zero_flag()) cpu.state_.pc += 0x01D1;
    ...
    // C8B0: BMI $01E3
    if (cpu.negative_flag()) cpu.state_.pc += 0x01E3;
    ...
    // C8C4: BMI $020D
    if (cpu.negative_flag()) cpu.state_.pc += 0x020D;
    ...
    // C8CB: BNE $0209
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0209;
