...
void routine_c6f7_impl(StarWarsCPU& cpu) {
    ...
    // C709: BPL $0005
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0005;
    ...
    // C7AA: BCS $00DF
    if (cpu.carry_flag()) cpu.state_.pc += 0x00DF;
    ...
    // C854: BMI $017A
    if (cpu.negative_flag()) cpu.state_.pc += 0x017A;
    ...
    // C87D: BRA $018B
    cpu.state_.pc += 0x018B;

    // C87F: LDD $3002
    cpu.state_.d = cpu.read_memory_word(0x3002);

    // C882: BRA $0190
    cpu.state_.pc += 0x0190;
    ...
    // C88C: BMI $01B2
    if (cpu.negative_flag()) cpu.state_.pc += 0x01B2;
    ...
    // C893: BNE $01AE
    if (!cpu.zero_flag()) cpu.state_.pc += 0x01AE;
    ...
    // C8CB: BNE $01E6
    if (!cpu.zero_flag()) cpu.state_.pc += 0x01E6;
    ...
    // C8D2: BNE $01E2
    if (!cpu.zero_flag()) cpu.state_.pc += 0x01E2;
    ...
    // C8E3: BNE $01FD
    if (!cpu.zero_flag()) cpu.state_.pc += 0x01FD;

    // C8E5: TST $4AEC
    // TODO: Convert TST $4AEC

    // C8E8: BMI $01F8
    if (cpu.negative_flag()) cpu.state_.pc += 0x01F8;

    // C8EA: LDD $3054
    cpu.state_.d = cpu.read_memory_word(0x3054);

    // C8ED: BRA $01FB
    cpu.state_.pc += 0x01FB;
