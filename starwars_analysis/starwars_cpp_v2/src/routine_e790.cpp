...
void routine_e790_impl(CPU6809& cpu) {
    ...
    // E797: BNE $0012
    if (!cpu.zero_flag()) cpu.m_pc = 0xE7AB;

    // E799: TST <$AD
    // TODO: Convert TST <$AD

    // E79B: BLE $0012
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xE7AF;
    ...
    // E7B0: BNE $002B
    if (!cpu.zero_flag()) cpu.m_pc = 0xE7DD;

    // E7B2: TST <$AD
    // TODO: Convert TST <$AD

    // E7B4: BLE $002B
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xE7E1;
    ...
    // E7D5: BCC $004C
    if (!cpu.carry_flag()) cpu.m_pc = 0xE823;
    ...
    // E833: BCS $00AC
    if (cpu.carry_flag()) cpu.m_pc = 0xE7E1;
    ...
    // E83E: BNE $00B5
    if (!cpu.zero_flag()) cpu.m_pc = 0xE7F5;
    ...
    // E847: BNE $00BF
    if (!cpu.zero_flag()) cpu.m_pc = 0xE808;
    ...
    // E851: BCS $00C8
    if (cpu.carry_flag()) cpu.m_pc = 0xE81B;
    ...
    // E85A: BNE $00D1
    if (!cpu.zero_flag()) cpu.m_pc = 0xE82D;
    ...
    // E863: BNE $00DA
    if (!cpu.zero_flag()) cpu.m_pc = 0xE83F;
    ...
    // E86C: BNE $00E3
    if (!cpu.zero_flag()) cpu.m_pc = 0xE851;
    ...
    // E875: BNE $00EC
    if (!cpu.zero_flag()) cpu.m_pc = 0xE863;
    ...
    // E87E: BNE $00F5
    if (!cpu.zero_flag()) cpu.m_pc = 0xE875;
    ...
    // E891: BPL $009B
    if (!cpu.negative_flag()) cpu.m_pc = 0xE82E;
