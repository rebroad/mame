...
void routine_e790_impl(StarWarsCPU& cpu) {
    ...
    // E797: BNE $0012
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE7AB;

    // E799: TST <$AD
    // TODO: Convert TST <$AD

    // E79B: BLE $0012
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xE7AF;

    // E79D: LDU $3002
    cpu.state_.u = cpu.read_memory_word(0x3002);

    // E7A0: BRA $0019
    cpu.state_.pc = 0xE7BB;
    ...
    // E7B0: BNE $002B
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE7DD;

    // E7B2: TST <$AD
    // TODO: Convert TST <$AD

    // E7B4: BLE $002B
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xE7E1;
    ...
    // E7D5: BCC $004C
    if (!cpu.carry_flag()) cpu.state_.pc = 0xE823;
    ...
    // E970: ADDD $01C6,PCR
    // TODO: Fix comma operator: ADDD $01C6,PCR
    ...
    // E97A: ADDD [$01D0,PCR]
    // TODO: Complex indexed addressing: [$01D0,PCR]
