...
void routine_97c2_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_97c2.md
    // Address: 0x97C2
    ...
    // 97E6: BRA $004E
    cpu.state_.pc = 0x9836;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $004E
    cpu.state_.pc = 0x983B;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $004E
    cpu.state_.pc = 0x9840;

    // 97F2: LDU #$9850
    cpu.state_.u = 0x9850;

    // 97F5: BRA $004E
    cpu.state_.pc = 0x9845;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $004E
    cpu.state_.pc = 0x984A;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $004E
    cpu.state_.pc = 0x984F;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $004E
    cpu.state_.pc = 0x9854;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $004E
    cpu.state_.pc = 0x9859;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $004E
    cpu.state_.pc = 0x985E;
    ...
    // 9876: BNE $00BC
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9834;
    ...
    // 9892: BEQ $00D5
    if (cpu.zero_flag()) cpu.state_.pc = 0x9869;
    ...
    // 98A3: BPL $00E5
    if (!cpu.negative_flag()) cpu.state_.pc = 0x988A;
    ...
    // 98AD: BCS $00DF
    if (cpu.carry_flag()) cpu.state_.pc = 0x988E;
    ...
    // 98CD: LBRA $212E
    // TODO: Convert LBRA $212E

    // 98D0: BRA $0131
    cpu.state_.pc = 0x9903;

    // 98D2: BRN $0139
    // TODO: Convert BRN $0139

    // 98D4: BVC $0134
    // TODO: Convert BVC $0134

    // 98D6: BRA $013A
    cpu.state_.pc = 0x9912;

    // 98D8: BCC $013C
    if (!cpu.carry_flag()) cpu.state_.pc = 0x9916;

    // 98DA: BCS $013F
    if (cpu.carry_flag()) cpu.state_.pc = 0x991B;
