...
void routine_b3e4_impl(CPU6809& cpu) {
    ...
    // B3E9: BCS $000C
    if (cpu.carry_flag()) cpu.m_pc = 0xB3F7;
    ...
    // B47A: BCS $009B
    if (cpu.carry_flag()) cpu.m_pc = 0xB417;
    ...
    // B4A0: BNE $00D1
    if (!cpu.zero_flag()) cpu.m_pc = 0xB473;
    ...
    // B4BF: BGT $012C
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xB4ED;
    ...
    // B4C5: BNE $0127
    if (!cpu.zero_flag()) cpu.m_pc = 0xB4EE;
    ...
    // B4CD: BLT $0132
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xB501;

    // B4CF: LDD $5042
    cpu.m_d = cpu.read_memory16(0x5042);

    // B4D2: BGT $0127
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xB4FB;
    ...
    // B4DD: BLT $0127
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xB506;
    ...
    // B4F5: BLT $0119
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xB510;
    ...
    // B4FF: BNE $0127
    if (!cpu.zero_flag()) cpu.m_pc = 0xB528;
    ...
    // B506: BPL $0125
    if (!cpu.negative_flag()) cpu.m_pc = 0xB52D;
    ...
    // B52B: BNE $0181
    if (!cpu.zero_flag()) cpu.m_pc = 0xB4AE;
    ...
    // B534: BCC $0159
    if (!cpu.carry_flag()) cpu.m_pc = 0xB58F;

    // B536: BMI $0159
    if (cpu.negative_flag()) cpu.m_pc = 0xB591;
    ...
    // B53E: BNE $014F
    if (!cpu.zero_flag()) cpu.m_pc = 0xB58F;

    // B540: LDA $49BD
    cpu.m_a = cpu.read_memory(0x49BD);

    // B543: BEQ $0181
    if (cpu.zero_flag()) cpu.m_pc = 0xB4C6;
    ...
    // B54B: BCS $016B
    if (cpu.carry_flag()) cpu.m_pc = 0xB5B8;
    ...
    // B559: BCC $0179
    if (!cpu.carry_flag()) cpu.m_pc = 0xB5D4;
    ...
    // B5B4: BCS $01D5
    if (cpu.carry_flag()) cpu.m_pc = 0xB58B;
    ...
    // B5DA: BNE $020B
    if (!cpu.zero_flag()) cpu.m_pc = 0xB5E7;
