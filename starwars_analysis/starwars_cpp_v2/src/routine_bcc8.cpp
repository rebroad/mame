...
void routine_bcc8_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_bcc8.md
    // Address: 0xBCC8
    ...
    // BCE6: BNE $0002
    if (!cpu.zero_flag()) cpu.state_.pc = 0xBCEA;
    ...
    // BCEE: BPL $002D
    if (!cpu.negative_flag()) cpu.state_.pc = 0xBD1D;

    // BCF1: BNE $0023
    if (!cpu.zero_flag()) cpu.state_.pc = 0xBD16;
    ...
    // BE28: BCS $015A
    if (cpu.carry_flag()) cpu.state_.pc = 0xBE84;
    ...
    // BE52: BPL $0184
    if (!cpu.negative_flag()) cpu.state_.pc = 0xBDD8;
    ...
    // BE5F: BCS $0172
    if (cpu.carry_flag()) cpu.state_.pc = 0xBED3;
