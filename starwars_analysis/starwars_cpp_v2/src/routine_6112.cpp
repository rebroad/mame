...
void routine_6112_impl(CPU6809& cpu) {
    ...
    // 612C: BCS $0012
    if (cpu.carry_flag()) cpu.m_pc = 0x6140;
    ...
    // 6141: BCS $0043
    if (cpu.carry_flag()) cpu.m_pc = 0x6186;
    ...
    // 6167: BNE $005F
    if (!cpu.zero_flag()) cpu.m_pc = 0x61C8;
    ...
    // 618C: BCS $006B
    if (cpu.carry_flag()) cpu.m_pc = 0x61F9;
    ...
    // 61A5: BCS $0084
    if (cpu.carry_flag()) cpu.m_pc = 0x612B;
    ...
    // 620C: BCS $00E1
    if (cpu.carry_flag()) cpu.m_pc = 0x61EF;
    ...
    // 622A: BCS $0100
    if (cpu.carry_flag()) cpu.m_pc = 0x622C;
    ...
    // 6231: BGT $012C
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x625F;
    ...
    // 6237: BEQ $012A
    if (cpu.zero_flag()) cpu.m_pc = 0x6263;
    ...
    // 6240: BLT $0139
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x627B;
    ...
    // 6246: BEQ $0139
    if (cpu.zero_flag()) cpu.m_pc = 0x6281;
    ...
    // 6250: BNE $0145
    if (!cpu.zero_flag()) cpu.m_pc = 0x6297;
    ...
    // 625A: BEQ $0157
    if (cpu.zero_flag()) cpu.m_pc = 0x62B3;
    ...
    // 6260: BEQ $0157
    if (cpu.zero_flag()) cpu.m_pc = 0x62B9;
    ...
    // 626E: BNE $0162
    if (!cpu.zero_flag()) cpu.m_pc = 0x62D2;
