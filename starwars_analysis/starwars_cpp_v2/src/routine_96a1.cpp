...
void routine_96a1_impl(StarWarsCPU& cpu) {
    ...
    // 9725: BNE $0098
    if (!cpu.zero_flag()) cpu.state_.pc = 0x96BF;

    // 9727: LDB $4B15
    cpu.state_.b = cpu.read_memory(0x4B15);

    // 972A: BEQ $0098
    if (cpu.zero_flag()) cpu.state_.pc = 0x96C4;
    ...
    // 9777: BEQ $010A
    if (cpu.zero_flag()) cpu.state_.pc = 0x9783;
    ...
    // 97A3: BNE $00E6
    if (!cpu.zero_flag()) cpu.state_.pc = 0x978B;
    ...
    // 97B1: BCS $0117
    if (cpu.carry_flag()) cpu.state_.pc = 0x97CA;
    ...
    // 97EB: BRA $016F
    cpu.state_.pc = 0x985C;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $016F
    cpu.state_.pc = 0x9861;
    ...
    // 97FA: BRA $016F
    cpu.state_.pc = 0x986B;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $016F
    cpu.state_.pc = 0x9870;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $016F
    cpu.state_.pc = 0x9875;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $016F
    cpu.state_.pc = 0x987A;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $016F
    cpu.state_.pc = 0x987F;
    ...
    // 9876: BNE $01DD
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9855;
    ...
    // 9892: BEQ $01F6
    if (cpu.zero_flag()) cpu.state_.pc = 0x988A;
    ...
    // 989C: BNE $020E
    if (!cpu.zero_flag()) cpu.state_.pc = 0x98AC;
