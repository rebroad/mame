...
void routine_6dca_impl(CPU6809& cpu) {
    ...
    // 6DD4: BEQ $0026
    if (cpu.zero_flag()) cpu.m_pc = 0x6DFC;
    ...
    // 6DFC: BEQ $004E
    if (cpu.zero_flag()) cpu.m_pc = 0x6E4C;

    // 6DFE: BLE $003D
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E3D;
    ...
    // 6E24: BEQ $0064
    if (cpu.zero_flag()) cpu.m_pc = 0x6E8A;

    // 6E26: BLE $0061
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E89;
    ...
    // 6E30: BPL $0069
    if (!cpu.negative_flag()) cpu.m_pc = 0x6E9B;
    ...
    // 6E38: BPL $0074
    if (!cpu.negative_flag()) cpu.m_pc = 0x6EAE;
    ...
    // 6E42: BPL $007B
    if (!cpu.negative_flag()) cpu.m_pc = 0x6EBF;
    ...
    // 6E4A: BPL $0086
    if (!cpu.negative_flag()) cpu.m_pc = 0x6DD2;
    ...
    // 6E58: BLE $009B
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6DF5;
    ...
    // 6E69: BGE $00A4
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6E0F;
    ...
    // 6E72: BLE $00B5
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E29;

    // 6E74: CMPD #$0032
    // TODO: Convert CMPD #$0032

    // 6E78: BLE $00B3
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E2D;
    ...
    // 6E83: BGE $00BE
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6E43;
    ...
    // 6EA5: BNE $00E0
    if (!cpu.zero_flag()) cpu.m_pc = 0x6E87;
    ...
    // 6EAE: BNE $00EF
    if (!cpu.zero_flag()) cpu.m_pc = 0x6E9F;

    // 6EB0: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 6EB2: BNE $00EF
    if (!cpu.zero_flag()) cpu.m_pc = 0x6EA3;
    ...
    // 6EBC: BLE $00F9
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6EB7;
    ...
    // 6EC9: BCS $00E0
    if (cpu.carry_flag()) cpu.m_pc = 0x6EAB;
    ...
    // 6ED4: BMI $0111
    if (cpu.negative_flag()) cpu.m_pc = 0x6EE7;
    ...
    // 6EE6: BMI $0123
    if (cpu.negative_flag()) cpu.m_pc = 0x6F0B;
    ...
    // 6F14: BLE $016F
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6F85;
    ...
    // 6F8B: BPL $01C4
    if (!cpu.negative_flag()) cpu.m_pc = 0x6F51;

    // 6F90: TST <$7D
    // TODO: Convert TST <$7D

    // 6F92: BGE $01CE
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6F62;
    ...
    // 6FAF: BPL $01E8
    if (!cpu.negative_flag()) cpu.m_pc = 0x6F99;

    // 6FB4: TST <$7F
    // TODO: Convert TST <$7F

    // 6FB6: BPL $01F2
    if (!cpu.negative_flag()) cpu.m_pc = 0x6FAA;
    ...
    // 6FC9: BLE $0201
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6FCC;
