...
void routine_9810_impl(CPU6809& cpu) {
    ...
    // 9876: BNE $006E
    if (!cpu.zero_flag()) cpu.m_pc = 0x98E6;
    ...
    // 9892: BEQ $0087
    if (cpu.zero_flag()) cpu.m_pc = 0x981B;
    ...
    // 989C: BNE $009F
    if (!cpu.zero_flag()) cpu.m_pc = 0x983D;
    ...
    // 98A3: BPL $0097
    if (!cpu.negative_flag()) cpu.m_pc = 0x983C;
    ...
    // 98AD: BCS $0091
    if (cpu.carry_flag()) cpu.m_pc = 0x9840;
    ...
    // 98B4: BEQ $00BA
    if (cpu.zero_flag()) cpu.m_pc = 0x9870;
    ...
    // 98D8: BCC $00EE
    if (!cpu.carry_flag()) cpu.m_pc = 0x98C8;

    // 98DA: BCS $00F1
    if (cpu.carry_flag()) cpu.m_pc = 0x98CD;
