...
void routine_97e3_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_97e3.md
    // Address: 0x97E3
    ...
    // 97E6: BRA $002D
    cpu.state_.pc = 0x9815;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $002D
    cpu.state_.pc = 0x981A;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $002D
    cpu.state_.pc = 0x981F;

    // 97F2: LDU #$9850
    cpu.state_.u = 0x9850;

    // 97F5: BRA $002D
    cpu.state_.pc = 0x9824;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $002D
    cpu.state_.pc = 0x9829;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $002D
    cpu.state_.pc = 0x982E;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $002D
    cpu.state_.pc = 0x9833;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $002D
    cpu.state_.pc = 0x9838;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $002D
    cpu.state_.pc = 0x983D;
    ...
    // 9876: BNE $009B
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9913;
    ...
    // 9892: BEQ $00B4
    if (cpu.zero_flag()) cpu.state_.pc = 0x9948;
    ...
    // 989C: BNE $00CC
    if (!cpu.zero_flag()) cpu.state_.pc = 0x996A;
    ...
    // 98A3: BPL $00C4
    if (!cpu.negative_flag()) cpu.state_.pc = 0x9969;
    ...
    // 98AD: BCS $00BE
    if (cpu.carry_flag()) cpu.state_.pc = 0x996D;
    ...
    // 98B4: BEQ $00E7
    if (cpu.zero_flag()) cpu.state_.pc = 0x999D;
    ...
    // 98CD: LBRA $210D
    // TODO: Convert LBRA $210D

    // 98D0: BRA $0110
    cpu.state_.pc = 0x99E2;

    // 98D2: BRN $0118
    // TODO: Convert BRN $0118

    // 98D4: BVC $0113
    // TODO: Convert BVC $0113

    // 98D6: BRA $0119
    cpu.state_.pc = 0x99F1;

    // 98D8: BCC $011B
    if (!cpu.carry_flag()) cpu.state_.pc = 0x99F5;

    // 98DA: BCS $011E
    if (cpu.carry_flag()) cpu.state_.pc = 0x99FA;
