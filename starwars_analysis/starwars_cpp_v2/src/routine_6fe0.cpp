...
void routine_6fe0_impl(CPU6809& cpu) {
    ...
    // 6FE7: BLE $000F
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6FF8;
    ...
    // 6FF8: BLE $0020
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x701A;
    ...
    // 701C: BCC $0043
    if (!cpu.carry_flag()) cpu.m_pc = 0x7061;
    ...
    // 704F: BPL $0072
    if (!cpu.negative_flag()) cpu.m_pc = 0x70C3;

    // 7053: TST <$7D
    // TODO: Convert TST <$7D

    // 7055: BGE $007B
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x70D2;
    ...
    // 7068: BLE $008D
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6FF7;
    ...
    // 7071: BGE $0096
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x7009;
    ...
    // 7089: BPL $00AC
    if (!cpu.negative_flag()) cpu.m_pc = 0x7037;

    // 708E: TST <$7F
    // TODO: Convert TST <$7F

    // 7090: BPL $00B6
    if (!cpu.negative_flag()) cpu.m_pc = 0x7048;
    ...
    // 70A8: BLE $00CD
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x7077;
    ...
    // 70B1: BGE $00D6
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x7089;
    ...
    // 70C6: BEQ $00EB
    if (cpu.zero_flag()) cpu.m_pc = 0x70B3;
    ...
    // 70D5: BEQ $00FA
    if (cpu.zero_flag()) cpu.m_pc = 0x70D1;
    ...
    // 70F2: BPL $0115
    if (!cpu.negative_flag()) cpu.m_pc = 0x7109;
    ...
    // 7113: BPL $0136
    if (!cpu.negative_flag()) cpu.m_pc = 0x714B;

    // 7116: CMPA #$4E
    cpu.compare_a(0x4E);

    // 7118: BCS $015A
    if (cpu.carry_flag()) cpu.m_pc = 0x7174;
    ...
    // 7122: BPL $014F
    if (!cpu.negative_flag()) cpu.m_pc = 0x7173;
    ...
    // 713C: BCS $017E
    if (cpu.carry_flag()) cpu.m_pc = 0x71BC;
    ...
    // 7146: BPL $0173
    if (!cpu.negative_flag()) cpu.m_pc = 0x71BB;
    ...
    // 7163: BEQ $01A1
    if (cpu.zero_flag()) cpu.m_pc = 0x7106;
    ...
    // 716B: BPL $0196
    if (!cpu.negative_flag()) cpu.m_pc = 0x7103;
    ...
    // 7184: BEQ $01C2
    if (cpu.zero_flag()) cpu.m_pc = 0x7148;
    ...
    // 718C: BPL $01B7
    if (!cpu.negative_flag()) cpu.m_pc = 0x7145;
    ...
    // 71A5: BEQ $01E3
    if (cpu.zero_flag()) cpu.m_pc = 0x718A;
    ...
    // 71AD: BPL $01D8
    if (!cpu.negative_flag()) cpu.m_pc = 0x7187;
    ...
    // 71C4: BPL $01E7
    if (!cpu.negative_flag()) cpu.m_pc = 0x71AD;
