...
void routine_d883_impl(StarWarsCPU& cpu) {
    ...
    // D883: BCC $0006
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD88B;
    ...
    // D88F: BEQ $0029
    if (cpu.zero_flag()) cpu.state_.pc = 0xD8BA;
    ...
    // D897: BPL $0021
    if (!cpu.negative_flag()) cpu.state_.pc = 0xD8BA;

    // D899: TST <$13
    // TODO: Convert TST <$13

    // D89B: BPL $002B
    if (!cpu.negative_flag()) cpu.state_.pc = 0xD8C8;

    // D89E: BMI $002B
    if (cpu.negative_flag()) cpu.state_.pc = 0xD8CB;
    ...
    // D8A6: BNE $0027
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD8CF;
    ...
    // D8B2: BNE $005B
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD90F;
    ...
    // D8B9: BPL $003D
    if (!cpu.negative_flag()) cpu.state_.pc = 0xD8F8;
    ...
    // D8C5: BGE $0034
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xD8FB;

    // D8C8: BNE $005B
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD925;
    ...
    // D8DC: BGE $004A
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xD928;
    ...
    // D8E5: BCC $006E
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD955;

    // D8E7: CMPA ,X+
    // TODO: Complex indexed addressing: ,X+

    // D8E9: BNE $0069
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD954;
    ...
    // D8FC: BNE $008F
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD88D;
    ...
    // D917: BCS $0077
    if (cpu.carry_flag()) cpu.state_.pc = 0xD990;
    ...
    // D92A: BCC $00BE
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD8EA;
    ...
    // D949: BCC $00DA
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD925;
    ...
    // D976: BNE $00F9
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD971;
    ...
    // D982: BCS $00F1
    if (cpu.carry_flag()) cpu.state_.pc = 0xD975;
    ...
    // D98C: BCC $0158
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD9E6;
    ...
    // D995: BEQ $0150
    if (cpu.zero_flag()) cpu.state_.pc = 0xD9E7;
    ...
    // DA0F: BCC $019B
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD9AC;
