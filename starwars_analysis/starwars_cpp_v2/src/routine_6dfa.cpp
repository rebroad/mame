...
void routine_6dfa_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_6dfa.md
    // Address: 0x6DFA
    ...
    // 6DFC: BEQ $001E
    if (cpu.zero_flag()) cpu.state_.pc = 0x6E1C;
    ...
    // 6E24: BEQ $0034
    if (cpu.zero_flag()) cpu.state_.pc = 0x6E5A;

    // 6E26: BLE $0031
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6E59;
    ...
    // 6E30: BPL $0039
    if (!cpu.negative_flag()) cpu.state_.pc = 0x6E6B;
    ...
    // 6E38: BPL $0044
    if (!cpu.negative_flag()) cpu.state_.pc = 0x6E7E;
    ...
    // 6E4A: BPL $0056
    if (!cpu.negative_flag()) cpu.state_.pc = 0x6EA2;
    ...
    // 6E54: BNE $0076
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6ECC;

    // 6E56: SUBD <$A3
    cpu.state_.d -= 0xA3;

    // 6E58: BLE $006B
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6EC5;

    // 6E5A: CMPD #$0010
    // TODO: Convert CMPD #$0010

    // 6E5E: BLE $0069
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6EC9;

    // 6E60: LDD #$0010
    cpu.state_.d = 0x0010;

    // 6E63: BRA $0074
    cpu.state_.pc = 0x6ED9;

    // 6E65: CMPD #$FFF0
    // TODO: Convert CMPD #$FFF0

    // 6E69: BGE $0074
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x6EDF;

    // 6E6B: LDD #$FFF0
    cpu.state_.d = 0xFFF0;

    // 6E6E: BRA $008E
    cpu.state_.pc = 0x6EFE;

    // 6E70: SUBD <$A3
    cpu.state_.d -= 0xA3;

    // 6E72: BLE $0085
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6EF9;

    // 6E74: CMPD #$0032
    // TODO: Convert CMPD #$0032

    // 6E78: BLE $0083
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6EFD;
    ...
    // 6E83: BGE $008E
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x6F13;
    ...
    // 6E9B: BEQ $00A6
    if (cpu.zero_flag()) cpu.state_.pc = 0x6F43;
    ...
    // 6EA5: BNE $00B0
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6F57;
    ...
    // 6EAE: BNE $00BF
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6F6F;

    // 6EB0: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 6EB2: BNE $00BF
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6F73;
    ...
    // 6EBC: BLE $00C9
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6F87;
    ...
    // 6EC9: BCS $00B0
    if (cpu.carry_flag()) cpu.state_.pc = 0x6F7B;
    ...
    // 6ED4: BMI $00E1
    if (cpu.negative_flag()) cpu.state_.pc = 0x6FB7;

    // 6ED6: LDB $5084
    cpu.state_.b = cpu.read_memory(0x5084);

    // 6ED9: BRA $00E6
    cpu.state_.pc = 0x6FC1;
    ...
    // 6EE6: BMI $00F3
    if (cpu.negative_flag()) cpu.state_.pc = 0x6FDB;

    // 6EE8: LDB $5082
    cpu.state_.b = cpu.read_memory(0x5082);

    // 6EEB: BRA $00F8
    cpu.state_.pc = 0x6FE5;
    ...
    // 6F14: BLE $013F
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x7055;
    ...
    // 6F1F: BVS $015A
    // TODO: Convert BVS $015A

    // 6F23: BVC $012F
    // TODO: Convert BVC $012F

    // 6F27: BRA $015A
    cpu.state_.pc = 0x7083;
    ...
    // 6F40: BVC $014C
    // TODO: Convert BVC $014C

    // 6F44: BRA $015A
    cpu.state_.pc = 0x70A0;
    ...
    // 6F8B: BPL $0194
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7121;

    // 6F90: TST <$7D
    // TODO: Convert TST <$7D

    // 6F92: BGE $019E
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x7132;
    ...
    // 6FAF: BPL $01B8
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7169;

    // 6FB4: TST <$7F
    // TODO: Convert TST <$7F

    // 6FB6: BPL $01C2
    if (!cpu.negative_flag()) cpu.state_.pc = 0x717A;
    ...
    // 6FC9: BLE $01D6
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x71A1;

    // 6FCB: LDD #$1C00
    cpu.state_.d = 0x1C00;

    // 6FCE: BRA $01DF
    cpu.state_.pc = 0x71AF;

    // 6FD0: CMPD #$0200
    // TODO: Convert CMPD #$0200

    // 6FD4: BGE $01DF
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x71B5;
    ...
    // 6FE7: BLE $01F5
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x71DE;
    ...
    // 6FF8: BLE $0206
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x7200;
