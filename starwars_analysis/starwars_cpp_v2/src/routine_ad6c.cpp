...
void routine_ad6c_impl(CPU6809& cpu) {
    ...
    // AD6E: BEQ $0042
    if (cpu.zero_flag()) cpu.m_pc = 0xADB2;
    ...
    // AD7B: BMI $0013
    if (cpu.negative_flag()) cpu.m_pc = 0xAD90;
    ...
    // AD8B: BGE $0023
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xADB0;
    ...
    // AD98: BMI $0037
    if (cpu.negative_flag()) cpu.m_pc = 0xADD1;

    // AD9A: CMPD <$48
    // TODO: Convert CMPD <$48

    // AD9D: BGE $0035
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xADD4;
    ...
    // ADAA: BLE $0042
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xADEE;
    ...
    // ADB1: BEQ $0067
    if (cpu.zero_flag()) cpu.m_pc = 0xAE1A;
    ...
    // ADE5: BGT $007F
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xAE66;
    ...
    // ADF2: BLT $00F3
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xADE7;
    ...
    // ADFD: BPL $0097
    if (!cpu.negative_flag()) cpu.m_pc = 0xAD96;
    ...
    // AE06: BGE $00F3
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xADFB;

    // AE08: LDD $5004
    cpu.m_d = cpu.read_memory16(0x5004);

    // AE0B: BPL $00A5
    if (!cpu.negative_flag()) cpu.m_pc = 0xADB2;
    ...
    // AE14: BGE $00F3
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xAE09;
    ...
    // AE40: BEQ $00DC
    if (cpu.zero_flag()) cpu.m_pc = 0xAE1E;

    // AE44: BPL $00D2
    if (!cpu.negative_flag()) cpu.m_pc = 0xAE18;
    ...
    // AE7A: BEQ $011B
    if (cpu.zero_flag()) cpu.m_pc = 0xAE97;
    ...
    // AE89: BLE $0133
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xAEBE;
    ...
    // AEA5: BNE $0146
    if (!cpu.zero_flag()) cpu.m_pc = 0xAEED;

    // AEA7: LDA $4B36
    cpu.m_a = cpu.read_memory(0x4B36);

    // AEAA: BNE $0143
    if (!cpu.zero_flag()) cpu.m_pc = 0xAEEF;
    ...
    // AEC9: BEQ $01B9
    if (cpu.zero_flag()) cpu.m_pc = 0xAE84;
    ...
    // AF2A: BNE $021A
    if (!cpu.zero_flag()) cpu.m_pc = 0xAF46;
