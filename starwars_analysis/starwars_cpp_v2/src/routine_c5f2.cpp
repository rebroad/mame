...
void routine_c5f2_impl(StarWarsCPU& cpu) {
    ...
    // C5FF: BCS $0006
    if (cpu.carry_flag()) cpu.state_.pc = 0xC607;
    ...
    // C60C: BCS $0012
    if (cpu.carry_flag()) cpu.state_.pc = 0xC620;
    ...
    // C625: BCS $002B
    if (cpu.carry_flag()) cpu.state_.pc = 0xC652;
    ...
    // C630: BNE $0069
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC69B;
    ...
    // C63B: BEQ $004E
    if (cpu.zero_flag()) cpu.state_.pc = 0xC68B;
    ...
    // C64D: BNE $0068
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC6B7;

    // C64F: CMPX #$4600
    cpu.compare_x(0x4600);

    // C652: BCS $0055
    if (cpu.carry_flag()) cpu.state_.pc = 0xC6A9;
    ...
    // C674: BCS $007B
    if (cpu.carry_flag()) cpu.state_.pc = 0xC6F1;
    ...
    // C685: BCS $008E
    if (cpu.carry_flag()) cpu.state_.pc = 0xC615;
    ...
    // C68D: BNE $0099
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC628;

    // C690: LDA $4AF7
    cpu.state_.a = cpu.read_memory(0x4AF7);

    // C693: BEQ $00C5
    if (cpu.zero_flag()) cpu.state_.pc = 0xC65A;

    // C695: CMPA #$01
    cpu.compare_a(0x01);

    // C697: BNE $00AB
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC644;

    // C699: LDB #$9C
    cpu.state_.b = 0x9C;

    // C69B: BRA $00C2
    cpu.state_.pc = 0xC65F;
    ...
    // C6EF: BPL $00EA
    if (!cpu.negative_flag()) cpu.state_.pc = 0xC6DB;
    ...
    // C71A: BCS $012C
    if (cpu.carry_flag()) cpu.state_.pc = 0xC748;
    ...
    // C72C: BEQ $0142
    if (cpu.zero_flag()) cpu.state_.pc = 0xC770;
    ...
    // C7AA: BCS $01E4
    if (cpu.carry_flag()) cpu.state_.pc = 0xC790;
