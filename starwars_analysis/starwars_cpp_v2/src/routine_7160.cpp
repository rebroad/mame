...
void routine_7160_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_7160.md
    // Address: 0x7160
    ...
    // 7163: BEQ $0021
    if (cpu.zero_flag()) cpu.state_.pc = 0x7186;
    ...
    // 716B: BPL $0016
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7183;
    ...
    // 7184: BEQ $0042
    if (cpu.zero_flag()) cpu.state_.pc = 0x71C8;
    ...
    // 718C: BPL $0037
    if (!cpu.negative_flag()) cpu.state_.pc = 0x71C5;
    ...
    // 71A5: BEQ $0063
    if (cpu.zero_flag()) cpu.state_.pc = 0x720A;
    ...
    // 71AD: BPL $0058
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7207;
    ...
    // 71C4: BPL $0067
    if (!cpu.negative_flag()) cpu.state_.pc = 0x722D;
    ...
    // 71E1: BCS $0083
    if (cpu.carry_flag()) cpu.state_.pc = 0x7166;
    ...
    // 725F: BCC $00ED
    if (!cpu.carry_flag()) cpu.state_.pc = 0x724E;
    ...
    // 72E1: BCC $0190
    if (!cpu.carry_flag()) cpu.state_.pc = 0x7273;

    // 72E3: ANDB #$10
    cpu.state_.b &= 0x10;

    // 72E5: BNE $018B
    if (!cpu.zero_flag()) cpu.state_.pc = 0x7272;

    // 72E7: LDB #$4C
    cpu.state_.b = 0x4C;

    // 72E9: BRA $018D
    cpu.state_.pc = 0x7278;
