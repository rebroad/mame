...
void routine_89d3_impl(CPU6809& cpu) {
    ...
    // 8AD5: BNE $0108
    if (!cpu.zero_flag()) cpu.m_pc = 0x8ADF;

    // 8AD7: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 8AD9: BEQ $0109
    if (cpu.zero_flag()) cpu.m_pc = 0x8AE4;
    ...
    // 8AE3: BNE $011A
    if (!cpu.zero_flag()) cpu.m_pc = 0x8AFF;
    ...
    // 8B30: BCS $0160
    if (cpu.carry_flag()) cpu.m_pc = 0x8B92;
    ...
    // 8B42: BCS $0172
    if (cpu.carry_flag()) cpu.m_pc = 0x8BB6;
    ...
    // 8B61: BNE $0193
    if (!cpu.zero_flag()) cpu.m_pc = 0x8AF6;
    ...
    // 8B76: BEQ $01A8
    if (cpu.zero_flag()) cpu.m_pc = 0x8B20;
    ...
    // 8B83: BCS $019D
    if (cpu.carry_flag()) cpu.m_pc = 0x8B22;
    ...
    // 8B91: BNE $0203
    if (!cpu.zero_flag()) cpu.m_pc = 0x8B96;
    ...
    // 8B97: BLT $01C8
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8B61;
    ...
    // 8B9D: BGT $01CE
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8B6D;
    ...
    // 8BA7: BLT $01D8
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8B81;
    ...
    // 8BAD: BGT $01DE
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8B8D;
    ...
    // 8BC1: BPL $01F1
    if (!cpu.negative_flag()) cpu.m_pc = 0x8BB4;

    // 8BC4: CMPA #$08
    cpu.compare_a(0x08);

    // 8BC6: BGT $0203
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8BCB;

    // 8BC8: LDA $C,U
    // TODO: Fix comma operator: LDA $C,U

    // 8BCB: BPL $01FB
    if (!cpu.negative_flag()) cpu.m_pc = 0x8BC8;

    // 8BCE: CMPA #$08
    cpu.compare_a(0x08);

    // 8BD0: BGT $0203
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8BD5;
