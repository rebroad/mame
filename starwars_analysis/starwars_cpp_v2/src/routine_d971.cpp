...
void routine_d971_impl(StarWarsCPU& cpu) {
    ...
    // D976: BNE $000B
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD983;
    ...
    // D982: BCS $0003
    if (cpu.carry_flag()) cpu.state_.pc = 0xD987;
    ...
    // D995: BEQ $0062
    if (cpu.zero_flag()) cpu.state_.pc = 0xD9F9;
    ...
    // D9D9: BCS $0022
    if (cpu.carry_flag()) cpu.state_.pc = 0xD9FD;
    ...
    // DA0F: BCC $00AD
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD9BE;
    ...
    // DA9A: BCC $0184
    if (!cpu.carry_flag()) cpu.state_.pc = 0xDA20;
    ...
    // DAA9: BCS $0140
    if (cpu.carry_flag()) cpu.state_.pc = 0xDAEB;
    ...
    // DAB1: BRA $017D
    cpu.state_.pc = 0xDB30;

    // DAB3: CMPD #$0160
    // TODO: Convert CMPD #$0160

    // DAB7: BCC $0150
    if (!cpu.carry_flag()) cpu.state_.pc = 0xDB09;
    ...
    // DACB: BCS $017D
    if (cpu.carry_flag()) cpu.state_.pc = 0xDB4A;
    ...
    // DAD5: BNE $016E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xDB45;
    ...
    // DAE5: BCS $0162
    if (cpu.carry_flag()) cpu.state_.pc = 0xDB49;
    ...
    // DAF3: BCS $012B
    if (cpu.carry_flag()) cpu.state_.pc = 0xDB20;
    ...
    // DAFC: BCS $0192
    if (cpu.carry_flag()) cpu.state_.pc = 0xDA90;
    ...
    // DB07: BCS $01BD
    if (cpu.carry_flag()) cpu.state_.pc = 0xDAC6;
    ...
    // DB15: BCS $01AE
    if (cpu.carry_flag()) cpu.state_.pc = 0xDAC5;
    ...
    // DB49: BRA $020B
    cpu.state_.pc = 0xDB56;

    // DB4C: XANDCC #$33
    // TODO: Convert XANDCC #$33

    // DB4E: BRA $022B
    cpu.state_.pc = 0xDB7B;

    // DB58: BRA $0235
    cpu.state_.pc = 0xDB8F;

    // DB5C: BGT $020D
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xDB6B;

    // DB61: BRA $0233
    cpu.state_.pc = 0xDB96;

    // DB67: BGE $0241
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xDBAA;

    // DB6B: LDX $1,U
    // TODO: Fix comma operator: LDX $1,U

    // DB6F: BRA $0252
    cpu.state_.pc = 0xDBC3;
