...
void routine_6ff1_impl(StarWarsCPU& cpu) {
    ...
    // 7090: BPL $00A5
    if (!cpu.negative_flag()) cpu.state_.pc += 0x00A5;
    ...
    // 70C6: BEQ $00DA
    if (cpu.zero_flag()) cpu.state_.pc += 0x00DA;
    ...
    // 70FD: BMI $0116
    if (cpu.negative_flag()) cpu.state_.pc += 0x0116;
    ...
    // 7146: BPL $0162
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0162;
    ...
    // 718C: BPL $01A6
    if (!cpu.negative_flag()) cpu.state_.pc += 0x01A6;
    ...
    // 71A5: BEQ $01D2
    if (cpu.zero_flag()) cpu.state_.pc += 0x01D2;
