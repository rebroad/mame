...
void routine_8e1c_impl(StarWarsCPU& cpu) {
    ...
    // 8E1E: BGT $0006
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8E26;
    ...
    // 8E38: BEQ $0035
    if (cpu.zero_flag()) cpu.state_.pc = 0x8E6F;
    ...
    // 8E40: BNE $0040
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8E82;

    // 8E42: ANDB $14,X
    // TODO: Fix comma operator: ANDB $14,X

    // 8E45: BNE $0040
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8E87;
    ...
    // 8E60: BEQ $0016
    if (cpu.zero_flag()) cpu.state_.pc = 0x8E78;
    ...
    // 8E82: BNE $0078
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8EFC;
    ...
    // 8E8E: BNE $006D
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8EFD;

    // 8E90: BCC $006D
    if (!cpu.carry_flag()) cpu.state_.pc = 0x8EFF;

    // 8E92: BRA $005F
    cpu.state_.pc = 0x8EF3;
    ...
    // 8EE4: BCS $00D8
    if (cpu.carry_flag()) cpu.state_.pc = 0x8EBE;

    // 8EE6: LDA $4B14
    cpu.state_.a = cpu.read_memory(0x4B14);

    // 8EEA: BCS $00D5
    if (cpu.carry_flag()) cpu.state_.pc = 0x8EC1;

    // 8EEC: LDX #$9078
    cpu.state_.x = 0x9078;

    // 8EEF: BRA $00D8
    cpu.state_.pc = 0x8EC9;
    ...
    // 8F0E: BEQ $010D
    if (cpu.zero_flag()) cpu.state_.pc = 0x8F1D;

    // 8F10: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8F12: BEQ $010D
    if (cpu.zero_flag()) cpu.state_.pc = 0x8F21;
    ...
    // 8F31: BCS $00EC
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F1F;
    ...
    // 8F7D: BEQ $0167
    if (cpu.zero_flag()) cpu.state_.pc = 0x8FE6;

    // 8F7F: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8F81: BNE $0195
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8F18;
    ...
    // 8F90: BCS $0184
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F16;

    // 8F92: LDA $4B14
    cpu.state_.a = cpu.read_memory(0x4B14);

    // 8F96: BCS $0181
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F19;

    // 8F98: LDX #$9078
    cpu.state_.x = 0x9078;

    // 8F9B: BRA $0184
    cpu.state_.pc = 0x8F21;
    ...
    // 8FB8: BEQ $01AA
    if (cpu.zero_flag()) cpu.state_.pc = 0x8F64;
    ...
    // 8FC2: BCS $0198
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F5C;

    // 8FC4: BRA $0207
    cpu.state_.pc = 0x8FCD;

    // 8FC6: LDU <$E4
    cpu.state_.u = cpu.read_memory_word(0xE4);

    // 8FC8: BEQ $0207
    if (cpu.zero_flag()) cpu.state_.pc = 0x8FD1;

    // 8FCA: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8FCC: BEQ $0207
    if (cpu.zero_flag()) cpu.state_.pc = 0x8FD5;
    ...
    // 901B: LDX <$00
    cpu.state_.x = cpu.read_memory_word(0x00);
