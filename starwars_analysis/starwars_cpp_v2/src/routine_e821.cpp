...
void routine_e821_impl(StarWarsCPU& cpu) {
    ...
    // E833: BCS $001B
    if (cpu.carry_flag()) cpu.state_.pc = 0xE850;
    ...
    // E83A: BRA $0067
    cpu.state_.pc = 0xE8A3;

    // E83C: CMPB #$74
    cpu.compare_b(0x74);

    // E83E: BNE $0024
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE864;

    // E840: LDX #$2FDE
    cpu.state_.x = 0x2FDE;

    // E843: BRA $0067
    cpu.state_.pc = 0xE8AC;

    // E845: CMPB #$80
    cpu.compare_b(0x80);

    // E847: BNE $002E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE877;

    // E849: LDX #$3058
    cpu.state_.x = 0x3058;

    // E84D: BRA $0067
    cpu.state_.pc = 0xE8B6;

    // E84F: CMPB #$60
    cpu.compare_b(0x60);

    // E851: BCS $0037
    if (cpu.carry_flag()) cpu.state_.pc = 0xE88A;

    // E853: LDX #$2FA4
    cpu.state_.x = 0x2FA4;

    // E856: BRA $0067
    cpu.state_.pc = 0xE8BF;
    ...
    // E863: BNE $0049
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8AE;
    ...
    // E86C: BNE $0052
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8C0;

    // E86E: LDX #$2FF6
    cpu.state_.x = 0x2FF6;

    // E871: BRA $0067
    cpu.state_.pc = 0xE8DA;

    // E873: CMPB #$5A
    cpu.compare_b(0x5A);

    // E875: BNE $005B
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8D2;

    // E877: LDX #$3000
    cpu.state_.x = 0x3000;

    // E87A: BRA $0067
    cpu.state_.pc = 0xE8E3;
    ...
    // E891: BPL $000A
    if (!cpu.negative_flag()) cpu.state_.pc = 0xE89D;
