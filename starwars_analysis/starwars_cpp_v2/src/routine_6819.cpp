...
void routine_6819_impl(StarWarsCPU& cpu) {
    ...
    // 684F: BNE $003E
    if (!cpu.zero_flag()) cpu.state_.pc = 0x688F;
    ...
    // 688F: BNE $008D
    if (!cpu.zero_flag()) cpu.state_.pc = 0x681E;
    ...
    // 6896: BLT $0088
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6820;

    // 6898: ANDA #$01
    cpu.state_.a &= 0x01;

    // 689A: BEQ $0088
    if (cpu.zero_flag()) cpu.state_.pc = 0x6824;

    // 689C: JSR $BD85
    cpu.call_function(0xBD85);

    // 689F: BRA $008B
    cpu.state_.pc = 0x682C;

    // 68A1: JSR $BDA8
    cpu.call_function(0xBDA8);

    // 68A4: BRA $00AD
    cpu.state_.pc = 0x6853;

    // 68A6: CMPD #$00C8
    // TODO: Convert CMPD #$00C8

    // 68AA: BNE $0098
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6844;

    // 68AC: JSR $BDAD
    cpu.call_function(0xBDAD);

    // 68AF: BRA $00AD
    cpu.state_.pc = 0x685E;

    // 68B1: CMPD #$0190
    // TODO: Convert CMPD #$0190

    // 68B5: BNE $00A3
    if (!cpu.zero_flag()) cpu.state_.pc = 0x685A;

    // 68B7: JSR $BD8A
    cpu.call_function(0xBD8A);

    // 68BA: BRA $00AD
    cpu.state_.pc = 0x6869;

    // 68BC: CMPD #$01A4
    // TODO: Convert CMPD #$01A4

    // 68C0: BCS $00AD
    if (cpu.carry_flag()) cpu.state_.pc = 0x686F;
    ...
    // 68CA: BCC $00B6
    if (!cpu.carry_flag()) cpu.state_.pc = 0x6882;
    ...
    // 6903: BNE $00F8
    if (!cpu.zero_flag()) cpu.state_.pc = 0x68FD;
    ...
    // 690B: BCS $00E8
    if (cpu.carry_flag()) cpu.state_.pc = 0x68F5;
    ...
    // 691F: BNE $0111
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6932;
    ...
    // 6926: BNE $0111
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6939;

    // 6928: BRA $0117
    cpu.state_.pc = 0x6941;
    ...
    // 694C: BLT $0139
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6987;
    ...
    // 695D: BNE $0149
    if (!cpu.zero_flag()) cpu.state_.pc = 0x69A8;
    ...
    // 698D: BNE $017A
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6A09;

    // 698F: LDA #$27
    cpu.state_.a = 0x27;

    // 6991: BRA $017C
    cpu.state_.pc = 0x6A0F;
    ...
    // 69AF: BCS $019A
    if (cpu.carry_flag()) cpu.state_.pc = 0x694B;
