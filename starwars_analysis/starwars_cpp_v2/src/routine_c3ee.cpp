...
void routine_c3ee_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_c3ee.md
    // Address: 0xC3EE
    ...
    // C410: BCS $001B
    if (cpu.carry_flag()) cpu.state_.pc = 0xC42D;

    // C413: JSR $C2C3
    cpu.call_function(0xC2C3);

    // C416: BEQ $0061
    if (cpu.zero_flag()) cpu.state_.pc = 0xC479;
    ...
    // C428: BCS $0033
    if (cpu.carry_flag()) cpu.state_.pc = 0xC45D;
    ...
    // C43F: BCS $0046
    if (cpu.carry_flag()) cpu.state_.pc = 0xC487;
    ...
    // C44A: BEQ $0061
    if (cpu.zero_flag()) cpu.state_.pc = 0xC4AD;
    ...
    // C458: BCS $0064
    if (cpu.carry_flag()) cpu.state_.pc = 0xC4BE;
    ...
    // C472: BPL $007B
    if (!cpu.negative_flag()) cpu.state_.pc = 0xC4EF;
    ...
    // C47C: BEQ $00FC
    if (cpu.zero_flag()) cpu.state_.pc = 0xC47A;
    ...
    // C487: BEQ $00FC
    if (cpu.zero_flag()) cpu.state_.pc = 0xC485;
    ...
    // C4AA: BCS $00B7
    if (cpu.carry_flag()) cpu.state_.pc = 0xC463;
    ...
    // C4B6: BEQ $00EC
    if (cpu.zero_flag()) cpu.state_.pc = 0xC4A4;
    ...
    // C4D8: BNE $00E3
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC4BD;
    ...
    // C4DF: BEQ $00F6
    if (cpu.zero_flag()) cpu.state_.pc = 0xC4D7;
    ...
    // C4EF: BNE $012B
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC51C;
    ...
    // C4F6: BCC $012B
    if (!cpu.carry_flag()) cpu.state_.pc = 0xC523;

    // C4F8: CMPB #$30
    cpu.compare_b(0x30);

    // C4FA: BLS $012B
    // TODO: Convert BLS $012B

    // C4FD: BMI $011E
    if (cpu.negative_flag()) cpu.state_.pc = 0xC51D;

    // C4FF: LDB $4AF6
    cpu.state_.b = cpu.read_memory(0x4AF6);

    // C503: BPL $0119
    if (!cpu.negative_flag()) cpu.state_.pc = 0xC51E;
    ...
    // C54F: BMI $0169
    if (cpu.negative_flag()) cpu.state_.pc = 0xC5BA;

    // C552: ASL $4AFA
    // TODO: Convert ASL $4AFA

    // C555: BRA $0160
    cpu.state_.pc = 0xC5B7;
    ...
    // C598: BCS $01A3
    if (cpu.carry_flag()) cpu.state_.pc = 0xC53D;
    ...
    // C5DE: BMI $01F5
    if (cpu.negative_flag()) cpu.state_.pc = 0xC5D5;

    // C5E1: BRA $01ED
    cpu.state_.pc = 0xC5D0;
    ...
    // C5ED: ADDB $0,X
    // TODO: Fix comma operator: ADDB $0,X
