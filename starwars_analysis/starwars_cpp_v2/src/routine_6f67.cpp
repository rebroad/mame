...
void routine_6f67_impl(StarWarsCPU& cpu) {
    ...
    // 6FE7: BLE $0088
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6F71;
    ...
    // 6FF8: BLE $0099
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6F93;
    ...
    // 701C: BCC $00BC
    if (!cpu.carry_flag()) cpu.state_.pc = 0x6FDA;

    // 701E: LDD #$0BB8
    cpu.state_.d = 0x0BB8;

    // 7021: BRA $00BF
    cpu.state_.pc = 0x6FE2;
    ...
    // 704F: BPL $00EB
    if (!cpu.negative_flag()) cpu.state_.pc = 0x703C;

    // 7053: TST <$7D
    // TODO: Convert TST <$7D

    // 7055: BGE $00F4
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x704B;
    ...
    // 7068: BLE $0106
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x7070;
    ...
    // 7071: BGE $010F
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x7082;
    ...
    // 7089: BPL $0125
    if (!cpu.negative_flag()) cpu.state_.pc = 0x70B0;

    // 708E: TST <$7F
    // TODO: Convert TST <$7F

    // 7090: BPL $012F
    if (!cpu.negative_flag()) cpu.state_.pc = 0x70C1;
    ...
    // 70A8: BLE $0146
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x70F0;
    ...
    // 70B1: BGE $014F
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x7102;
    ...
    // 70C6: BEQ $0164
    if (cpu.zero_flag()) cpu.state_.pc = 0x712C;
    ...
    // 70D5: BEQ $0173
    if (cpu.zero_flag()) cpu.state_.pc = 0x714A;
    ...
    // 70F2: BPL $018E
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7082;
    ...
    // 70FD: BMI $01A0
    if (cpu.negative_flag()) cpu.state_.pc = 0x709F;
    ...
    // 7113: BPL $01AF
    if (!cpu.negative_flag()) cpu.state_.pc = 0x70C4;

    // 7116: CMPA #$4E
    cpu.compare_a(0x4E);

    // 7118: BCS $01D3
    if (cpu.carry_flag()) cpu.state_.pc = 0x70ED;
    ...
    // 7122: BPL $01C8
    if (!cpu.negative_flag()) cpu.state_.pc = 0x70EC;
    ...
    // 713C: BCS $01F7
    if (cpu.carry_flag()) cpu.state_.pc = 0x7135;
    ...
    // 7163: BEQ $021A
    if (cpu.zero_flag()) cpu.state_.pc = 0x717F;
