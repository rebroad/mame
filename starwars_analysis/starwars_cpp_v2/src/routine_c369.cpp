...
void routine_c369_impl(StarWarsCPU& cpu) {
    ...
    // C3D9: BNE $005F
    if (!cpu.zero_flag()) cpu.state_.pc += 0x005F;
    ...
    // C410: BCS $00A0
    if (cpu.carry_flag()) cpu.state_.pc += 0x00A0;
    ...
    // C43F: BCS $00CB
    if (cpu.carry_flag()) cpu.state_.pc += 0x00CB;
    ...
    // C458: BCS $00E9
    if (cpu.carry_flag()) cpu.state_.pc += 0x00E9;
    ...
    // C535: BEQ $023A
    if (cpu.zero_flag()) cpu.state_.pc += 0x023A;
    ...
    // C54F: BMI $01EE
    if (cpu.negative_flag()) cpu.state_.pc += 0x01EE;
