...
void routine_c2b3_impl(StarWarsCPU& cpu) {
    ...
    // C345: BCS $0087
    if (cpu.carry_flag()) cpu.state_.pc = 0xC2CE;
    ...
    // C353: BEQ $00B9
    if (cpu.zero_flag()) cpu.state_.pc = 0xC30E;
    ...
    // C360: BEQ $00C9
    if (cpu.zero_flag()) cpu.state_.pc = 0xC32B;
    ...
    // C367: BEQ $00B9
    if (cpu.zero_flag()) cpu.state_.pc = 0xC322;
    ...
    // C381: BNE $00F4
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC377;
    ...
    // C393: BNE $00F2
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC387;
    ...
    // C3A5: BEQ $0113
    if (cpu.zero_flag()) cpu.state_.pc = 0xC3BA;
    ...
    // C3C4: BNE $010A
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC3D0;
    ...
    // C3CB: BEQ $0125
    if (cpu.zero_flag()) cpu.state_.pc = 0xC3F2;
    ...
    // C3D3: BEQ $0125
    if (cpu.zero_flag()) cpu.state_.pc = 0xC3FA;

    // C3D5: JSR $C24E
    cpu.call_function(0xC24E);

    // C3D9: BNE $0115
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC3F0;
    ...
    // C410: BCS $0156
    if (cpu.carry_flag()) cpu.state_.pc = 0xC468;

    // C413: JSR $C2C3
    cpu.call_function(0xC2C3);

    // C416: BEQ $019C
    if (cpu.zero_flag()) cpu.state_.pc = 0xC3B4;
    ...
    // C428: BCS $016E
    if (cpu.carry_flag()) cpu.state_.pc = 0xC498;
    ...
    // C43F: BCS $0181
    if (cpu.carry_flag()) cpu.state_.pc = 0xC3C2;
    ...
    // C44A: BEQ $019C
    if (cpu.zero_flag()) cpu.state_.pc = 0xC3E8;
    ...
    // C458: BCS $019F
    if (cpu.carry_flag()) cpu.state_.pc = 0xC3F9;
    ...
    // C472: BPL $01B6
    if (!cpu.negative_flag()) cpu.state_.pc = 0xC42A;
    ...
    // C47C: BEQ $0237
    if (cpu.zero_flag()) cpu.state_.pc = 0xC4B5;
    ...
    // C48E: BEQ $01E5
    if (cpu.zero_flag()) cpu.state_.pc = 0xC475;
    ...
    // C4AA: BCS $01F2
    if (cpu.carry_flag()) cpu.state_.pc = 0xC49E;
