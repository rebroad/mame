...
void routine_987f_impl(CPU6809& cpu) {
    ...
    // 989C: BNE $0030
    if (!cpu.zero_flag()) cpu.m_pc = 0x98CE;
    ...
    // 98A3: BPL $0028
    if (!cpu.negative_flag()) cpu.m_pc = 0x98CD;
    ...
    // 98AD: BCS $0022
    if (cpu.carry_flag()) cpu.m_pc = 0x98D1;
    ...
    // 98B4: BEQ $004B
    if (cpu.zero_flag()) cpu.m_pc = 0x9901;
    ...
    // 98D8: BCC $007F
    if (!cpu.carry_flag()) cpu.m_pc = 0x9959;

    // 98DA: BCS $0082
    if (cpu.carry_flag()) cpu.m_pc = 0x985E;
