...
void routine_e7d3_impl(StarWarsCPU& cpu) {
    ...
    // E7D5: BCC $0009
    if (!cpu.carry_flag()) cpu.state_.pc = 0xE7E0;
    ...
    // E833: BCS $0069
    if (cpu.carry_flag()) cpu.state_.pc = 0xE89E;
    ...
    // E83A: BRA $00B5
    cpu.state_.pc = 0xE7F1;

    // E83C: CMPB #$74
    cpu.compare_b(0x74);

    // E83E: BNE $0072
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8B2;
    ...
    // E847: BNE $007C
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8C5;
    ...
    // E851: BCS $0085
    if (cpu.carry_flag()) cpu.state_.pc = 0xE7D8;
    ...
    // E85A: BNE $008E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE7EA;

    // E85C: LDX #$2FC2
    cpu.state_.x = 0x2FC2;

    // E85F: BRA $00B5
    cpu.state_.pc = 0xE816;

    // E861: CMPB #$4E
    cpu.compare_b(0x4E);

    // E863: BNE $0097
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE7FC;

    // E865: LDX #$2FFE
    cpu.state_.x = 0x2FFE;

    // E868: BRA $00B5
    cpu.state_.pc = 0xE81F;

    // E86A: CMPB #$58
    cpu.compare_b(0x58);

    // E86C: BNE $00A0
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE80E;

    // E86E: LDX #$2FF6
    cpu.state_.x = 0x2FF6;

    // E871: BRA $00B5
    cpu.state_.pc = 0xE828;

    // E873: CMPB #$5A
    cpu.compare_b(0x5A);

    // E875: BNE $00A9
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE820;

    // E877: LDX #$3000
    cpu.state_.x = 0x3000;

    // E87A: BRA $00B5
    cpu.state_.pc = 0xE831;

    // E87C: CMPB #$4A
    cpu.compare_b(0x4A);

    // E87E: BNE $00B2
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE832;
    ...
    // E891: BPL $0058
    if (!cpu.negative_flag()) cpu.state_.pc = 0xE8EB;
