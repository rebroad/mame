...
void routine_8735_impl(StarWarsCPU& cpu) {
    ...
    // 873C: BNE $000A
    if (!cpu.zero_flag()) cpu.state_.pc += 0x000A;
    ...
    // 8829: BPL $00FA
    if (!cpu.negative_flag()) cpu.state_.pc += 0x00FA;
    ...
    // 8888: BEQ $0169
    if (cpu.zero_flag()) cpu.state_.pc += 0x0169;
    ...
    // 88FC: BEQ $021B
    if (cpu.zero_flag()) cpu.state_.pc += 0x021B;
