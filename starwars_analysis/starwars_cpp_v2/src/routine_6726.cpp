...
void routine_6726_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_6726.md
    // Address: 0x6726
    ...
    // 672C: STU $2608
    cpu.write_memory(0x2608, cpu.state_.u);

    // 672F: LDA $4B06
    cpu.state_.a = cpu.read_memory(0x4B06);

    // 6732: STA $4B34
    cpu.write_memory(0x4B34, cpu.state_.a);

    // 6735: BRA $0032
    cpu.state_.pc = 0x6769;
    ...
    // 673D: BEQ $0032
    if (cpu.zero_flag()) cpu.state_.pc = 0x6771;
    ...
    // 674A: BNE $0032
    if (!cpu.zero_flag()) cpu.state_.pc = 0x677E;
    ...
    // 6764: EORA [$99FF,PCR]
    // TODO: Complex indexed addressing: [$99FF,PCR]
    ...
    // 684F: BNE $0131
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6982;
    ...
    // 688F: BNE $0180
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6A11;
    ...
    // 6896: BLT $017B
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6A13;

    // 6898: ANDA #$01
    cpu.state_.a &= 0x01;

    // 689A: BEQ $017B
    if (cpu.zero_flag()) cpu.state_.pc = 0x6A17;

    // 689C: JSR $BD85
    cpu.call_function(0xBD85);

    // 689F: BRA $017E
    cpu.state_.pc = 0x6A1F;

    // 68A1: JSR $BDA8
    cpu.call_function(0xBDA8);

    // 68A4: BRA $01A0
    cpu.state_.pc = 0x6A46;

    // 68A6: CMPD #$00C8
    // TODO: Convert CMPD #$00C8

    // 68AA: BNE $018B
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6A37;

    // 68AC: JSR $BDAD
    cpu.call_function(0xBDAD);

    // 68AF: BRA $01A0
    cpu.state_.pc = 0x6A51;

    // 68B1: CMPD #$0190
    // TODO: Convert CMPD #$0190

    // 68B5: BNE $0196
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6A4D;

    // 68B7: JSR $BD8A
    cpu.call_function(0xBD8A);

    // 68BA: BRA $01A0
    cpu.state_.pc = 0x6A5C;

    // 68BC: CMPD #$01A4
    // TODO: Convert CMPD #$01A4

    // 68C0: BCS $01A0
    if (cpu.carry_flag()) cpu.state_.pc = 0x6A62;
    ...
    // 68CA: BCC $01A9
    if (!cpu.carry_flag()) cpu.state_.pc = 0x6A75;
    ...
    // 6903: BNE $01EB
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6AF0;
    ...
    // 690B: BCS $01DB
    if (cpu.carry_flag()) cpu.state_.pc = 0x6AE8;
    ...
    // 691F: BNE $0204
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6B25;
