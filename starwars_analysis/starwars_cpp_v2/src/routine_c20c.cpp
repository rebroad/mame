...
void routine_c20c_impl(StarWarsCPU& cpu) {
    ...
    // C250: BCC $00A7
    if (!cpu.carry_flag()) cpu.state_.pc += 0x00A7;

    // C252: CMPA #$02
    cpu.compare_a(0x02);

    // C254: BCS $0073
    if (cpu.carry_flag()) cpu.state_.pc += 0x0073;
    ...
    // C27B: BCS $0068
    if (cpu.carry_flag()) cpu.state_.pc += 0x0068;
    ...
    // C313: BCS $0100
    if (cpu.carry_flag()) cpu.state_.pc += 0x0100;
