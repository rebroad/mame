...
void routine_c6f4_impl(CPU6809& cpu) {
    ...
    // C709: BPL $0008
    if (!cpu.negative_flag()) cpu.m_pc = 0xC713;
    ...
    // C71A: BCS $002A
    if (cpu.carry_flag()) cpu.m_pc = 0xC746;
    ...
    // C72C: BEQ $0040
    if (cpu.zero_flag()) cpu.m_pc = 0xC76E;
    ...
    // C7AA: BCS $00E2
    if (cpu.carry_flag()) cpu.m_pc = 0xC78E;
    ...
    // C840: BNE $0153
    if (!cpu.zero_flag()) cpu.m_pc = 0xC895;
    ...
    // C854: BMI $017D
    if (cpu.negative_flag()) cpu.m_pc = 0xC8D3;
    ...
    // C873: BNE $0190
    if (!cpu.zero_flag()) cpu.m_pc = 0xC805;

    // C875: TST $4AEC
    // TODO: Convert TST $4AEC

    // C878: BMI $018B
    if (cpu.negative_flag()) cpu.m_pc = 0xC805;
    ...
    // C88C: BMI $01B5
    if (cpu.negative_flag()) cpu.m_pc = 0xC843;
    ...
    // C89A: BNE $01AD
    if (!cpu.zero_flag()) cpu.m_pc = 0xC849;
    ...
    // C8AB: BNE $01C8
    if (!cpu.zero_flag()) cpu.m_pc = 0xC875;

    // C8AD: TST $4AEC
    // TODO: Convert TST $4AEC

    // C8B0: BMI $01C3
    if (cpu.negative_flag()) cpu.m_pc = 0xC875;
    ...
    // C8C4: BMI $01ED
    if (cpu.negative_flag()) cpu.m_pc = 0xC8B3;
    ...
    // C8D2: BNE $01E5
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8B9;
    ...
    // C8E3: BNE $0200
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8E5;

    // C8E5: TST $4AEC
    // TODO: Convert TST $4AEC

    // C8E8: BMI $01FB
    if (cpu.negative_flag()) cpu.m_pc = 0xC8E5;
