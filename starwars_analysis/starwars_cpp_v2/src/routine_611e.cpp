...
void routine_611e_impl(CPU6809& cpu) {
    ...
    // 612C: BCS $0006
    if (cpu.carry_flag()) cpu.m_pc = 0x6134;
    ...
    // 6141: BCS $0037
    if (cpu.carry_flag()) cpu.m_pc = 0x617A;
    ...
    // 6167: BNE $0053
    if (!cpu.zero_flag()) cpu.m_pc = 0x61BC;
    ...
    // 618C: BCS $005F
    if (cpu.carry_flag()) cpu.m_pc = 0x61ED;
    ...
    // 61A5: BCS $0078
    if (cpu.carry_flag()) cpu.m_pc = 0x621F;
    ...
    // 622A: BCS $00F4
    if (cpu.carry_flag()) cpu.m_pc = 0x6220;
    ...
    // 6231: BGT $0120
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6253;
    ...
    // 6240: BLT $012D
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x626F;
    ...
    // 6246: BEQ $012D
    if (cpu.zero_flag()) cpu.m_pc = 0x6275;
    ...
    // 6250: BNE $0139
    if (!cpu.zero_flag()) cpu.m_pc = 0x628B;
    ...
    // 6260: BEQ $014B
    if (cpu.zero_flag()) cpu.m_pc = 0x62AD;
    ...
    // 626E: BNE $0156
    if (!cpu.zero_flag()) cpu.m_pc = 0x62C6;
