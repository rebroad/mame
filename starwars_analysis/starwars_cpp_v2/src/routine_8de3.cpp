...
void routine_8de3_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_8de3.md
    // Address: 0x8DE3
    ...
    // 8DE9: BLT $000B
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8DF6;
    ...
    // 8DF0: BGT $0012
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8E04;
    ...
    // 8DFB: BLT $001D
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8E1A;
    ...
    // 8E02: BGT $0024
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8E28;
    ...
    // 8E0D: BLT $002F
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8E3E;
    ...
    // 8E14: BGT $0036
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8E4C;
    ...
    // 8E1E: BGT $003F
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8E5F;
    ...
    // 8E38: BEQ $006E
    if (cpu.zero_flag()) cpu.state_.pc = 0x8EA8;
    ...
    // 8E40: BNE $0079
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8EBB;

    // 8E42: ANDB $14,X
    // TODO: Fix comma operator: ANDB $14,X

    // 8E45: BNE $0079
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8EC0;
    ...
    // 8E60: BEQ $004F
    if (cpu.zero_flag()) cpu.state_.pc = 0x8EB1;
    ...
    // 8E82: BNE $00B1
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8E35;

    // 8E84: ANDB $16,X
    // TODO: Fix comma operator: ANDB $16,X

    // 8E87: BNE $00B1
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8E3A;
    ...
    // 8E8E: BNE $00A6
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8E36;

    // 8E90: BCC $00A6
    if (!cpu.carry_flag()) cpu.state_.pc = 0x8E38;

    // 8E92: BRA $0098
    cpu.state_.pc = 0x8E2C;
    ...
    // 8EE4: BCS $0111
    if (cpu.carry_flag()) cpu.state_.pc = 0x8EF7;

    // 8EE6: LDA $4B14
    cpu.state_.a = cpu.read_memory(0x4B14);

    // 8EEA: BCS $010E
    if (cpu.carry_flag()) cpu.state_.pc = 0x8EFA;

    // 8EEC: LDX #$9078
    cpu.state_.x = 0x9078;

    // 8EEF: BRA $0111
    cpu.state_.pc = 0x8F02;
    ...
    // 8F0E: BEQ $0146
    if (cpu.zero_flag()) cpu.state_.pc = 0x8F56;

    // 8F10: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8F12: BEQ $0146
    if (cpu.zero_flag()) cpu.state_.pc = 0x8F5A;
    ...
    // 8F31: BCS $0125
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F58;
    ...
    // 8F7D: BEQ $01A0
    if (cpu.zero_flag()) cpu.state_.pc = 0x8F1F;

    // 8F7F: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8F81: BNE $01CE
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8F51;
    ...
    // 8F90: BCS $01BD
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F4F;

    // 8F92: LDA $4B14
    cpu.state_.a = cpu.read_memory(0x4B14);

    // 8F96: BCS $01BA
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F52;

    // 8F98: LDX #$9078
    cpu.state_.x = 0x9078;

    // 8F9B: BRA $01BD
    cpu.state_.pc = 0x8F5A;
    ...
    // 8FB8: BEQ $01E3
    if (cpu.zero_flag()) cpu.state_.pc = 0x8F9D;
    ...
    // 8FC2: BCS $01D1
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F95;

    // 8FC4: BRA $0240
    cpu.state_.pc = 0x9006;

    // 8FC6: LDU <$E4
    cpu.state_.u = cpu.read_memory_word(0xE4);

    // 8FC8: BEQ $0240
    if (cpu.zero_flag()) cpu.state_.pc = 0x900A;

    // 8FCA: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8FCC: BEQ $0240
    if (cpu.zero_flag()) cpu.state_.pc = 0x900E;
