...
void routine_6864_impl(CPU6809& cpu) {
    ...
    // 688F: BNE $0042
    if (!cpu.zero_flag()) cpu.m_pc = 0x68D3;
    ...
    // 6896: BLT $003D
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x68D5;

    // 6898: ANDA #$01
    cpu.m_a &= 0x01;

    // 689A: BEQ $003D
    if (cpu.zero_flag()) cpu.m_pc = 0x68D9;
    ...
    // 68AA: BNE $004D
    if (!cpu.zero_flag()) cpu.m_pc = 0x68F9;
    ...
    // 68B5: BNE $0058
    if (!cpu.zero_flag()) cpu.m_pc = 0x690F;
    ...
    // 68C0: BCS $0062
    if (cpu.carry_flag()) cpu.m_pc = 0x6924;
    ...
    // 68CA: BCC $006B
    if (!cpu.carry_flag()) cpu.m_pc = 0x6937;
    ...
    // 6903: BNE $00AD
    if (!cpu.zero_flag()) cpu.m_pc = 0x68B2;
    ...
    // 690B: BCS $009D
    if (cpu.carry_flag()) cpu.m_pc = 0x68AA;
    ...
    // 691F: BNE $00C6
    if (!cpu.zero_flag()) cpu.m_pc = 0x68E7;
    ...
    // 6926: BNE $00C6
    if (!cpu.zero_flag()) cpu.m_pc = 0x68EE;
    ...
    // 694C: BLT $00EE
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x693C;
    ...
    // 6956: BNE $00FE
    if (!cpu.zero_flag()) cpu.m_pc = 0x6956;
    ...
    // 695D: BNE $00FE
    if (!cpu.zero_flag()) cpu.m_pc = 0x695D;
    ...
    // 698D: BNE $012F
    if (!cpu.zero_flag()) cpu.m_pc = 0x69BE;
    ...
    // 69AF: BCS $014F
    if (cpu.carry_flag()) cpu.m_pc = 0x6A00;
    ...
    // 6A2A: BNE $01D5
    if (!cpu.zero_flag()) cpu.m_pc = 0x6A01;
    ...
    // 6A31: BNE $01D2
    if (!cpu.zero_flag()) cpu.m_pc = 0x6A05;
    ...
    // 6A3D: BCS $01EB
    if (cpu.carry_flag()) cpu.m_pc = 0x6A2A;
    ...
    // 6A49: BCC $01EB
    if (!cpu.carry_flag()) cpu.m_pc = 0x6A36;
