...
void routine_f182_impl(CPU6809& cpu) {
    ...
    // F19A: BLE $001C
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xF1B8;
    ...
    // F1A0: BGE $0022
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xF1C4;
    ...
    // F1B6: BLE $0038
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xF1F0;
    ...
    // F1BC: BGE $003E
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xF1FC;
    ...
    // F1C8: BCC $004E
    if (!cpu.carry_flag()) cpu.m_pc = 0xF218;

    // F1CA: CMPA $1,X
    // TODO: Fix comma operator: CMPA $1,X

    // F1CC: BCS $004E
    if (cpu.carry_flag()) cpu.m_pc = 0xF21C;
    ...
    // F1D4: BCC $0056
    if (!cpu.carry_flag()) cpu.m_pc = 0xF22C;
    ...
    // F1E1: BNE $0063
    if (!cpu.zero_flag()) cpu.m_pc = 0xF246;
    ...
    // F1F3: BCS $0075
    if (cpu.carry_flag()) cpu.m_pc = 0xF26A;
    ...
    // F205: BGE $0088
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xF18F;

    // F208: DEC <$51
    // TODO: Convert DEC <$51

    // F20A: BLE $008D
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xF199;
    ...
    // F219: BCC $009B
    if (!cpu.carry_flag()) cpu.m_pc = 0xF1B6;
    ...
    // F220: BPL $00A4
    if (!cpu.negative_flag()) cpu.m_pc = 0xF1C6;
    ...
    // F233: BCC $00B5
    if (!cpu.carry_flag()) cpu.m_pc = 0xF1EA;
    ...
    // F28A: BNE $00FF
    if (!cpu.zero_flag()) cpu.m_pc = 0xF28B;
    ...
    // F2A7: BCS $0120
    if (cpu.carry_flag()) cpu.m_pc = 0xF2C9;
    ...
    // F2BA: BCS $0130
    if (cpu.carry_flag()) cpu.m_pc = 0xF2EC;
    ...
    // F2CA: BCS $0140
    if (cpu.carry_flag()) cpu.m_pc = 0xF30C;
    ...
    // F2D1: BNE $0154
    if (!cpu.zero_flag()) cpu.m_pc = 0xF327;
    ...
    // F2E5: BNE $0161
    if (!cpu.zero_flag()) cpu.m_pc = 0xF348;
    ...
    // F2F3: BNE $015A
    if (!cpu.zero_flag()) cpu.m_pc = 0xF34F;
    ...
    // F303: BCS $0179
    if (cpu.carry_flag()) cpu.m_pc = 0xF37E;
    ...
    // F37B: BEQ $01FE
    if (cpu.zero_flag()) cpu.m_pc = 0xF37B;
