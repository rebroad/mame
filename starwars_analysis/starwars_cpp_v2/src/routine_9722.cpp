...
void routine_9722_impl(CPU6809& cpu) {
    ...
    // 972A: BEQ $0017
    if (cpu.zero_flag()) cpu.m_pc = 0x9743;
    ...
    // 9769: BNE $0052
    if (!cpu.zero_flag()) cpu.m_pc = 0x97BD;
    ...
    // 9777: BEQ $0089
    if (cpu.zero_flag()) cpu.m_pc = 0x9702;
    ...
    // 97A3: BNE $0065
    if (!cpu.zero_flag()) cpu.m_pc = 0x980A;
    ...
    // 97B1: BCS $0096
    if (cpu.carry_flag()) cpu.m_pc = 0x9749;
    ...
    // 9892: BEQ $0175
    if (cpu.zero_flag()) cpu.m_pc = 0x9909;
    ...
    // 989C: BNE $018D
    if (!cpu.zero_flag()) cpu.m_pc = 0x982B;
    ...
    // 98A3: BPL $0185
    if (!cpu.negative_flag()) cpu.m_pc = 0x982A;
    ...
    // 98AD: BCS $017F
    if (cpu.carry_flag()) cpu.m_pc = 0x992E;
    ...
    // 98B4: BEQ $01A8
    if (cpu.zero_flag()) cpu.m_pc = 0x985E;
    ...
    // 98D8: BCC $01DC
    if (!cpu.carry_flag()) cpu.m_pc = 0x98B6;

    // 98DA: BCS $01DF
    if (cpu.carry_flag()) cpu.m_pc = 0x98BB;
