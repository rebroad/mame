...
void routine_9722_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_9722.md
    // Address: 0x9722
    ...
    // 972A: BEQ $0017
    if (cpu.zero_flag()) cpu.state_.pc = 0x9743;
    ...
    // 9761: BEQ $0052
    if (cpu.zero_flag()) cpu.state_.pc = 0x97B5;
    ...
    // 9769: BNE $0052
    if (!cpu.zero_flag()) cpu.state_.pc = 0x97BD;
    ...
    // 9777: BEQ $0089
    if (cpu.zero_flag()) cpu.state_.pc = 0x9702;
    ...
    // 97A3: BNE $0065
    if (!cpu.zero_flag()) cpu.state_.pc = 0x980A;
    ...
    // 97B1: BCS $0096
    if (cpu.carry_flag()) cpu.state_.pc = 0x9749;

    // 97B3: LDU #$9847
    cpu.state_.u = 0x9847;

    // 97B6: BRA $009F
    cpu.state_.pc = 0x9757;
    ...
    // 97E6: BRA $00EE
    cpu.state_.pc = 0x97D6;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $00EE
    cpu.state_.pc = 0x97DB;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $00EE
    cpu.state_.pc = 0x97E0;

    // 97F2: LDU #$9850
    cpu.state_.u = 0x9850;

    // 97F5: BRA $00EE
    cpu.state_.pc = 0x97E5;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $00EE
    cpu.state_.pc = 0x97EA;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $00EE
    cpu.state_.pc = 0x97EF;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $00EE
    cpu.state_.pc = 0x97F4;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $00EE
    cpu.state_.pc = 0x97F9;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $00EE
    cpu.state_.pc = 0x97FE;
    ...
    // 9876: BNE $015C
    if (!cpu.zero_flag()) cpu.state_.pc = 0x98D4;
    ...
    // 9892: BEQ $0175
    if (cpu.zero_flag()) cpu.state_.pc = 0x9909;
    ...
    // 989C: BNE $018D
    if (!cpu.zero_flag()) cpu.state_.pc = 0x982B;
    ...
    // 98A3: BPL $0185
    if (!cpu.negative_flag()) cpu.state_.pc = 0x982A;
    ...
    // 98AD: BCS $017F
    if (cpu.carry_flag()) cpu.state_.pc = 0x992E;
    ...
    // 98B4: BEQ $01A8
    if (cpu.zero_flag()) cpu.state_.pc = 0x985E;
    ...
    // 98CD: LBRA $21CE
    // TODO: Convert LBRA $21CE

    // 98D0: BRA $01D1
    cpu.state_.pc = 0x98A3;

    // 98D2: BRN $01D9
    // TODO: Convert BRN $01D9

    // 98D4: BVC $01D4
    // TODO: Convert BVC $01D4

    // 98D6: BRA $01DA
    cpu.state_.pc = 0x98B2;

    // 98D8: BCC $01DC
    if (!cpu.carry_flag()) cpu.state_.pc = 0x98B6;

    // 98DA: BCS $01DF
    if (cpu.carry_flag()) cpu.state_.pc = 0x98BB;
