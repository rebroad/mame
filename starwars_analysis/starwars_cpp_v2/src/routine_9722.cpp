...
void routine_9722_impl(StarWarsCPU& cpu) {
    ...
    // 9777: BEQ $0089
    if (cpu.zero_flag()) cpu.state_.pc = 0x9802;
    ...
    // 97B1: BCS $0096
    if (cpu.carry_flag()) cpu.state_.pc = 0x9849;

    // 97B3: LDU #$9847
    cpu.state_.u = 0x9847;

    // 97B6: BRA $009F
    cpu.state_.pc = 0x9857;
    ...
    // 97E6: BRA $00EE
    cpu.state_.pc = 0x98D6;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $00EE
    cpu.state_.pc = 0x98DB;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $00EE
    cpu.state_.pc = 0x98E0;

    // 97F2: LDU #$9850
    cpu.state_.u = 0x9850;

    // 97F5: BRA $00EE
    cpu.state_.pc = 0x98E5;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $00EE
    cpu.state_.pc = 0x98EA;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $00EE
    cpu.state_.pc = 0x98EF;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $00EE
    cpu.state_.pc = 0x98F4;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $00EE
    cpu.state_.pc = 0x98F9;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $00EE
    cpu.state_.pc = 0x98FE;
    ...
    // 9876: BNE $015C
    if (!cpu.zero_flag()) cpu.state_.pc = 0x99D4;
    ...
    // 9892: BEQ $0175
    if (cpu.zero_flag()) cpu.state_.pc = 0x9A09;
    ...
    // 989C: BNE $018D
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9A2B;
    ...
    // 98A3: BPL $0185
    if (!cpu.negative_flag()) cpu.state_.pc = 0x9A2A;
    ...
    // 98AD: BCS $017F
    if (cpu.carry_flag()) cpu.state_.pc = 0x9A2E;
    ...
    // 98B4: BEQ $01A8
    if (cpu.zero_flag()) cpu.state_.pc = 0x9A5E;
    ...
    // 98D6: BRA $01DA
    cpu.state_.pc = 0x9AB2;

    // 98D8: BCC $01DC
    if (!cpu.carry_flag()) cpu.state_.pc = 0x9AB6;

    // 98DA: BCS $01DF
    if (cpu.carry_flag()) cpu.state_.pc = 0x9ABB;
