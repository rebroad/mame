...
void routine_620f_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_620f.md
    // Address: 0x620F
    ...
    // 622A: BCS $0003
    if (cpu.carry_flag()) cpu.state_.pc = 0x622F;
    ...
    // 6231: BGT $002F
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x6262;
    ...
    // 623C: BRA $003C
    cpu.state_.pc = 0x627A;

    // 623E: CMPB #$60
    cpu.compare_b(0x60);

    // 6240: BLT $003C
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x627E;
    ...
    // 6246: BEQ $003C
    if (cpu.zero_flag()) cpu.state_.pc = 0x6284;
    ...
    // 6250: BNE $0048
    if (!cpu.zero_flag()) cpu.state_.pc = 0x629A;
    ...
    // 625A: BEQ $005A
    if (cpu.zero_flag()) cpu.state_.pc = 0x62B6;
    ...
    // 6260: BEQ $005A
    if (cpu.zero_flag()) cpu.state_.pc = 0x62BC;
    ...
    // 631E: BPL $0113
    if (!cpu.negative_flag()) cpu.state_.pc = 0x6333;
    ...
    // 6360: BPL $0155
    if (!cpu.negative_flag()) cpu.state_.pc = 0x63B7;
    ...
    // 6373: BNE $018E
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6303;

    // 6375: LDA $4ADB
    cpu.state_.a = cpu.read_memory(0x4ADB);

    // 6378: BMI $016E
    if (cpu.negative_flag()) cpu.state_.pc = 0x63E8;
    ...
    // 6382: BNE $017F
    if (!cpu.zero_flag()) cpu.state_.pc = 0x6403;
    ...
    // 6396: BCS $018B
    if (cpu.carry_flag()) cpu.state_.pc = 0x6323;
    ...
    // 63A3: BPL $0199
    if (!cpu.negative_flag()) cpu.state_.pc = 0x633E;
    ...
    // 63B2: BCC $01B7
    if (!cpu.carry_flag()) cpu.state_.pc = 0x636B;
    ...
    // 63BB: BCC $01B4
    if (!cpu.carry_flag()) cpu.state_.pc = 0x6371;
    ...
    // 63D8: BNE $01DC
    if (!cpu.zero_flag()) cpu.state_.pc = 0x63B6;
    ...
    // 63DE: BNE $01D5
    if (!cpu.zero_flag()) cpu.state_.pc = 0x63B5;

    // 63E0: LDB #$06
    cpu.state_.b = 0x06;

    // 63E2: BRA $01D7
    cpu.state_.pc = 0x63BB;
    ...
    // 63F3: BNE $0201
    if (!cpu.zero_flag()) cpu.state_.pc = 0x63F6;
    ...
    // 63FE: BEQ $01F5
    if (cpu.zero_flag()) cpu.state_.pc = 0x63F5;

    // 6400: BRA $0201
    cpu.state_.pc = 0x6403;
