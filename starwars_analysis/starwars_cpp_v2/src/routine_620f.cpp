...
void routine_620f_impl(StarWarsCPU& cpu) {
    ...
    // 623C: BRA $003C
    cpu.state_.pc += 0x003C;

    // 623E: CMPB #$60
    cpu.compare_b(0x60);

    // 6240: BLT $003C
    // TODO: Convert BLT $003C
    ...
    // 6360: BPL $0155
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0155;
    ...
    // 6378: BMI $016E
    if (cpu.negative_flag()) cpu.state_.pc += 0x016E;
    ...
    // 6396: BCS $018B
    if (cpu.carry_flag()) cpu.state_.pc += 0x018B;
    ...
    // 63A3: BPL $0199
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0199;
    ...
    // 63B2: BCC $01B7
    if (!cpu.carry_flag()) cpu.state_.pc += 0x01B7;
    ...
    // 63D8: BNE $01DC
    if (!cpu.zero_flag()) cpu.state_.pc += 0x01DC;
