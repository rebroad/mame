...
void routine_8be1_impl(StarWarsCPU& cpu) {
    ...
    // 8BFF: BMI $0034
    if (cpu.negative_flag()) cpu.state_.pc += 0x0034;
    ...
    // 8C1C: BCC $0040
    if (!cpu.carry_flag()) cpu.state_.pc += 0x0040;
    ...
    // 8C2A: BCC $004E
    if (!cpu.carry_flag()) cpu.state_.pc += 0x004E;
    ...
    // 8C3F: BCC $0063
    if (!cpu.carry_flag()) cpu.state_.pc += 0x0063;
    ...
    // 8C89: BMI $00CD
    if (cpu.negative_flag()) cpu.state_.pc += 0x00CD;
    ...
    // 8CB3: BEQ $0124
    if (cpu.zero_flag()) cpu.state_.pc += 0x0124;
    ...
    // 8CEC: BNE $011C
    if (!cpu.zero_flag()) cpu.state_.pc += 0x011C;
    ...
    // 8D10: BNE $0159
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0159;

    // 8D12: LDA $5002
    cpu.state_.a = cpu.read_memory(0x5002);

    // 8D15: BMI $013B
    if (cpu.negative_flag()) cpu.state_.pc += 0x013B;

    // 8D17: JSR $8979
    cpu.call_function(0x8979);

    // 8D1A: BRA $013E
    cpu.state_.pc += 0x013E;
    ...
    // 8D30: BMI $0156
    if (cpu.negative_flag()) cpu.state_.pc += 0x0156;

    // 8D32: JSR $8961
    cpu.call_function(0x8961);

    // 8D35: BRA $0159
    cpu.state_.pc += 0x0159;
    ...
    // 8D41: BMI $0167
    if (cpu.negative_flag()) cpu.state_.pc += 0x0167;

    // 8D43: JSR $8951
    cpu.call_function(0x8951);

    // 8D46: BRA $016A
    cpu.state_.pc += 0x016A;
    ...
    // 8D5C: BMI $0182
    if (cpu.negative_flag()) cpu.state_.pc += 0x0182;

    // 8D5E: JSR $8969
    cpu.call_function(0x8969);

    // 8D61: BRA $0185
    cpu.state_.pc += 0x0185;
    ...
    // 8DA3: BNE $01FE
    if (!cpu.zero_flag()) cpu.state_.pc += 0x01FE;
    ...
    // 8DCC: BCC $01F0
    if (!cpu.carry_flag()) cpu.state_.pc += 0x01F0;
    ...
    // 8DDA: BCC $01FE
    if (!cpu.carry_flag()) cpu.state_.pc += 0x01FE;
