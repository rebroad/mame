...
void routine_67aa_impl(CPU6809& cpu) {
    ...
    // 684F: BNE $00AD
    if (!cpu.zero_flag()) cpu.m_pc = 0x67FE;
    ...
    // 688F: BNE $00FC
    if (!cpu.zero_flag()) cpu.m_pc = 0x688D;
    ...
    // 6896: BLT $00F7
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x688F;

    // 6898: ANDA #$01
    cpu.m_a &= 0x01;

    // 689A: BEQ $00F7
    if (cpu.zero_flag()) cpu.m_pc = 0x6893;
    ...
    // 68AA: BNE $0107
    if (!cpu.zero_flag()) cpu.m_pc = 0x68B3;
    ...
    // 68C0: BCS $011C
    if (cpu.carry_flag()) cpu.m_pc = 0x68DE;
    ...
    // 68CA: BCC $0125
    if (!cpu.carry_flag()) cpu.m_pc = 0x68F1;
    ...
    // 6903: BNE $0167
    if (!cpu.zero_flag()) cpu.m_pc = 0x696C;
    ...
    // 690B: BCS $0157
    if (cpu.carry_flag()) cpu.m_pc = 0x6964;
    ...
    // 6926: BNE $0180
    if (!cpu.zero_flag()) cpu.m_pc = 0x68A8;
    ...
    // 694C: BLT $01A8
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x68F6;
    ...
    // 6956: BNE $01B8
    if (!cpu.zero_flag()) cpu.m_pc = 0x6910;
    ...
    // 698D: BNE $01E9
    if (!cpu.zero_flag()) cpu.m_pc = 0x6978;
