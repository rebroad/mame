...
void routine_8b6d_impl(StarWarsCPU& cpu) {
    ...
    // 8BBC: BRA $0069
    cpu.state_.pc += 0x0069;

    // 8BBE: LDA $A,U
    // TODO: Fix comma operator: LDA $A,U

    // 8BC1: BPL $0057
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0057;
    ...
    // 8BCB: BPL $0061
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0061;
    ...
    // 8BDE: BCS $001C
    if (cpu.carry_flag()) cpu.state_.pc += 0x001C;
    ...
    // 8C1C: BCC $00B4
    if (!cpu.carry_flag()) cpu.state_.pc += 0x00B4;
    ...
    // 8C23: BCC $00BB
    if (!cpu.carry_flag()) cpu.state_.pc += 0x00BB;
    ...
    // 8C2A: BCC $00C2
    if (!cpu.carry_flag()) cpu.state_.pc += 0x00C2;
    ...
    // 8C31: BCC $00C9
    if (!cpu.carry_flag()) cpu.state_.pc += 0x00C9;
    ...
    // 8C38: BCC $00D0
    if (!cpu.carry_flag()) cpu.state_.pc += 0x00D0;
    ...
    // 8C89: BMI $0141
    if (cpu.negative_flag()) cpu.state_.pc += 0x0141;
    ...
    // 8CB3: BEQ $0198
    if (cpu.zero_flag()) cpu.state_.pc += 0x0198;
    ...
    // 8CCD: BCS $0167
    if (cpu.carry_flag()) cpu.state_.pc += 0x0167;

    // 8CCF: LDU #$8D99
    cpu.state_.u = 0x8D99;

    // 8CD2: BRA $016E
    cpu.state_.pc += 0x016E;
    ...
    // 8CEC: BNE $0190
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0190;
    ...
    // 8D0C: BEQ $01F9
    if (cpu.zero_flag()) cpu.state_.pc += 0x01F9;
    ...
    // 8D41: BMI $01DB
    if (cpu.negative_flag()) cpu.state_.pc += 0x01DB;
    ...
    // 8D5C: BMI $01F6
    if (cpu.negative_flag()) cpu.state_.pc += 0x01F6;
