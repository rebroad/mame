...
void routine_8b6d_impl(CPU6809& cpu) {
    ...
    // 8B76: BEQ $000E
    if (cpu.zero_flag()) cpu.m_pc = 0x8B86;
    ...
    // 8B83: BCS $0003
    if (cpu.carry_flag()) cpu.m_pc = 0x8B88;
    ...
    // 8B91: BNE $0069
    if (!cpu.zero_flag()) cpu.m_pc = 0x8BFC;
    ...
    // 8B97: BLT $002E
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8BC7;
    ...
    // 8B9D: BGT $0034
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8BD3;
    ...
    // 8BA7: BLT $003E
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8BE7;
    ...
    // 8BAD: BGT $0044
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8BF3;
    ...
    // 8BC1: BPL $0057
    if (!cpu.negative_flag()) cpu.m_pc = 0x8C1A;

    // 8BC4: CMPA #$08
    cpu.compare_a(0x08);

    // 8BC6: BGT $0069
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8C31;

    // 8BC8: LDA $C,U
    // TODO: Fix comma operator: LDA $C,U

    // 8BCB: BPL $0061
    if (!cpu.negative_flag()) cpu.m_pc = 0x8C2E;

    // 8BCE: CMPA #$08
    cpu.compare_a(0x08);

    // 8BD0: BGT $0069
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8C3B;
    ...
    // 8BDE: BCS $001C
    if (cpu.carry_flag()) cpu.m_pc = 0x8BFC;
    ...
    // 8BFF: BMI $00A8
    if (cpu.negative_flag()) cpu.m_pc = 0x8BA9;
    ...
    // 8C1C: BCC $00B4
    if (!cpu.carry_flag()) cpu.m_pc = 0x8BD2;
    ...
    // 8C23: BCC $00BB
    if (!cpu.carry_flag()) cpu.m_pc = 0x8BE0;
    ...
    // 8C2A: BCC $00C2
    if (!cpu.carry_flag()) cpu.m_pc = 0x8BEE;
    ...
    // 8C38: BCC $00D0
    if (!cpu.carry_flag()) cpu.m_pc = 0x8C0A;
    ...
    // 8C3F: BCC $00D7
    if (!cpu.carry_flag()) cpu.m_pc = 0x8C18;
    ...
    // 8C66: BEQ $0114
    if (cpu.zero_flag()) cpu.m_pc = 0x8C7C;
    ...
    // 8C89: BMI $0141
    if (cpu.negative_flag()) cpu.m_pc = 0x8CCC;
    ...
    // 8C90: BGE $0141
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8CD3;
    ...
    // 8CB3: BEQ $0198
    if (cpu.zero_flag()) cpu.m_pc = 0x8C4D;
    ...
    // 8CBA: BNE $0198
    if (!cpu.zero_flag()) cpu.m_pc = 0x8C54;
    ...
    // 8CC2: BLE $0198
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8C5C;

    // 8CC4: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 8CC6: BNE $0198
    if (!cpu.zero_flag()) cpu.m_pc = 0x8C60;
    ...
    // 8CCD: BCS $0167
    if (cpu.carry_flag()) cpu.m_pc = 0x8D36;
    ...
    // 8CDF: BNE $0198
    if (!cpu.zero_flag()) cpu.m_pc = 0x8C79;
    ...
    // 8CEC: BNE $0190
    if (!cpu.zero_flag()) cpu.m_pc = 0x8C7E;
    ...
    // 8D03: BCS $017D
    if (cpu.carry_flag()) cpu.m_pc = 0x8D82;
    ...
    // 8D0C: BEQ $01F9
    if (cpu.zero_flag()) cpu.m_pc = 0x8D07;

    // 8D0E: LDA <$4F
    cpu.m_a = cpu.read_memory(0x4F);

    // 8D10: BNE $01CD
    if (!cpu.zero_flag()) cpu.m_pc = 0x8CDF;

    // 8D12: LDA $5002
    cpu.m_a = cpu.read_memory(0x5002);

    // 8D15: BMI $01AF
    if (cpu.negative_flag()) cpu.m_pc = 0x8CC6;
    ...
    // 8D21: BNE $01CD
    if (!cpu.zero_flag()) cpu.m_pc = 0x8CF0;
    ...
    // 8D30: BMI $01CA
    if (cpu.negative_flag()) cpu.m_pc = 0x8CFC;
    ...
    // 8D3C: BNE $01F9
    if (!cpu.zero_flag()) cpu.m_pc = 0x8D37;

    // 8D3E: LDA $5004
    cpu.m_a = cpu.read_memory(0x5004);

    // 8D41: BMI $01DB
    if (cpu.negative_flag()) cpu.m_pc = 0x8D1E;
    ...
    // 8D4D: BNE $01F9
    if (!cpu.zero_flag()) cpu.m_pc = 0x8D48;
    ...
    // 8D5C: BMI $01F6
    if (cpu.negative_flag()) cpu.m_pc = 0x8D54;
