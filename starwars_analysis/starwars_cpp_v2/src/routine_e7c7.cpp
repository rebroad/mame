...
void routine_e7c7_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_e7c7.md
    // Address: 0xE7C7
    ...
    // E7C9: BCC $000B
    if (!cpu.carry_flag()) cpu.state_.pc = 0xE7D6;

    // E7CB: STB $48AE
    cpu.write_memory(0x48AE, cpu.state_.b);

    // E7CE: BSR $0016
    // TODO: Convert BSR $0016

    // E7D0: BSR $0023
    // TODO: Convert BSR $0023

    // E7D3: CMPB #$D6
    cpu.compare_b(0xD6);

    // E7D5: BCC $0015
    if (!cpu.carry_flag()) cpu.state_.pc = 0xE7EC;
    ...
    // E833: BCS $0075
    if (cpu.carry_flag()) cpu.state_.pc = 0xE8AA;
    ...
    // E83A: BRA $00C1
    cpu.state_.pc = 0xE8FD;

    // E83C: CMPB #$74
    cpu.compare_b(0x74);

    // E83E: BNE $007E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8BE;

    // E840: LDX #$2FDE
    cpu.state_.x = 0x2FDE;

    // E843: BRA $00C1
    cpu.state_.pc = 0xE906;

    // E845: CMPB #$80
    cpu.compare_b(0x80);

    // E847: BNE $0088
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8D1;

    // E849: LDX #$3058
    cpu.state_.x = 0x3058;

    // E84D: BRA $00C1
    cpu.state_.pc = 0xE910;

    // E84F: CMPB #$60
    cpu.compare_b(0x60);

    // E851: BCS $0091
    if (cpu.carry_flag()) cpu.state_.pc = 0xE8E4;

    // E853: LDX #$2FA4
    cpu.state_.x = 0x2FA4;

    // E856: BRA $00C1
    cpu.state_.pc = 0xE919;

    // E858: CMPB #$40
    cpu.compare_b(0x40);

    // E85A: BNE $009A
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8F6;

    // E85C: LDX #$2FC2
    cpu.state_.x = 0x2FC2;

    // E85F: BRA $00C1
    cpu.state_.pc = 0xE922;
    ...
    // E868: BRA $00C1
    cpu.state_.pc = 0xE92B;

    // E86A: CMPB #$58
    cpu.compare_b(0x58);

    // E86C: BNE $00AC
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE91A;
    ...
    // E875: BNE $00B5
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE92C;
    ...
    // E87E: BNE $00BE
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE93E;

    // E880: LDX #$300C
    cpu.state_.x = 0x300C;

    // E883: BRA $00C1
    cpu.state_.pc = 0xE946;
    ...
    // E891: BPL $0064
    if (!cpu.negative_flag()) cpu.state_.pc = 0xE8F7;
    ...
    // E945: BRA $0162
    cpu.state_.pc = 0xEAA9;

    // E947: BNE $0164
    if (!cpu.zero_flag()) cpu.state_.pc = 0xEAAD;
    ...
    // E970: ADDD $018F,PCR
    // TODO: Fix comma operator: ADDD $018F,PCR
    ...
    // E97A: ADDD [$0199,PCR]
    // TODO: Complex indexed addressing: [$0199,PCR]
