...
void routine_d882_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_d882.md
    // Address: 0xD882
    ...
    // D88F: BEQ $002A
    if (cpu.zero_flag()) cpu.state_.pc = 0xD8BB;
    ...
    // D897: BPL $0022
    if (!cpu.negative_flag()) cpu.state_.pc = 0xD8BB;

    // D899: TST <$13
    // TODO: Convert TST <$13

    // D89B: BPL $002C
    if (!cpu.negative_flag()) cpu.state_.pc = 0xD8C9;

    // D89E: BMI $002C
    if (cpu.negative_flag()) cpu.state_.pc = 0xD8CC;
    ...
    // D8A6: BNE $0028
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD8D0;
    ...
    // D8B9: BPL $003E
    if (!cpu.negative_flag()) cpu.state_.pc = 0xD8F9;
    ...
    // D8C5: BGE $0035
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xD8FC;

    // D8C8: BNE $005C
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD926;
    ...
    // D8DC: BGE $004B
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xD929;
    ...
    // D8E5: BCC $006F
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD956;

    // D8E7: CMPA ,X+
    // TODO: Complex indexed addressing: ,X+

    // D8E9: BNE $006A
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD955;
    ...
    // D8FC: BNE $0090
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD88E;
    ...
    // D917: BCS $0078
    if (cpu.carry_flag()) cpu.state_.pc = 0xD991;
    ...
    // D92A: BCC $00BF
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD8EB;
    ...
    // D949: BCC $00DB
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD926;
    ...
    // D976: BNE $00FA
    if (!cpu.zero_flag()) cpu.state_.pc = 0xD972;
    ...
    // D982: BCS $00F2
    if (cpu.carry_flag()) cpu.state_.pc = 0xD976;
    ...
    // D98C: BCC $0159
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD9E7;
    ...
    // D995: BEQ $0151
    if (cpu.zero_flag()) cpu.state_.pc = 0xD9E8;
    ...
    // D9D9: BCS $0111
    if (cpu.carry_flag()) cpu.state_.pc = 0xD9EC;
    ...
    // DA07: LBCC $0212
    // TODO: Convert LBCC $0212

    // DA0B: CMPD #$0040
    // TODO: Convert CMPD #$0040

    // DA0F: BCC $019C
    if (!cpu.carry_flag()) cpu.state_.pc = 0xD9AD;
