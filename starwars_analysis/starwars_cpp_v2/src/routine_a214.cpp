...
void routine_a214_impl(CPU6809& cpu) {
    ...
    // A248: BCS $0039
    if (cpu.carry_flag()) cpu.m_pc = 0xA283;
    ...
    // A277: BPL $006A
    if (!cpu.negative_flag()) cpu.m_pc = 0xA2E3;
    ...
    // A285: BLT $009C
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA223;

    // A287: CMPD #$3C00
    // TODO: Convert CMPD #$3C00

    // A28B: BCC $009C
    if (!cpu.carry_flag()) cpu.m_pc = 0xA229;

    // A28D: LDD $5002
    cpu.m_d = cpu.read_memory16(0x5002);

    // A290: BPL $0082
    if (!cpu.negative_flag()) cpu.m_pc = 0xA214;
    ...
    // A299: BCC $009C
    if (!cpu.carry_flag()) cpu.m_pc = 0xA237;
    ...
    // A2A6: BEQ $009A
    if (cpu.zero_flag()) cpu.m_pc = 0xA242;
    ...
    // A2BC: BEQ $00AE
    if (cpu.zero_flag()) cpu.m_pc = 0xA26C;
    ...
    // A2C8: BEQ $00C2
    if (cpu.zero_flag()) cpu.m_pc = 0xA28C;
    ...
    // A2D0: BNE $00C0
    if (!cpu.zero_flag()) cpu.m_pc = 0xA292;
    ...
    // A2E6: BCS $00D7
    if (cpu.carry_flag()) cpu.m_pc = 0xA2BF;
    ...
    // A2FE: BNE $00F0
    if (!cpu.zero_flag()) cpu.m_pc = 0xA2F0;
    ...
    // A31F: BNE $0111
    if (!cpu.zero_flag()) cpu.m_pc = 0xA332;

    // A321: BITA #$10
    // TODO: Convert BITA #$10

    // A323: BNE $011A
    if (!cpu.zero_flag()) cpu.m_pc = 0xA33F;
    ...
    // A335: BNE $0128
    if (!cpu.zero_flag()) cpu.m_pc = 0xA35F;
    ...
    // A36B: BEQ $0193
    if (cpu.zero_flag()) cpu.m_pc = 0xA300;

    // A36D: LDA <$60
    cpu.m_a = cpu.read_memory(0x60);

    // A36F: BLT $0160
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA3D1;
    ...
    // A37D: BGT $0191
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA310;
    ...
    // A38D: BLT $0181
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA310;
    ...
    // A397: BNE $0189
    if (!cpu.zero_flag()) cpu.m_pc = 0xA322;
    ...
    // A3A0: BPL $018F
    if (!cpu.negative_flag()) cpu.m_pc = 0xA331;
    ...
    // A3AD: BGT $01E0
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA38F;
    ...
    // A3BF: BNE $01E0
    if (!cpu.zero_flag()) cpu.m_pc = 0xA3A1;
    ...
    // A3C7: BGE $01E0
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA3A9;
    ...
    // A3D2: BGT $01E0
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA3B4;
    ...
    // A3DC: BLT $01D0
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA3AE;
    ...
    // A3E6: BNE $01E0
    if (!cpu.zero_flag()) cpu.m_pc = 0xA3C8;
    ...
    // A3ED: BMI $01DC
    if (cpu.negative_flag()) cpu.m_pc = 0xA3CB;
    ...
    // A3F9: BGT $01EC
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA3E7;
