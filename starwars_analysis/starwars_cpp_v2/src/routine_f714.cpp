...
void routine_f714_impl(CPU6809& cpu) {
    ...
    // F751: BPL $0068
    if (!cpu.negative_flag()) cpu.m_pc = 0xF7BB;
    ...
    // F75D: BNE $0053
    if (!cpu.zero_flag()) cpu.m_pc = 0xF7B2;

    // F75F: ANDB $1,X
    // TODO: Fix comma operator: ANDB $1,X

    // F761: BNE $0053
    if (!cpu.zero_flag()) cpu.m_pc = 0xF7B6;
    ...
    // F771: BCS $0066
    if (cpu.carry_flag()) cpu.m_pc = 0xF7D9;
    ...
    // F7AF: BCS $00A2
    if (cpu.carry_flag()) cpu.m_pc = 0xF753;
    ...
    // F7BA: BNE $0097
    if (!cpu.zero_flag()) cpu.m_pc = 0xF753;
    ...
    // F7D8: BEQ $00CB
    if (cpu.zero_flag()) cpu.m_pc = 0xF7A5;
    ...
    // F853: BNE $013D
    if (!cpu.zero_flag()) cpu.m_pc = 0xF892;
    ...
    // F86F: BCC $0162
    if (!cpu.carry_flag()) cpu.m_pc = 0xF8D3;
    ...
    // F87C: BPL $015A
    if (!cpu.negative_flag()) cpu.m_pc = 0xF8D8;
    ...
    // F8A5: BCS $0187
    if (cpu.carry_flag()) cpu.m_pc = 0xF82E;
    ...
    // F8EF: BNE $01EC
    if (!cpu.zero_flag()) cpu.m_pc = 0xF8DD;

    // F8F1: CMPB #$FC
    cpu.compare_b(0xFC);

    // F8F3: BCS $01E8
    if (cpu.carry_flag()) cpu.m_pc = 0xF8DD;
    ...
    // F902: BCS $01F2
    if (cpu.carry_flag()) cpu.m_pc = 0xF8F6;

    // F906: CMPA #$08
    cpu.compare_a(0x08);

    // F908: BCS $01FB
    if (cpu.carry_flag()) cpu.m_pc = 0xF905;
    ...
    // F913: BCS $0201
    if (cpu.carry_flag()) cpu.m_pc = 0xF916;
