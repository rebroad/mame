...
void routine_f714_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_f714.md
    // Address: 0xF714
    ...
    // F751: BPL $0068
    if (!cpu.negative_flag()) cpu.state_.pc = 0xF7BB;
    ...
    // F75D: BNE $0053
    if (!cpu.zero_flag()) cpu.state_.pc = 0xF7B2;

    // F75F: ANDB $1,X
    // TODO: Fix comma operator: ANDB $1,X

    // F761: BNE $0053
    if (!cpu.zero_flag()) cpu.state_.pc = 0xF7B6;

    // F763: LDA #$3B
    cpu.state_.a = 0x3B;

    // F765: BRA $0055
    cpu.state_.pc = 0xF7BC;
    ...
    // F771: BCS $0066
    if (cpu.carry_flag()) cpu.state_.pc = 0xF7D9;
    ...
    // F7AF: BCS $00A2
    if (cpu.carry_flag()) cpu.state_.pc = 0xF753;
    ...
    // F7BA: BNE $0097
    if (!cpu.zero_flag()) cpu.state_.pc = 0xF753;
    ...
    // F7D8: BEQ $00CB
    if (cpu.zero_flag()) cpu.state_.pc = 0xF7A5;
    ...
    // F853: BNE $013D
    if (!cpu.zero_flag()) cpu.state_.pc = 0xF892;
    ...
    // F86F: BCC $0162
    if (!cpu.carry_flag()) cpu.state_.pc = 0xF8D3;

    // F871: LDX $3022
    cpu.state_.x = cpu.read_memory_word(0x3022);

    // F874: BRA $0165
    cpu.state_.pc = 0xF8DB;
    ...
    // F87C: BPL $015A
    if (!cpu.negative_flag()) cpu.state_.pc = 0xF8D8;
    ...
    // F8A5: BCS $0187
    if (cpu.carry_flag()) cpu.state_.pc = 0xF82E;
    ...
    // F8EF: BNE $01EC
    if (!cpu.zero_flag()) cpu.state_.pc = 0xF8DD;

    // F8F1: CMPB #$FC
    cpu.compare_b(0xFC);

    // F8F3: BCS $01E8
    if (cpu.carry_flag()) cpu.state_.pc = 0xF8DD;
    ...
    // F8FE: BRA $0203
    cpu.state_.pc = 0xF903;

    // F900: CMPB #$B0
    cpu.compare_b(0xB0);

    // F902: BCS $01F2
    if (cpu.carry_flag()) cpu.state_.pc = 0xF8F6;

    // F906: CMPA #$08
    cpu.compare_a(0x08);

    // F908: BCS $01FB
    if (cpu.carry_flag()) cpu.state_.pc = 0xF905;
    ...
    // F913: BCS $0201
    if (cpu.carry_flag()) cpu.state_.pc = 0xF916;
