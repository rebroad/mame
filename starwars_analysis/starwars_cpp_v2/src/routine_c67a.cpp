...
void routine_c67a_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_c67a.md
    // Address: 0xC67A
    ...
    // C685: BCS $0006
    if (cpu.carry_flag()) cpu.state_.pc = 0xC68D;
    ...
    // C68D: BNE $0011
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC6A0;

    // C690: LDA $4AF7
    cpu.state_.a = cpu.read_memory(0x4AF7);

    // C693: BEQ $003D
    if (cpu.zero_flag()) cpu.state_.pc = 0xC6D2;
    ...
    // C6D1: BCS $004E
    if (cpu.carry_flag()) cpu.state_.pc = 0xC721;
    ...
    // C6EF: BPL $0062
    if (!cpu.negative_flag()) cpu.state_.pc = 0xC753;
    ...
    // C709: BPL $0082
    if (!cpu.negative_flag()) cpu.state_.pc = 0xC68D;
    ...
    // C71A: BCS $00A4
    if (cpu.carry_flag()) cpu.state_.pc = 0xC6C0;
    ...
    // C72C: BEQ $00BA
    if (cpu.zero_flag()) cpu.state_.pc = 0xC6E8;
    ...
    // C7AA: BCS $015C
    if (cpu.carry_flag()) cpu.state_.pc = 0xC808;
    ...
    // C840: BNE $01CD
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC80F;
    ...
    // C854: BMI $01F7
    if (cpu.negative_flag()) cpu.state_.pc = 0xC84D;
    ...
    // C85B: BNE $01F3
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC850;
    ...
    // C862: BNE $01EF
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC853;

    // C864: LDD $4B10
    cpu.state_.d = cpu.read_memory_word(0x4B10);

    // C867: BRA $01F1
    cpu.state_.pc = 0xC85A;

    // C869: LDD <$01
    cpu.state_.d = cpu.read_memory_word(0x01);

    // C86B: BRA $01F5
    cpu.state_.pc = 0xC862;
    ...
    // C873: BNE $020A
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC87F;
