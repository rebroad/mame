...
void routine_c67a_impl(CPU6809& cpu) {
    ...
    // C685: BCS $0006
    if (cpu.carry_flag()) cpu.m_pc = 0xC68D;
    ...
    // C68D: BNE $0011
    if (!cpu.zero_flag()) cpu.m_pc = 0xC6A0;

    // C690: LDA $4AF7
    cpu.m_a = cpu.read_memory(0x4AF7);

    // C693: BEQ $003D
    if (cpu.zero_flag()) cpu.m_pc = 0xC6D2;

    // C695: CMPA #$01
    cpu.compare_a(0x01);

    // C697: BNE $0023
    if (!cpu.zero_flag()) cpu.m_pc = 0xC6BC;
    ...
    // C6D1: BCS $004E
    if (cpu.carry_flag()) cpu.m_pc = 0xC721;
    ...
    // C6EF: BPL $0062
    if (!cpu.negative_flag()) cpu.m_pc = 0xC753;
    ...
    // C709: BPL $0082
    if (!cpu.negative_flag()) cpu.m_pc = 0xC68D;
    ...
    // C71A: BCS $00A4
    if (cpu.carry_flag()) cpu.m_pc = 0xC6C0;
    ...
    // C72C: BEQ $00BA
    if (cpu.zero_flag()) cpu.m_pc = 0xC6E8;
    ...
    // C7AA: BCS $015C
    if (cpu.carry_flag()) cpu.m_pc = 0xC808;
    ...
    // C840: BNE $01CD
    if (!cpu.zero_flag()) cpu.m_pc = 0xC80F;
    ...
    // C854: BMI $01F7
    if (cpu.negative_flag()) cpu.m_pc = 0xC84D;
    ...
    // C85B: BNE $01F3
    if (!cpu.zero_flag()) cpu.m_pc = 0xC850;
    ...
    // C862: BNE $01EF
    if (!cpu.zero_flag()) cpu.m_pc = 0xC853;
    ...
    // C873: BNE $020A
    if (!cpu.zero_flag()) cpu.m_pc = 0xC87F;
