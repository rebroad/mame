...
void routine_960f_impl(StarWarsCPU& cpu) {
    ...
    // 9672: BRA $0091
    cpu.state_.pc = 0x9605;

    // 9674: LDA <$8F
    cpu.state_.a = cpu.read_memory(0x8F);

    // 9676: BNE $0081
    if (!cpu.zero_flag()) cpu.state_.pc = 0x95F9;
    ...
    // 9694: BNE $0091
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9627;
    ...
    // 972A: BEQ $012A
    if (cpu.zero_flag()) cpu.state_.pc = 0x9756;
    ...
    // 9761: BEQ $0165
    if (cpu.zero_flag()) cpu.state_.pc = 0x97C8;
    ...
    // 9769: BNE $0165
    if (!cpu.zero_flag()) cpu.state_.pc = 0x97D0;
    ...
    // 9777: BEQ $019C
    if (cpu.zero_flag()) cpu.state_.pc = 0x9715;
    ...
    // 97A3: BNE $0178
    if (!cpu.zero_flag()) cpu.state_.pc = 0x981D;
    ...
    // 97B1: BCS $01A9
    if (cpu.carry_flag()) cpu.state_.pc = 0x975C;

    // 97B3: LDU #$9847
    cpu.state_.u = 0x9847;

    // 97B6: BRA $01B2
    cpu.state_.pc = 0x976A;
    ...
    // 97E6: BRA $0201
    cpu.state_.pc = 0x97E9;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $0201
    cpu.state_.pc = 0x97EE;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $0201
    cpu.state_.pc = 0x97F3;

    // 97F2: LDU #$9850
    cpu.state_.u = 0x9850;

    // 97F5: BRA $0201
    cpu.state_.pc = 0x97F8;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $0201
    cpu.state_.pc = 0x97FD;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $0201
    cpu.state_.pc = 0x9802;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $0201
    cpu.state_.pc = 0x9807;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $0201
    cpu.state_.pc = 0x980C;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $0201
    cpu.state_.pc = 0x9811;
