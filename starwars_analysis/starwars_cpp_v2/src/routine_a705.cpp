...
void routine_a705_impl(CPU6809& cpu) {
    ...
    // A70C: BEQ $0012
    if (cpu.zero_flag()) cpu.m_pc = 0xA720;
    ...
    // A713: BCS $0003
    if (cpu.carry_flag()) cpu.m_pc = 0xA718;
    ...
    // A73C: BMI $003D
    if (cpu.negative_flag()) cpu.m_pc = 0xA77B;
    ...
    // A765: BPL $0066
    if (!cpu.negative_flag()) cpu.m_pc = 0xA7CD;
    ...
    // A781: BCC $007F
    if (!cpu.carry_flag()) cpu.m_pc = 0xA802;
    ...
    // A78A: BPL $008B
    if (!cpu.negative_flag()) cpu.m_pc = 0xA717;
    ...
    // A792: BPL $0090
    if (!cpu.negative_flag()) cpu.m_pc = 0xA724;
    ...
    // A7A8: BNE $00A9
    if (!cpu.zero_flag()) cpu.m_pc = 0xA753;
    ...
    // A7F9: BEQ $00FB
    if (cpu.zero_flag()) cpu.m_pc = 0xA7F6;
    ...
    // A80D: BEQ $010F
    if (cpu.zero_flag()) cpu.m_pc = 0xA81E;
    ...
    // A83F: BLT $013E
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA87F;
    ...
    // A850: BEQ $015C
    if (cpu.zero_flag()) cpu.m_pc = 0xA8AE;
    ...
    // A857: BCC $015B
    if (!cpu.carry_flag()) cpu.m_pc = 0xA8B4;
    ...
    // A868: BCS $0147
    if (cpu.carry_flag()) cpu.m_pc = 0xA8B1;

    // A86B: DEC $5,X
    // TODO: Convert DEC $5,X

    // A86D: BGT $016F
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA8DE;
    ...
    // A877: BGT $017B
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA8F4;
