...
void routine_a705_impl(StarWarsCPU& cpu) {
    ...
    // A78A: BPL $008B
    if (!cpu.negative_flag()) cpu.state_.pc = 0xA717;

    // A78C: CLR $6,X
    // TODO: Fix comma operator: CLR $6,X

    // A78E: BRA $00AD
    cpu.state_.pc = 0xA73D;

    // A790: LDA <$7D
    cpu.state_.a = cpu.read_memory(0x7D);

    // A792: BPL $0090
    if (!cpu.negative_flag()) cpu.state_.pc = 0xA724;
    ...
    // A7A8: BNE $00A9
    if (!cpu.zero_flag()) cpu.state_.pc = 0xA753;

    // A7AA: LDA #$07
    cpu.state_.a = 0x07;

    // A7AC: BRA $00AB
    cpu.state_.pc = 0xA759;
    ...
    // A7F9: BEQ $00FB
    if (cpu.zero_flag()) cpu.state_.pc = 0xA7F6;

    // A7FB: JSR $A705
    cpu.call_function(0xA705);

    // A7FE: BRA $00FE
    cpu.state_.pc = 0xA7FE;
    ...
    // A809: BRA $011A
    cpu.state_.pc = 0xA825;

    // A80B: LDA <$95
    cpu.state_.a = cpu.read_memory(0x95);

    // A80D: BEQ $010F
    if (cpu.zero_flag()) cpu.state_.pc = 0xA81E;

    // A80F: JSR $A705
    cpu.call_function(0xA705);

    // A812: BRA $0112
    cpu.state_.pc = 0xA826;
    ...
    // A83F: BLT $013E
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xA87F;
    ...
    // A850: BEQ $015C
    if (cpu.zero_flag()) cpu.state_.pc = 0xA8AE;
    ...
    // A857: BCC $015B
    if (!cpu.carry_flag()) cpu.state_.pc = 0xA8B4;
    ...
    // A868: BCS $0147
    if (cpu.carry_flag()) cpu.state_.pc = 0xA8B1;

    // A86B: DEC $5,X
    // TODO: Convert DEC $5,X

    // A86D: BGT $016F
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xA8DE;
    ...
    // A877: BGT $017B
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xA8F4;
