...
void routine_973a_impl(StarWarsCPU& cpu) {
    ...
    // 97E6: BRA $00D6
    cpu.state_.pc = 0x97BE;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $00D6
    cpu.state_.pc = 0x97C3;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $00D6
    cpu.state_.pc = 0x97C8;
    ...
    // 97FA: BRA $00D6
    cpu.state_.pc = 0x97D2;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $00D6
    cpu.state_.pc = 0x97D7;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $00D6
    cpu.state_.pc = 0x97DC;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $00D6
    cpu.state_.pc = 0x97E1;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $00D6
    cpu.state_.pc = 0x97E6;
    ...
    // 9876: BNE $0144
    if (!cpu.zero_flag()) cpu.state_.pc = 0x98BC;
    ...
    // 9892: BEQ $015D
    if (cpu.zero_flag()) cpu.state_.pc = 0x98F1;
    ...
    // 98A3: BPL $016D
    if (!cpu.negative_flag()) cpu.state_.pc = 0x9912;
    ...
    // 98B4: BEQ $0190
    if (cpu.zero_flag()) cpu.state_.pc = 0x9846;
    ...
    // 98D6: BRA $01C2
    cpu.state_.pc = 0x989A;

    // 98D8: BCC $01C4
    if (!cpu.carry_flag()) cpu.state_.pc = 0x989E;

    // 98DA: BCS $01C7
    if (cpu.carry_flag()) cpu.state_.pc = 0x98A3;
