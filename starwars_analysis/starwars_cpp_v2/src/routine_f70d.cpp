...
void routine_f70d_impl(StarWarsCPU& cpu) {
    ...
    // F751: BPL $006F
    if (!cpu.negative_flag()) cpu.state_.pc = 0xF7C2;
    ...
    // F75D: BNE $005A
    if (!cpu.zero_flag()) cpu.state_.pc = 0xF7B9;

    // F75F: ANDB $1,X
    // TODO: Fix comma operator: ANDB $1,X

    // F761: BNE $005A
    if (!cpu.zero_flag()) cpu.state_.pc = 0xF7BD;

    // F763: LDA #$3B
    cpu.state_.a = 0x3B;

    // F765: BRA $005C
    cpu.state_.pc = 0xF7C3;
    ...
    // F771: BCS $006D
    if (cpu.carry_flag()) cpu.state_.pc = 0xF7E0;
    ...
    // F7AF: BCS $00A9
    if (cpu.carry_flag()) cpu.state_.pc = 0xF75A;
    ...
    // F7BA: BNE $009E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xF75A;
    ...
    // F7D8: BEQ $00D2
    if (cpu.zero_flag()) cpu.state_.pc = 0xF7AC;
    ...
    // F86F: BCC $0169
    if (!cpu.carry_flag()) cpu.state_.pc = 0xF8DA;

    // F871: LDX $3022
    cpu.state_.x = cpu.read_memory_word(0x3022);

    // F874: BRA $016C
    cpu.state_.pc = 0xF8E2;
    ...
    // F87C: BPL $0161
    if (!cpu.negative_flag()) cpu.state_.pc = 0xF8DF;
    ...
    // F8A5: BCS $018E
    if (cpu.carry_flag()) cpu.state_.pc = 0xF835;
    ...
    // F8EF: BNE $01F3
    if (!cpu.zero_flag()) cpu.state_.pc = 0xF8E4;

    // F8F1: CMPB #$FC
    cpu.compare_b(0xFC);

    // F8F3: BCS $01EF
    if (cpu.carry_flag()) cpu.state_.pc = 0xF8E4;
    ...
    // F8FE: BRA $020A
    cpu.state_.pc = 0xF90A;

    // F900: CMPB #$B0
    cpu.compare_b(0xB0);

    // F902: BCS $01F9
    if (cpu.carry_flag()) cpu.state_.pc = 0xF8FD;

    // F906: CMPA #$08
    cpu.compare_a(0x08);

    // F908: BCS $0202
    if (cpu.carry_flag()) cpu.state_.pc = 0xF90C;
