...
void routine_8e32_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_8e32.md
    // Address: 0x8E32
    ...
    // 8E38: BEQ $001F
    if (cpu.zero_flag()) cpu.state_.pc = 0x8E59;
    ...
    // 8E40: BNE $002A
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8E6C;
    ...
    // 8E60: BEQ $0000
    if (cpu.zero_flag()) cpu.state_.pc = 0x8E62;
    ...
    // 8E7D: BEQ $0062
    if (cpu.zero_flag()) cpu.state_.pc = 0x8EE1;

    // 8E7F: ANDA $15,X
    // TODO: Fix comma operator: ANDA $15,X

    // 8E82: BNE $0062
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8EE6;
    ...
    // 8E8E: BNE $0057
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8EE7;

    // 8E90: BCC $0057
    if (!cpu.carry_flag()) cpu.state_.pc = 0x8EE9;

    // 8E92: BRA $0049
    cpu.state_.pc = 0x8EDD;
    ...
    // 8EE4: BCS $00C2
    if (cpu.carry_flag()) cpu.state_.pc = 0x8EA8;

    // 8EE6: LDA $4B14
    cpu.state_.a = cpu.read_memory(0x4B14);

    // 8EEA: BCS $00BF
    if (cpu.carry_flag()) cpu.state_.pc = 0x8EAB;

    // 8EEC: LDX #$9078
    cpu.state_.x = 0x9078;

    // 8EEF: BRA $00C2
    cpu.state_.pc = 0x8EB3;
    ...
    // 8F12: BEQ $00F7
    if (cpu.zero_flag()) cpu.state_.pc = 0x8F0B;
    ...
    // 8F31: BCS $00D6
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F09;
    ...
    // 8F7D: BEQ $0151
    if (cpu.zero_flag()) cpu.state_.pc = 0x8FD0;

    // 8F7F: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8F81: BNE $017F
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9002;
    ...
    // 8F90: BCS $016E
    if (cpu.carry_flag()) cpu.state_.pc = 0x9000;

    // 8F92: LDA $4B14
    cpu.state_.a = cpu.read_memory(0x4B14);

    // 8F96: BCS $016B
    if (cpu.carry_flag()) cpu.state_.pc = 0x9003;

    // 8F98: LDX #$9078
    cpu.state_.x = 0x9078;

    // 8F9B: BRA $016E
    cpu.state_.pc = 0x900B;
    ...
    // 8FB8: BEQ $0194
    if (cpu.zero_flag()) cpu.state_.pc = 0x8F4E;
    ...
    // 8FC2: BCS $0182
    if (cpu.carry_flag()) cpu.state_.pc = 0x8F46;

    // 8FC4: BRA $01F1
    cpu.state_.pc = 0x8FB7;

    // 8FC6: LDU <$E4
    cpu.state_.u = cpu.read_memory_word(0xE4);

    // 8FC8: BEQ $01F1
    if (cpu.zero_flag()) cpu.state_.pc = 0x8FBB;

    // 8FCA: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8FCC: BEQ $01F1
    if (cpu.zero_flag()) cpu.state_.pc = 0x8FBF;
