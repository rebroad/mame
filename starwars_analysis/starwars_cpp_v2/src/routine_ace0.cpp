...
void routine_ace0_impl(StarWarsCPU& cpu) {
    ...
    // AD24: BEQ $005D
    if (cpu.zero_flag()) cpu.state_.pc += 0x005D;
    ...
    // AD7B: BMI $009F
    if (cpu.negative_flag()) cpu.state_.pc += 0x009F;
    ...
    // AE06: BGE $017F
    // TODO: Convert BGE $017F

    // AE08: LDD $5004
    cpu.state_.d = cpu.read_memory_word(0x5004);

    // AE0B: BPL $0131
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0131;
    ...
    // AE44: BPL $015E
    if (!cpu.negative_flag()) cpu.state_.pc += 0x015E;
    ...
    // AEC9: BEQ $0245
    if (cpu.zero_flag()) cpu.state_.pc += 0x0245;
