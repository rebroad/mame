...
void routine_953b_impl(StarWarsCPU& cpu) {
    ...
    // 9540: BEQ $001C
    if (cpu.zero_flag()) cpu.state_.pc = 0x955E;
    ...
    // 954F: BCC $0018
    if (!cpu.carry_flag()) cpu.state_.pc = 0x9569;
    ...
    // 955A: BLE $0065
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x95C1;

    // 955C: LDA <$8C
    cpu.state_.a = cpu.read_memory(0x8C);

    // 955E: BGT $0065
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x95C5;
    ...
    // 9570: BGE $003D
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x95AF;
    ...
    // 957C: BNE $004B
    if (!cpu.zero_flag()) cpu.state_.pc = 0x95C9;
    ...
    // 9584: BRA $005E
    cpu.state_.pc = 0x95E4;

    // 9586: CMPA #$01
    cpu.compare_a(0x01);

    // 9588: BNE $0054
    if (!cpu.zero_flag()) cpu.state_.pc = 0x95DE;

    // 958A: JSR $BDDF
    cpu.call_function(0xBDDF);

    // 958D: BRA $005E
    cpu.state_.pc = 0x95ED;

    // 958F: CMPA #$02
    cpu.compare_a(0x02);

    // 9591: BNE $005E
    if (!cpu.zero_flag()) cpu.state_.pc = 0x95F1;
    ...
    // 95A2: BEQ $006B
    if (cpu.zero_flag()) cpu.state_.pc = 0x960F;
    ...
    // 95A9: BNE $0077
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9622;

    // 95AB: LDA <$60
    cpu.state_.a = cpu.read_memory(0x60);

    // 95AD: BGT $0077
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x9626;
    ...
    // 95CD: BEQ $0099
    if (cpu.zero_flag()) cpu.state_.pc = 0x9568;

    // 95CF: JSR $962A
    cpu.call_function(0x962A);

    // 95D2: BRA $009C
    cpu.state_.pc = 0x9570;
    ...
    // 95E3: BLE $00AF
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x9594;

    // 95E5: LDU #$A018
    cpu.state_.u = 0xA018;

    // 95E8: BRA $00B7
    cpu.state_.pc = 0x95A1;
    ...
    // 95F8: BGE $00C0
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x95BA;
    ...
    // 9619: BEQ $00E2
    if (cpu.zero_flag()) cpu.state_.pc = 0x95FD;
    ...
    // 962C: BNE $0110
    if (!cpu.zero_flag()) cpu.state_.pc = 0x963E;
    ...
    // 9641: BNE $010E
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9651;

    // 9643: INC <$8D
    // TODO: Convert INC <$8D

    // 9645: BLT $010E
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x9655;

    // 9647: INC <$91
    // TODO: Convert INC <$91

    // 9649: BRA $0165
    cpu.state_.pc = 0x96B0;
    ...
    // 9655: BNE $0139
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9690;
    ...
    // 9665: BNE $0137
    if (!cpu.zero_flag()) cpu.state_.pc = 0x969E;
    ...
    // 9672: BRA $0165
    cpu.state_.pc = 0x96D9;

    // 9674: LDA <$8F
    cpu.state_.a = cpu.read_memory(0x8F);

    // 9676: BNE $0155
    if (!cpu.zero_flag()) cpu.state_.pc = 0x96CD;
    ...
    // 9686: BNE $0153
    if (!cpu.zero_flag()) cpu.state_.pc = 0x96DB;

    // 9688: DEC <$8D
    // TODO: Convert DEC <$8D

    // 968A: BGT $0153
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x96DF;

    // 968C: INC <$8F
    // TODO: Convert INC <$8F

    // 968E: BRA $0165
    cpu.state_.pc = 0x96F5;
    ...
    // 9694: BNE $0165
    if (!cpu.zero_flag()) cpu.state_.pc = 0x96FB;
    ...
    // 9725: BNE $01FE
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9725;

    // 9727: LDB $4B15
    cpu.state_.b = cpu.read_memory(0x4B15);

    // 972A: BEQ $01FE
    if (cpu.zero_flag()) cpu.state_.pc = 0x972A;
