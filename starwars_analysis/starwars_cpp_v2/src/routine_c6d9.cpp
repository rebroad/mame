...
void routine_c6d9_impl(StarWarsCPU& cpu) {
    ...
    // C71A: BCS $0045
    if (cpu.carry_flag()) cpu.state_.pc += 0x0045;
    ...
    // C7AA: BCS $00FD
    if (cpu.carry_flag()) cpu.state_.pc += 0x00FD;
    ...
    // C85B: BNE $0194
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0194;
    ...
    // C8C4: BMI $0208
    if (cpu.negative_flag()) cpu.state_.pc += 0x0208;
