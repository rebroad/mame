...
void routine_9604_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_9604.md
    // Address: 0x9604
    ...
    // 962C: BNE $0047
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9675;
    ...
    // 9641: BNE $0045
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9688;

    // 9643: INC <$8D
    // TODO: Convert INC <$8D

    // 9645: BLT $0045
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x968C;

    // 9647: INC <$91
    // TODO: Convert INC <$91

    // 9649: BRA $009C
    cpu.state_.pc = 0x95E7;
    ...
    // 9655: BNE $0070
    if (!cpu.zero_flag()) cpu.state_.pc = 0x96C7;
    ...
    // 9665: BNE $006E
    if (!cpu.zero_flag()) cpu.state_.pc = 0x96D5;
    ...
    // 9686: BNE $008A
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9612;

    // 9688: DEC <$8D
    // TODO: Convert DEC <$8D

    // 968A: BGT $008A
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x9616;

    // 968C: INC <$8F
    // TODO: Convert INC <$8F

    // 968E: BRA $009C
    cpu.state_.pc = 0x962C;
    ...
    // 9725: BNE $0135
    if (!cpu.zero_flag()) cpu.state_.pc = 0x975C;
    ...
    // 9761: BEQ $0170
    if (cpu.zero_flag()) cpu.state_.pc = 0x97D3;
    ...
    // 9777: BEQ $01A7
    if (cpu.zero_flag()) cpu.state_.pc = 0x9720;
    ...
    // 97A3: BNE $0183
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9728;
    ...
    // 97B1: BCS $01B4
    if (cpu.carry_flag()) cpu.state_.pc = 0x9767;

    // 97B3: LDU #$9847
    cpu.state_.u = 0x9847;

    // 97B6: BRA $01BD
    cpu.state_.pc = 0x9775;
    ...
    // 97F5: BRA $020C
    cpu.state_.pc = 0x9803;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $020C
    cpu.state_.pc = 0x9808;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $020C
    cpu.state_.pc = 0x980D;
