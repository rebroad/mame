...
void routine_7a48_impl(CPU6809& cpu) {
    ...
    // 7B12: LDA $A9BA
    cpu.m_a = cpu.read_memory(0xA9BA);

    // 7B15: JSR [W,Y]
    // TODO: Complex indexed addressing: [W,Y]

    // 7B17: STU <$BB
    cpu.write_memory16(0xBB, cpu.m_u);

    // 7B19: LDA $BBDF
    cpu.m_a = cpu.read_memory(0xBBDF);

    // 7B1C: LDA $AB7E
    cpu.m_a = cpu.read_memory(0xAB7E);
    ...
    // 7B7D: BCS $01B5
    if (cpu.carry_flag()) cpu.m_pc = 0x7B34;
    ...
    // 7BA3: BNE $00DF
    if (!cpu.zero_flag()) cpu.m_pc = 0x7B84;
    ...
    // 7BBD: BCS $01F5
    if (cpu.carry_flag()) cpu.m_pc = 0x7BB4;
    ...
    // 7BDD: BCS $0215
    if (cpu.carry_flag()) cpu.m_pc = 0x7BF4;
    ...
    // 7BE3: BCS $011F
    if (cpu.carry_flag()) cpu.m_pc = 0x7C04;
