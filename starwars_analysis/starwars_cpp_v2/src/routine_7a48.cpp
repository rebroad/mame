...
void routine_7a48_impl(StarWarsCPU& cpu) {
    ...
    // 7B01: BRA $00EC
    cpu.state_.pc = 0x7AEF;

    // 7B04: XANDCC #$33
    // TODO: Convert XANDCC #$33

    // 7B06: BRA $0101
    cpu.state_.pc = 0x7B09;
    ...
    // 7B7D: BCS $01B5
    if (cpu.carry_flag()) cpu.state_.pc = 0x7B34;
    ...
    // 7BA3: BNE $00DF
    if (!cpu.zero_flag()) cpu.state_.pc = 0x7B84;
    ...
    // 7BBD: BCS $01F5
    if (cpu.carry_flag()) cpu.state_.pc = 0x7BB4;
    ...
    // 7BDD: BCS $0215
    if (cpu.carry_flag()) cpu.state_.pc = 0x7BF4;
    ...
    // 7BE3: BCS $011F
    if (cpu.carry_flag()) cpu.state_.pc = 0x7C04;
