...
void routine_7707_impl(StarWarsCPU& cpu) {
    ...
    // 777D: BGE $009C
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x771B;
    ...
    // 7786: BPL $0085
    if (!cpu.negative_flag()) cpu.state_.pc = 0x770D;
    ...
    // 778F: BGE $009C
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x772D;
    ...
    // 77A7: BLE $00CC
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x7775;
    ...
    // 77B3: BPL $00B2
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7767;
    ...
    // 77BC: BGE $00CC
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x778A;
    ...
    // 77CE: BGE $00CC
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x779C;
    ...
    // 7809: BCC $0107
    if (!cpu.carry_flag()) cpu.state_.pc = 0x7812;
    ...
    // 7823: BNE $0123
    if (!cpu.zero_flag()) cpu.state_.pc = 0x7848;

    // 7825: LDU #$B728
    cpu.state_.u = 0xB728;

    // 7828: BRA $0132
    cpu.state_.pc = 0x785C;
    ...
    // 7871: BEQ $016F
    if (cpu.zero_flag()) cpu.state_.pc = 0x78E2;
    ...
    // 787E: BCS $0166
    if (cpu.carry_flag()) cpu.state_.pc = 0x78E6;
    ...
    // 78D2: BNE $01DD
    if (!cpu.zero_flag()) cpu.state_.pc = 0x78B1;
    ...
    // 78E2: BRA $01F6
    cpu.state_.pc = 0x78DA;

    // 78E4: CMPA $2,X
    // TODO: Fix comma operator: CMPA $2,X

    // 78E6: BNE $01F6
    if (!cpu.zero_flag()) cpu.state_.pc = 0x78DE;

    // 78E8: CMPU $4B39
    // TODO: Convert CMPU $4B39

    // 78EC: BGT $01EC
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x78DA;

    // 78EE: STU $4B39
    cpu.write_memory(0x4B39, cpu.state_.u);

    // 78F1: BRA $01F6
    cpu.state_.pc = 0x78E9;

    // 78F3: BCS $01F6
    if (cpu.carry_flag()) cpu.state_.pc = 0x78EB;
    ...
    // 7904: BNE $0202
    if (!cpu.zero_flag()) cpu.state_.pc = 0x7908;
