...
void routine_786a_impl(StarWarsCPU& cpu) {
    ...
    // 78D2: BNE $007A
    if (!cpu.zero_flag()) cpu.state_.pc += 0x007A;
    ...
    // 78E2: BRA $0093
    cpu.state_.pc += 0x0093;

    // 78E4: CMPA $2,X
    // TODO: Fix comma operator: CMPA $2,X

    // 78E6: BNE $0093
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0093;

    // 78E8: CMPU $4B39
    // TODO: Convert CMPU $4B39

    // 78EC: BGT $0089
    // TODO: Convert BGT $0089
    ...
    // 78F3: BCS $0093
    if (cpu.carry_flag()) cpu.state_.pc += 0x0093;
    ...
    // 7968: BRA $0103
    cpu.state_.pc += 0x0103;

    // 796A: JSR $BD2B
    cpu.call_function(0xBD2B);

    // 796D: BRA $0108
    cpu.state_.pc += 0x0108;
