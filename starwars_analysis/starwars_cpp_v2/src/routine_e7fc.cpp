...
void routine_e7fc_impl(StarWarsCPU& cpu) {
    ...
    // E871: BRA $008C
    cpu.state_.pc = 0xE7FF;

    // E873: CMPB #$5A
    cpu.compare_b(0x5A);

    // E875: BNE $0080
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE7F7;

    // E877: LDX #$3000
    cpu.state_.x = 0x3000;

    // E87A: BRA $008C
    cpu.state_.pc = 0xE808;
    ...
    // E947: BNE $012F
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE978;
