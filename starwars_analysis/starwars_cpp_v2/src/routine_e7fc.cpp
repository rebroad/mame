...
void routine_e7fc_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_e7fc.md
    // Address: 0xE7FC
    ...
    // E833: BCS $0040
    if (cpu.carry_flag()) cpu.state_.pc = 0xE875;
    ...
    // E83A: BRA $008C
    cpu.state_.pc = 0xE8C8;

    // E83C: CMPB #$74
    cpu.compare_b(0x74);

    // E83E: BNE $0049
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE889;

    // E840: LDX #$2FDE
    cpu.state_.x = 0x2FDE;

    // E843: BRA $008C
    cpu.state_.pc = 0xE8D1;

    // E845: CMPB #$80
    cpu.compare_b(0x80);

    // E847: BNE $0053
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE89C;

    // E849: LDX #$3058
    cpu.state_.x = 0x3058;

    // E84D: BRA $008C
    cpu.state_.pc = 0xE8DB;

    // E84F: CMPB #$60
    cpu.compare_b(0x60);

    // E851: BCS $005C
    if (cpu.carry_flag()) cpu.state_.pc = 0xE8AF;

    // E853: LDX #$2FA4
    cpu.state_.x = 0x2FA4;

    // E856: BRA $008C
    cpu.state_.pc = 0xE8E4;

    // E858: CMPB #$40
    cpu.compare_b(0x40);

    // E85A: BNE $0065
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8C1;

    // E85C: LDX #$2FC2
    cpu.state_.x = 0x2FC2;

    // E85F: BRA $008C
    cpu.state_.pc = 0xE8ED;

    // E861: CMPB #$4E
    cpu.compare_b(0x4E);

    // E863: BNE $006E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8D3;

    // E865: LDX #$2FFE
    cpu.state_.x = 0x2FFE;

    // E868: BRA $008C
    cpu.state_.pc = 0xE8F6;

    // E86A: CMPB #$58
    cpu.compare_b(0x58);

    // E86C: BNE $0077
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8E5;

    // E86E: LDX #$2FF6
    cpu.state_.x = 0x2FF6;

    // E871: BRA $008C
    cpu.state_.pc = 0xE8FF;

    // E873: CMPB #$5A
    cpu.compare_b(0x5A);

    // E875: BNE $0080
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8F7;

    // E877: LDX #$3000
    cpu.state_.x = 0x3000;

    // E87A: BRA $008C
    cpu.state_.pc = 0xE908;

    // E87C: CMPB #$4A
    cpu.compare_b(0x4A);

    // E87E: BNE $0089
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE909;

    // E880: LDX #$300C
    cpu.state_.x = 0x300C;

    // E883: BRA $008C
    cpu.state_.pc = 0xE911;
    ...
    // E891: BPL $002F
    if (!cpu.negative_flag()) cpu.state_.pc = 0xE8C2;
    ...
    // E945: BRA $012D
    cpu.state_.pc = 0xEA74;

    // E947: BNE $012F
    if (!cpu.zero_flag()) cpu.state_.pc = 0xEA78;
