...
void routine_be00_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_be00.md
    // Address: 0xBE00

    // BE00: CMPX $E986
    cpu.compare_x(cpu.read_memory_word(0xE986));

    // BE03: PSHU S,Y,X,DP,D
    // TODO: Convert PSHU S,Y,X,DP,D

    // BE05: CMPX $E986
    cpu.compare_x(cpu.read_memory_word(0xE986));

    // BE08: PULU D,DP,X,Y,S
    // TODO: Convert PULU D,DP,X,Y,S

    // BE0A: CMPX $E986
    cpu.compare_x(cpu.read_memory_word(0xE986));

    // BE0D: XANDCC #$7E
    // TODO: Convert XANDCC #$7E
    ...
    // BE28: BCS $0022
    if (cpu.carry_flag()) cpu.state_.pc = 0xBE4C;
    ...
    // BE52: BPL $004C
    if (!cpu.negative_flag()) cpu.state_.pc = 0xBEA0;
    ...
    // BE5F: BCS $003A
    if (cpu.carry_flag()) cpu.state_.pc = 0xBE9B;
    ...
    // BF4C: BEQ $017C
    if (cpu.zero_flag()) cpu.state_.pc = 0xC0CA;
    ...
    // BF64: BCS $0169
    if (cpu.carry_flag()) cpu.state_.pc = 0xC0CF;
    ...
    // BF77: BCS $017C
    if (cpu.carry_flag()) cpu.state_.pc = 0xC0F5;
    ...
    // BFA3: BEQ $01B4
    if (cpu.zero_flag()) cpu.state_.pc = 0xC159;
    ...
    // BFED: BCS $01F4
    if (cpu.carry_flag()) cpu.state_.pc = 0xC1E3;

    // BFEF: LDD #$1ED4
    cpu.state_.d = 0x1ED4;

    // BFF2: BRA $01F7
    cpu.state_.pc = 0xC1EB;
