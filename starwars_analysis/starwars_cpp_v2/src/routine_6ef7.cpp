...
void routine_6ef7_impl(StarWarsCPU& cpu) {
    ...
    // 6F8B: BPL $0097
    if (!cpu.negative_flag()) cpu.state_.pc = 0x6F24;

    // 6F90: TST <$7D
    // TODO: Convert TST <$7D

    // 6F92: BGE $00A1
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x6F35;
    ...
    // 6FAF: BPL $00BB
    if (!cpu.negative_flag()) cpu.state_.pc = 0x6F6C;

    // 6FB4: TST <$7F
    // TODO: Convert TST <$7F

    // 6FB6: BPL $00C5
    if (!cpu.negative_flag()) cpu.state_.pc = 0x6F7D;
    ...
    // 6FC9: BLE $00D9
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6FA4;

    // 6FCB: LDD #$1C00
    cpu.state_.d = 0x1C00;

    // 6FCE: BRA $00E2
    cpu.state_.pc = 0x6FB2;

    // 6FD0: CMPD #$0200
    // TODO: Convert CMPD #$0200

    // 6FD4: BGE $00E2
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x6FB8;
    ...
    // 6FE7: BLE $00F8
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x6FE1;
    ...
    // 6FF8: BLE $0109
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x7003;
    ...
    // 701C: BCC $012C
    if (!cpu.carry_flag()) cpu.state_.pc = 0x704A;

    // 701E: LDD #$0BB8
    cpu.state_.d = 0x0BB8;

    // 7021: BRA $012F
    cpu.state_.pc = 0x7052;
    ...
    // 704F: BPL $015B
    if (!cpu.negative_flag()) cpu.state_.pc = 0x70AC;

    // 7053: TST <$7D
    // TODO: Convert TST <$7D

    // 7055: BGE $0164
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x70BB;
    ...
    // 7068: BLE $0176
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x70E0;
    ...
    // 7071: BGE $017F
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x70F2;
    ...
    // 7089: BPL $0195
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7020;

    // 708E: TST <$7F
    // TODO: Convert TST <$7F

    // 7090: BPL $019F
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7031;
    ...
    // 70A8: BLE $01B6
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x7060;
    ...
    // 70B1: BGE $01BF
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x7072;
    ...
    // 70C6: BEQ $01D4
    if (cpu.zero_flag()) cpu.state_.pc = 0x709C;
    ...
    // 70D5: BEQ $01E3
    if (cpu.zero_flag()) cpu.state_.pc = 0x70BA;
