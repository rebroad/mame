...
void routine_70f0_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_70f0.md
    // Address: 0x70F0
    ...
    // 70FD: BMI $0017
    if (cpu.negative_flag()) cpu.state_.pc = 0x7116;

    // 70FF: ADDA $8,X
    // TODO: Fix comma operator: ADDA $8,X

    // 7101: BVC $0015
    // TODO: Convert BVC $0015

    // 7103: LDA #$7F
    cpu.state_.a = 0x7F;

    // 7105: BRA $001E
    cpu.state_.pc = 0x7125;
    ...
    // 7113: BPL $0026
    if (!cpu.negative_flag()) cpu.state_.pc = 0x713B;

    // 7116: CMPA #$4E
    cpu.compare_a(0x4E);

    // 7118: BCS $004A
    if (cpu.carry_flag()) cpu.state_.pc = 0x7164;
    ...
    // 7122: BPL $003F
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7163;
    ...
    // 713C: BCS $006E
    if (cpu.carry_flag()) cpu.state_.pc = 0x71AC;
    ...
    // 7146: BPL $0063
    if (!cpu.negative_flag()) cpu.state_.pc = 0x71AB;
    ...
    // 7163: BEQ $0091
    if (cpu.zero_flag()) cpu.state_.pc = 0x70F6;
    ...
    // 7184: BEQ $00B2
    if (cpu.zero_flag()) cpu.state_.pc = 0x7138;
    ...
    // 718C: BPL $00A7
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7135;
    ...
    // 71A5: BEQ $00D3
    if (cpu.zero_flag()) cpu.state_.pc = 0x717A;
    ...
    // 71AD: BPL $00C8
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7177;
    ...
    // 71C4: BPL $00D7
    if (!cpu.negative_flag()) cpu.state_.pc = 0x719D;
    ...
    // 71E1: BCS $00F3
    if (cpu.carry_flag()) cpu.state_.pc = 0x71D6;
    ...
    // 725F: BCC $015D
    if (!cpu.carry_flag()) cpu.state_.pc = 0x72BE;
    ...
    // 72D8: BNE $0200
    if (!cpu.zero_flag()) cpu.state_.pc = 0x72DA;
    ...
    // 72E1: BCC $0200
    if (!cpu.carry_flag()) cpu.state_.pc = 0x72E3;

    // 72E3: ANDB #$10
    cpu.state_.b &= 0x10;

    // 72E5: BNE $01FB
    if (!cpu.zero_flag()) cpu.state_.pc = 0x72E2;

    // 72E7: LDB #$4C
    cpu.state_.b = 0x4C;

    // 72E9: BRA $01FD
    cpu.state_.pc = 0x72E8;
