...
void routine_c70e_impl(CPU6809& cpu) {
    ...
    // C71A: BCS $0010
    if (cpu.carry_flag()) cpu.m_pc = 0xC72C;
    ...
    // C72C: BEQ $0026
    if (cpu.zero_flag()) cpu.m_pc = 0xC754;
    ...
    // C7AA: BCS $00C8
    if (cpu.carry_flag()) cpu.m_pc = 0xC774;
    ...
    // C840: BNE $0139
    if (!cpu.zero_flag()) cpu.m_pc = 0xC87B;
    ...
    // C854: BMI $0163
    if (cpu.negative_flag()) cpu.m_pc = 0xC8B9;
    ...
    // C862: BNE $015B
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8BF;
    ...
    // C873: BNE $0176
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8EB;

    // C875: TST $4AEC
    // TODO: Convert TST $4AEC

    // C878: BMI $0171
    if (cpu.negative_flag()) cpu.m_pc = 0xC8EB;
    ...
    // C88C: BMI $019B
    if (cpu.negative_flag()) cpu.m_pc = 0xC829;
    ...
    // C893: BNE $0197
    if (!cpu.zero_flag()) cpu.m_pc = 0xC82C;
    ...
    // C89A: BNE $0193
    if (!cpu.zero_flag()) cpu.m_pc = 0xC82F;
    ...
    // C8AB: BNE $01AE
    if (!cpu.zero_flag()) cpu.m_pc = 0xC85B;

    // C8AD: TST $4AEC
    // TODO: Convert TST $4AEC

    // C8B0: BMI $01A9
    if (cpu.negative_flag()) cpu.m_pc = 0xC85B;
    ...
    // C8C4: BMI $01D3
    if (cpu.negative_flag()) cpu.m_pc = 0xC899;
    ...
    // C8CB: BNE $01CF
    if (!cpu.zero_flag()) cpu.m_pc = 0xC89C;
    ...
    // C8D2: BNE $01CB
    if (!cpu.zero_flag()) cpu.m_pc = 0xC89F;
    ...
    // C8E3: BNE $01E6
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8CB;

    // C8E5: TST $4AEC
    // TODO: Convert TST $4AEC

    // C8E8: BMI $01E1
    if (cpu.negative_flag()) cpu.m_pc = 0xC8CB;
