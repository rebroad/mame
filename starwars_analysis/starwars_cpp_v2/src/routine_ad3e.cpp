...
void routine_ad3e_impl(StarWarsCPU& cpu) {
    ...
    // ADB1: BEQ $0095
    if (cpu.zero_flag()) cpu.state_.pc = 0xAD48;
    ...
    // ADE5: BGT $00AD
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xAD94;
    ...
    // ADF2: BLT $0121
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xAE15;
    ...
    // ADFD: BPL $00C5
    if (!cpu.negative_flag()) cpu.state_.pc = 0xADC4;
    ...
    // AE06: BGE $0121
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xAE29;

    // AE08: LDD $5004
    cpu.state_.d = cpu.read_memory_word(0x5004);

    // AE0B: BPL $00D3
    if (!cpu.negative_flag()) cpu.state_.pc = 0xADE0;
    ...
    // AE14: BGE $0121
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xAE37;
    ...
    // AE40: BEQ $010A
    if (cpu.zero_flag()) cpu.state_.pc = 0xAE4C;

    // AE44: BPL $0100
    if (!cpu.negative_flag()) cpu.state_.pc = 0xAE46;
    ...
    // AE7A: BEQ $0149
    if (cpu.zero_flag()) cpu.state_.pc = 0xAEC5;
    ...
    // AE89: BLE $0161
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xAEEC;
    ...
    // AEAA: BNE $0171
    if (!cpu.zero_flag()) cpu.state_.pc = 0xAF1D;
    ...
    // AEC1: BNE $0186
    if (!cpu.zero_flag()) cpu.state_.pc = 0xAE49;
    ...
    // AEC9: BEQ $01E7
    if (cpu.zero_flag()) cpu.state_.pc = 0xAEB2;
    ...
    // AF2A: BNE $0248
    if (!cpu.zero_flag()) cpu.state_.pc = 0xAF74;
