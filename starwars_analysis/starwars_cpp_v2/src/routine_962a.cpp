...
void routine_962a_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_962a.md
    // Address: 0x962A
    ...
    // 962C: BNE $0021
    if (!cpu.zero_flag()) cpu.state_.pc = 0x964F;
    ...
    // 9641: BNE $001F
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9662;

    // 9643: INC <$8D
    // TODO: Convert INC <$8D

    // 9645: BLT $001F
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x9666;

    // 9647: INC <$91
    // TODO: Convert INC <$91

    // 9649: BRA $0076
    cpu.state_.pc = 0x96C1;
    ...
    // 9655: BNE $004A
    if (!cpu.zero_flag()) cpu.state_.pc = 0x96A1;
    ...
    // 9665: BNE $0048
    if (!cpu.zero_flag()) cpu.state_.pc = 0x96AF;
    ...
    // 9686: BNE $0064
    if (!cpu.zero_flag()) cpu.state_.pc = 0x96EC;

    // 9688: DEC <$8D
    // TODO: Convert DEC <$8D

    // 968A: BGT $0064
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x96F0;

    // 968C: INC <$8F
    // TODO: Convert INC <$8F

    // 968E: BRA $0076
    cpu.state_.pc = 0x9706;
    ...
    // 9694: BNE $0076
    if (!cpu.zero_flag()) cpu.state_.pc = 0x970C;
    ...
    // 9725: BNE $010F
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9736;

    // 9727: LDB $4B15
    cpu.state_.b = cpu.read_memory(0x4B15);

    // 972A: BEQ $010F
    if (cpu.zero_flag()) cpu.state_.pc = 0x973B;
    ...
    // 9761: BEQ $014A
    if (cpu.zero_flag()) cpu.state_.pc = 0x97AD;
    ...
    // 9777: BEQ $0181
    if (cpu.zero_flag()) cpu.state_.pc = 0x96FA;
    ...
    // 97B1: BCS $018E
    if (cpu.carry_flag()) cpu.state_.pc = 0x9741;

    // 97B3: LDU #$9847
    cpu.state_.u = 0x9847;

    // 97B6: BRA $0197
    cpu.state_.pc = 0x974F;
    ...
    // 97E6: BRA $01E6
    cpu.state_.pc = 0x97CE;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $01E6
    cpu.state_.pc = 0x97D3;
    ...
    // 97F5: BRA $01E6
    cpu.state_.pc = 0x97DD;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $01E6
    cpu.state_.pc = 0x97E2;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $01E6
    cpu.state_.pc = 0x97E7;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $01E6
    cpu.state_.pc = 0x97EC;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $01E6
    cpu.state_.pc = 0x97F1;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $01E6
    cpu.state_.pc = 0x97F6;
