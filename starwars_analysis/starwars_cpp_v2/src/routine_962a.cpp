...
void routine_962a_impl(StarWarsCPU& cpu) {
    ...
    // 972A: BEQ $010F
    if (cpu.zero_flag()) cpu.state_.pc = 0x983B;
    ...
    // 9761: BEQ $014A
    if (cpu.zero_flag()) cpu.state_.pc = 0x98AD;
    ...
    // 9769: BNE $014A
    if (!cpu.zero_flag()) cpu.state_.pc = 0x98B5;
    ...
    // 97A3: BNE $015D
    if (!cpu.zero_flag()) cpu.state_.pc = 0x9902;
    ...
    // 97B1: BCS $018E
    if (cpu.carry_flag()) cpu.state_.pc = 0x9941;

    // 97B3: LDU #$9847
    cpu.state_.u = 0x9847;

    // 97B6: BRA $0197
    cpu.state_.pc = 0x994F;
    ...
    // 97E6: BRA $01E6
    cpu.state_.pc = 0x99CE;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $01E6
    cpu.state_.pc = 0x99D3;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $01E6
    cpu.state_.pc = 0x99D8;

    // 97F2: LDU #$9850
    cpu.state_.u = 0x9850;

    // 97F5: BRA $01E6
    cpu.state_.pc = 0x99DD;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $01E6
    cpu.state_.pc = 0x99E2;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $01E6
    cpu.state_.pc = 0x99E7;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $01E6
    cpu.state_.pc = 0x99EC;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $01E6
    cpu.state_.pc = 0x99F1;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $01E6
    cpu.state_.pc = 0x99F6;
