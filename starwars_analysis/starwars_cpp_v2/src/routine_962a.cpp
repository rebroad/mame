...
void routine_962a_impl(CPU6809& cpu) {
    ...
    // 962C: BNE $0021
    if (!cpu.zero_flag()) cpu.m_pc = 0x964F;
    ...
    // 9641: BNE $001F
    if (!cpu.zero_flag()) cpu.m_pc = 0x9662;

    // 9643: INC <$8D
    // TODO: Convert INC <$8D

    // 9645: BLT $001F
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x9666;
    ...
    // 9655: BNE $004A
    if (!cpu.zero_flag()) cpu.m_pc = 0x96A1;
    ...
    // 9665: BNE $0048
    if (!cpu.zero_flag()) cpu.m_pc = 0x96AF;
    ...
    // 9686: BNE $0064
    if (!cpu.zero_flag()) cpu.m_pc = 0x96EC;

    // 9688: DEC <$8D
    // TODO: Convert DEC <$8D

    // 968A: BGT $0064
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x96F0;
    ...
    // 9694: BNE $0076
    if (!cpu.zero_flag()) cpu.m_pc = 0x970C;
    ...
    // 972A: BEQ $010F
    if (cpu.zero_flag()) cpu.m_pc = 0x973B;
    ...
    // 9761: BEQ $014A
    if (cpu.zero_flag()) cpu.m_pc = 0x97AD;
    ...
    // 9769: BNE $014A
    if (!cpu.zero_flag()) cpu.m_pc = 0x97B5;
    ...
    // 9777: BEQ $0181
    if (cpu.zero_flag()) cpu.m_pc = 0x96FA;
    ...
    // 97A3: BNE $015D
    if (!cpu.zero_flag()) cpu.m_pc = 0x9802;
    ...
    // 97B1: BCS $018E
    if (cpu.carry_flag()) cpu.m_pc = 0x9741;
