...
void routine_60be_impl(CPU6809& cpu) {
    ...
    // 60C0: BNE $000C
    if (!cpu.zero_flag()) cpu.m_pc = 0x60CE;

    // 60C2: INC <$42
    // TODO: Convert INC <$42

    // 60C4: BNE $000C
    if (!cpu.zero_flag()) cpu.m_pc = 0x60D2;
    ...
    // 60CD: BNE $0016
    if (!cpu.zero_flag()) cpu.m_pc = 0x60E5;
    ...
    // 60D7: BNE $0026
    if (!cpu.zero_flag()) cpu.m_pc = 0x60FF;
    ...
    // 60EA: BCC $0036
    if (!cpu.carry_flag()) cpu.m_pc = 0x6122;
    ...
    // 612C: BCS $0066
    if (cpu.carry_flag()) cpu.m_pc = 0x6194;
    ...
    // 6141: BCS $0097
    if (cpu.carry_flag()) cpu.m_pc = 0x60DA;
    ...
    // 6167: BNE $00B3
    if (!cpu.zero_flag()) cpu.m_pc = 0x611C;
    ...
    // 61A5: BCS $00D8
    if (cpu.carry_flag()) cpu.m_pc = 0x617F;
    ...
    // 620C: BCS $0135
    if (cpu.carry_flag()) cpu.m_pc = 0x6243;
    ...
    // 622A: BCS $0154
    if (cpu.carry_flag()) cpu.m_pc = 0x6280;
    ...
    // 6231: BGT $0180
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x61B3;
    ...
    // 6240: BLT $018D
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x61CF;
    ...
    // 6250: BNE $0199
    if (!cpu.zero_flag()) cpu.m_pc = 0x61EB;
    ...
    // 625A: BEQ $01AB
    if (cpu.zero_flag()) cpu.m_pc = 0x6207;
    ...
    // 6260: BEQ $01AB
    if (cpu.zero_flag()) cpu.m_pc = 0x620D;
    ...
    // 626E: BNE $01B6
    if (!cpu.zero_flag()) cpu.m_pc = 0x6226;
