...
void routine_d942_impl(StarWarsCPU& cpu) {
    ...
    // D949: BCC $001B
    if (!cpu.carry_flag()) cpu.state_.pc += 0x001B;
    ...
    // D94F: BCC $0015
    if (!cpu.carry_flag()) cpu.state_.pc += 0x0015;
    ...
    // D995: BEQ $0091
    if (cpu.zero_flag()) cpu.state_.pc += 0x0091;
    ...
    // DA9A: BCC $01B3
    if (!cpu.carry_flag()) cpu.state_.pc += 0x01B3;
    ...
    // DAA9: BCS $016F
    if (cpu.carry_flag()) cpu.state_.pc += 0x016F;
    ...
    // DACB: BCS $01AC
    if (cpu.carry_flag()) cpu.state_.pc += 0x01AC;
    ...
    // DAD5: BNE $019D
    if (!cpu.zero_flag()) cpu.state_.pc += 0x019D;
    ...
    // DB07: BCS $01EC
    if (cpu.carry_flag()) cpu.state_.pc += 0x01EC;
