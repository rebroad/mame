...
void routine_b579_impl(StarWarsCPU& cpu) {
    ...
    // B5F9: BGT $00D1
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xB5CC;
    ...
    // B607: BLT $00D7
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB5E0;

    // B609: LDD $5042
    cpu.state_.d = cpu.read_memory_word(0x5042);

    // B60C: BLT $00CC
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB5DA;
    ...
    // B617: BLT $00CC
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB5E5;

    // B619: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // B61C: BGT $00CC
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xB5EA;
    ...
    // B62F: BLT $00BE
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB5EF;
    ...
    // B640: BPL $00CA
    if (!cpu.negative_flag()) cpu.state_.pc = 0xB60C;
    ...
    // B665: BNE $0126
    if (!cpu.zero_flag()) cpu.state_.pc = 0xB68D;
    ...
    // B66E: BCC $00FE
    if (!cpu.carry_flag()) cpu.state_.pc = 0xB66E;

    // B670: BMI $00FE
    if (cpu.negative_flag()) cpu.state_.pc = 0xB670;

    // B672: INC $49BD
    // TODO: Convert INC $49BD

    // B675: BRA $0101
    cpu.state_.pc = 0xB678;

    // B678: BNE $00F4
    if (!cpu.zero_flag()) cpu.state_.pc = 0xB66E;

    // B67A: LDA $49BD
    cpu.state_.a = cpu.read_memory(0x49BD);

    // B67D: BEQ $0126
    if (cpu.zero_flag()) cpu.state_.pc = 0xB6A5;
    ...
    // B685: BCS $0110
    if (cpu.carry_flag()) cpu.state_.pc = 0xB697;
    ...
    // B693: BCC $011E
    if (!cpu.carry_flag()) cpu.state_.pc = 0xB6B3;
    ...
    // B6D9: BPL $0163
    if (!cpu.negative_flag()) cpu.state_.pc = 0xB73E;
    ...
    // B6E1: BPL $016B
    if (!cpu.negative_flag()) cpu.state_.pc = 0xB74E;
    ...
    // B6FA: BPL $0184
    if (!cpu.negative_flag()) cpu.state_.pc = 0xB680;
    ...
    // B702: BPL $018C
    if (!cpu.negative_flag()) cpu.state_.pc = 0xB690;
