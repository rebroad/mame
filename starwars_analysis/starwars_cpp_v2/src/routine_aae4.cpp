...
void routine_aae4_impl(StarWarsCPU& cpu) {
    ...
    // AB5A: BPL $007C
    if (!cpu.negative_flag()) cpu.state_.pc += 0x007C;
    ...
    // AB89: BPL $00AB
    if (!cpu.negative_flag()) cpu.state_.pc += 0x00AB;
    ...
    // ABB8: BCC $00DC
    if (!cpu.carry_flag()) cpu.state_.pc += 0x00DC;
    ...
    // ABC4: BEQ $013E
    if (cpu.zero_flag()) cpu.state_.pc += 0x013E;

    // ABC6: LDD $5018
    cpu.state_.d = cpu.read_memory_word(0x5018);

    // ABCB: BMI $013E
    if (cpu.negative_flag()) cpu.state_.pc += 0x013E;
    ...
    // ABDE: BLE $0101
    // TODO: Convert BLE $0101

    // ABE0: SUBD #$0110
    cpu.state_.d -= 0x0110;

    // ABE3: BGT $013E
    // TODO: Convert BGT $013E

    // ABE5: LDD <$D6
    cpu.state_.d = cpu.read_memory_word(0xD6);

    // ABE7: BMI $010C
    if (cpu.negative_flag()) cpu.state_.pc += 0x010C;

    // ABE9: SUBD #$01C0
    cpu.state_.d -= 0x01C0;

    // ABEC: BGE $013E
    // TODO: Convert BGE $013E
    ...
    // AC07: BNE $0130
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0130;
    ...
    // AC2B: BEQ $014D
    if (cpu.zero_flag()) cpu.state_.pc += 0x014D;

    // AC2D: LDA #$00
    cpu.state_.a = 0x00;

    // AC2F: BRA $014E
    cpu.state_.pc += 0x014E;
    ...
    // AC61: BPL $0177
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0177;
    ...
    // AC83: BCC $01A6
    if (!cpu.carry_flag()) cpu.state_.pc += 0x01A6;

    // AC85: LDA #$03
    cpu.state_.a = 0x03;

    // AC88: BRA $01AD
    cpu.state_.pc += 0x01AD;
