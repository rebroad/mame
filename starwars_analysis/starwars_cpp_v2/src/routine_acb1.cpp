...
void routine_acb1_impl(CPU6809& cpu) {
    ...
    // ACFB: BNE $004F
    if (!cpu.zero_flag()) cpu.m_pc = 0xAD4C;
    ...
    // AD03: BPL $0055
    if (!cpu.negative_flag()) cpu.m_pc = 0xAD5A;
    ...
    // AD0E: BNE $006E
    if (!cpu.zero_flag()) cpu.m_pc = 0xAD7E;
    ...
    // AD24: BEQ $008C
    if (cpu.zero_flag()) cpu.m_pc = 0xACB2;
    ...
    // AD63: BCS $00A6
    if (cpu.carry_flag()) cpu.m_pc = 0xAD0B;
    ...
    // AD6E: BEQ $00FD
    if (cpu.zero_flag()) cpu.m_pc = 0xAD6D;
    ...
    // AD7B: BMI $00CE
    if (cpu.negative_flag()) cpu.m_pc = 0xAD4B;
    ...
    // AD8B: BGE $00DE
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xAD6B;
    ...
    // AD98: BMI $00F2
    if (cpu.negative_flag()) cpu.m_pc = 0xAD8C;
    ...
    // ADAA: BLE $00FD
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xADA9;
    ...
    // ADB1: BEQ $0122
    if (cpu.zero_flag()) cpu.m_pc = 0xADD5;
    ...
    // ADE5: BGT $013A
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xAE21;
    ...
    // ADF2: BLT $01AE
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xADA2;
    ...
    // ADFD: BPL $0152
    if (!cpu.negative_flag()) cpu.m_pc = 0xAE51;
    ...
    // AE06: BGE $01AE
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xADB6;

    // AE08: LDD $5004
    cpu.m_d = cpu.read_memory16(0x5004);

    // AE0B: BPL $0160
    if (!cpu.negative_flag()) cpu.m_pc = 0xAE6D;
    ...
    // AE14: BGE $01AE
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xADC4;
    ...
    // AE40: BEQ $0197
    if (cpu.zero_flag()) cpu.m_pc = 0xADD9;

    // AE44: BPL $018D
    if (!cpu.negative_flag()) cpu.m_pc = 0xADD3;
    ...
    // AE7A: BEQ $01D6
    if (cpu.zero_flag()) cpu.m_pc = 0xAE52;
    ...
    // AE89: BLE $01EE
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xAE79;
    ...
    // AEA5: BNE $0201
    if (!cpu.zero_flag()) cpu.m_pc = 0xAEA8;
