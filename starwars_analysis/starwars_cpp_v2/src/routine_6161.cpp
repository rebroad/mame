...
void routine_6161_impl(CPU6809& cpu) {
    ...
    // 6167: BNE $0010
    if (!cpu.zero_flag()) cpu.m_pc = 0x6179;
    ...
    // 618C: BCS $001C
    if (cpu.carry_flag()) cpu.m_pc = 0x61AA;
    ...
    // 61A5: BCS $0035
    if (cpu.carry_flag()) cpu.m_pc = 0x61DC;
    ...
    // 620C: BCS $0092
    if (cpu.carry_flag()) cpu.m_pc = 0x61A0;
    ...
    // 622A: BCS $00B1
    if (cpu.carry_flag()) cpu.m_pc = 0x61DD;
    ...
    // 6231: BGT $00DD
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6210;
    ...
    // 6237: BEQ $00DB
    if (cpu.zero_flag()) cpu.m_pc = 0x6214;
    ...
    // 6240: BLT $00EA
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x622C;
    ...
    // 6246: BEQ $00EA
    if (cpu.zero_flag()) cpu.m_pc = 0x6232;
    ...
    // 625A: BEQ $0108
    if (cpu.zero_flag()) cpu.m_pc = 0x6264;
    ...
    // 6260: BEQ $0108
    if (cpu.zero_flag()) cpu.m_pc = 0x626A;
    ...
    // 626E: BNE $0113
    if (!cpu.zero_flag()) cpu.m_pc = 0x6283;
    ...
    // 631E: BPL $01C1
    if (!cpu.negative_flag()) cpu.m_pc = 0x62E1;
    ...
    // 6360: BPL $0201
    if (!cpu.negative_flag()) cpu.m_pc = 0x6363;
