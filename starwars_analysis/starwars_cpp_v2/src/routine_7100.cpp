...
void routine_7100_impl(CPU6809& cpu) {
    ...
    // 7113: BPL $0016
    if (!cpu.negative_flag()) cpu.m_pc = 0x712B;

    // 7116: CMPA #$4E
    cpu.compare_a(0x4E);

    // 7118: BCS $003A
    if (cpu.carry_flag()) cpu.m_pc = 0x7154;
    ...
    // 713C: BCS $005E
    if (cpu.carry_flag()) cpu.m_pc = 0x719C;
    ...
    // 7163: BEQ $0081
    if (cpu.zero_flag()) cpu.m_pc = 0x70E6;
    ...
    // 716B: BPL $0076
    if (!cpu.negative_flag()) cpu.m_pc = 0x71E3;
    ...
    // 7184: BEQ $00A2
    if (cpu.zero_flag()) cpu.m_pc = 0x7128;
    ...
    // 718C: BPL $0097
    if (!cpu.negative_flag()) cpu.m_pc = 0x7125;
    ...
    // 71A5: BEQ $00C3
    if (cpu.zero_flag()) cpu.m_pc = 0x716A;
    ...
    // 71AD: BPL $00B8
    if (!cpu.negative_flag()) cpu.m_pc = 0x7167;
    ...
    // 71C4: BPL $00C7
    if (!cpu.negative_flag()) cpu.m_pc = 0x718D;
    ...
    // 71E1: BCS $00E3
    if (cpu.carry_flag()) cpu.m_pc = 0x71C6;
    ...
    // 725F: BCC $014D
    if (!cpu.carry_flag()) cpu.m_pc = 0x72AE;
    ...
    // 72D8: BNE $01F0
    if (!cpu.zero_flag()) cpu.m_pc = 0x72CA;
    ...
    // 72E1: BCC $01F0
    if (!cpu.carry_flag()) cpu.m_pc = 0x72D3;

    // 72E3: ANDB #$10
    cpu.m_b &= 0x10;

    // 72E5: BNE $01EB
    if (!cpu.zero_flag()) cpu.m_pc = 0x72D2;
