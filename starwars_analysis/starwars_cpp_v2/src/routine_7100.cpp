...
void routine_7100_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_7100.md
    // Address: 0x7100
    ...
    // 7118: BCS $003A
    if (cpu.carry_flag()) cpu.state_.pc = 0x7154;
    ...
    // 7122: BPL $002F
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7153;
    ...
    // 713C: BCS $005E
    if (cpu.carry_flag()) cpu.state_.pc = 0x719C;
    ...
    // 7163: BEQ $0081
    if (cpu.zero_flag()) cpu.state_.pc = 0x70E6;
    ...
    // 716B: BPL $0076
    if (!cpu.negative_flag()) cpu.state_.pc = 0x71E3;
    ...
    // 7184: BEQ $00A2
    if (cpu.zero_flag()) cpu.state_.pc = 0x7128;
    ...
    // 718C: BPL $0097
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7125;
    ...
    // 71A5: BEQ $00C3
    if (cpu.zero_flag()) cpu.state_.pc = 0x716A;
    ...
    // 71AD: BPL $00B8
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7167;
    ...
    // 71C4: BPL $00C7
    if (!cpu.negative_flag()) cpu.state_.pc = 0x718D;
    ...
    // 71E1: BCS $00E3
    if (cpu.carry_flag()) cpu.state_.pc = 0x71C6;
    ...
    // 725F: BCC $014D
    if (!cpu.carry_flag()) cpu.state_.pc = 0x72AE;
    ...
    // 72D8: BNE $01F0
    if (!cpu.zero_flag()) cpu.state_.pc = 0x72CA;
    ...
    // 72E1: BCC $01F0
    if (!cpu.carry_flag()) cpu.state_.pc = 0x72D3;

    // 72E3: ANDB #$10
    cpu.state_.b &= 0x10;

    // 72E5: BNE $01EB
    if (!cpu.zero_flag()) cpu.state_.pc = 0x72D2;

    // 72E7: LDB #$4C
    cpu.state_.b = 0x4C;

    // 72E9: BRA $01ED
    cpu.state_.pc = 0x72D8;
