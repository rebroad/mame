...
void routine_caf3_impl(CPU6809& cpu) {
    ...
    // CB01: BCS $0015
    if (cpu.carry_flag()) cpu.m_pc = 0xCB18;
    ...
    // CB14: BPL $0027
    if (!cpu.negative_flag()) cpu.m_pc = 0xCB3D;
    ...
    // CB20: BCC $0056
    if (!cpu.carry_flag()) cpu.m_pc = 0xCB78;
    ...
    // CB2B: BPL $003E
    if (!cpu.negative_flag()) cpu.m_pc = 0xCB6B;
    ...
    // CB35: BCC $0056
    if (!cpu.carry_flag()) cpu.m_pc = 0xCB8D;
    ...
    // CB3D: BCC $0056
    if (!cpu.carry_flag()) cpu.m_pc = 0xCB95;
    ...
    // CB4E: BCS $0018
    if (cpu.carry_flag()) cpu.m_pc = 0xCB68;
    ...
    // CB55: BCC $0066
    if (!cpu.carry_flag()) cpu.m_pc = 0xCBBD;
    ...
    // CB5D: BEQ $00B0
    if (cpu.zero_flag()) cpu.m_pc = 0xCB0F;
    ...
    // CB64: BNE $008F
    if (!cpu.zero_flag()) cpu.m_pc = 0xCAF5;
    ...
    // CB74: BEQ $008A
    if (cpu.zero_flag()) cpu.m_pc = 0xCB00;
    ...
    // CB84: BNE $009E
    if (!cpu.zero_flag()) cpu.m_pc = 0xCB24;
    ...
    // CB99: BCS $00AD
    if (cpu.carry_flag()) cpu.m_pc = 0xCB48;
    ...
    // CBCB: BCC $00D9
    if (!cpu.carry_flag()) cpu.m_pc = 0xCBA6;

    // CBCD: INC <$FE
    // TODO: Convert INC <$FE

    // CBCF: BCC $00DD
    if (!cpu.carry_flag()) cpu.m_pc = 0xCBAE;

    // CBD1: CWAI #$FE
    // TODO: Convert CWAI #$FE

    // CBD3: BCC $00E1
    if (!cpu.carry_flag()) cpu.m_pc = 0xCBB6;

    // CBD5: INC [W,S]
    // TODO: Convert INC [W,S]

    // CBD7: BCC $00E5
    if (!cpu.carry_flag()) cpu.m_pc = 0xCBBE;

    // CBD9: CMPX <$FE
    cpu.compare_x(cpu.read_memory16(0xFE));

    // CBDB: BCC $00E9
    if (!cpu.carry_flag()) cpu.m_pc = 0xCBC6;
    ...
    // CBE3: BCC $00F2
    if (!cpu.carry_flag()) cpu.m_pc = 0xCBD7;

    // CBE5: BGE $00F2
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xCBD9;

    // CBE7: BCC $00F6
    if (!cpu.carry_flag()) cpu.m_pc = 0xCBDF;
    ...
    // CBF7: BCC $0107
    if (!cpu.carry_flag()) cpu.m_pc = 0xCC00;

    // CBF9: ANDCC #$FE
    // TODO: Convert ANDCC #$FE

    // CBFB: BCC $010B
    if (!cpu.carry_flag()) cpu.m_pc = 0xCC08;
    ...
    // CC20: BNE $0145
    if (!cpu.zero_flag()) cpu.m_pc = 0xCC67;
    ...
    // CC3F: BCC $0168
    if (!cpu.carry_flag()) cpu.m_pc = 0xCCA9;
    ...
    // CC4D: BCC $0168
    if (!cpu.carry_flag()) cpu.m_pc = 0xCCB7;
    ...
    // CC53: BCC $0168
    if (!cpu.carry_flag()) cpu.m_pc = 0xCCBD;

    // CC55: CMPX #$4AB6
    cpu.compare_x(0x4AB6);

    // CC58: BCS $0156
    if (cpu.carry_flag()) cpu.m_pc = 0xCCB0;
    ...
    // CC68: BCS $016E
    if (cpu.carry_flag()) cpu.m_pc = 0xCCD8;
    ...
    // CC77: BCS $017D
    if (cpu.carry_flag()) cpu.m_pc = 0xCCF6;
    ...
    // CCA6: BCS $0206
    if (cpu.carry_flag()) cpu.m_pc = 0xCCAE;
