...
void routine_a849_impl(StarWarsCPU& cpu) {
    ...
    // A850: BEQ $0018
    if (cpu.zero_flag()) cpu.state_.pc += 0x0018;
    ...
    // A857: BCC $0017
    if (!cpu.carry_flag()) cpu.state_.pc += 0x0017;
    ...
    // A911: BMI $00D6
    if (cpu.negative_flag()) cpu.state_.pc += 0x00D6;
    ...
    // A92C: BPL $00E9
    if (!cpu.negative_flag()) cpu.state_.pc += 0x00E9;
    ...
    // A958: BNE $0115
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0115;
    ...
    // A9C3: BMI $018C
    if (cpu.negative_flag()) cpu.state_.pc += 0x018C;
    ...
    // A9ED: BMI $01B6
    if (cpu.negative_flag()) cpu.state_.pc += 0x01B6;
