...
void routine_e7ad_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_e7ad.md
    // Address: 0xE7AD
    ...
    // E7B0: BNE $000E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE7C0;

    // E7B2: TST <$AD
    // TODO: Convert TST <$AD

    // E7B4: BLE $000E
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xE7C4;

    // E7B6: LDU $3002
    cpu.state_.u = cpu.read_memory_word(0x3002);

    // E7B9: BRA $0015
    cpu.state_.pc = 0xE7D0;
    ...
    // E7C9: BCC $0025
    if (!cpu.carry_flag()) cpu.state_.pc = 0xE7F0;

    // E7CB: STB $48AE
    cpu.write_memory(0x48AE, cpu.state_.b);

    // E7CE: BSR $0030
    // TODO: Convert BSR $0030

    // E7D0: BSR $003D
    // TODO: Convert BSR $003D

    // E7D3: CMPB #$D6
    cpu.compare_b(0xD6);

    // E7D5: BCC $002F
    if (!cpu.carry_flag()) cpu.state_.pc = 0xE806;
    ...
    // E833: BCS $008F
    if (cpu.carry_flag()) cpu.state_.pc = 0xE8C4;
    ...
    // E83A: BRA $00DB
    cpu.state_.pc = 0xE917;

    // E83C: CMPB #$74
    cpu.compare_b(0x74);

    // E83E: BNE $0098
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8D8;

    // E840: LDX #$2FDE
    cpu.state_.x = 0x2FDE;

    // E843: BRA $00DB
    cpu.state_.pc = 0xE920;

    // E845: CMPB #$80
    cpu.compare_b(0x80);

    // E847: BNE $00A2
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE8EB;

    // E849: LDX #$3058
    cpu.state_.x = 0x3058;

    // E84D: BRA $00DB
    cpu.state_.pc = 0xE92A;

    // E84F: CMPB #$60
    cpu.compare_b(0x60);

    // E851: BCS $00AB
    if (cpu.carry_flag()) cpu.state_.pc = 0xE8FE;

    // E853: LDX #$2FA4
    cpu.state_.x = 0x2FA4;

    // E856: BRA $00DB
    cpu.state_.pc = 0xE933;
    ...
    // E85F: BRA $00DB
    cpu.state_.pc = 0xE93C;

    // E861: CMPB #$4E
    cpu.compare_b(0x4E);

    // E863: BNE $00BD
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE922;

    // E865: LDX #$2FFE
    cpu.state_.x = 0x2FFE;

    // E868: BRA $00DB
    cpu.state_.pc = 0xE945;
    ...
    // E871: BRA $00DB
    cpu.state_.pc = 0xE94E;

    // E873: CMPB #$5A
    cpu.compare_b(0x5A);

    // E875: BNE $00CF
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE946;

    // E877: LDX #$3000
    cpu.state_.x = 0x3000;

    // E87A: BRA $00DB
    cpu.state_.pc = 0xE957;

    // E87C: CMPB #$4A
    cpu.compare_b(0x4A);

    // E87E: BNE $00D8
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE958;
    ...
    // E891: BPL $007E
    if (!cpu.negative_flag()) cpu.state_.pc = 0xE911;
    ...
    // E945: BRA $017C
    cpu.state_.pc = 0xEAC3;

    // E947: BNE $017E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xEAC7;
    ...
    // E970: ADDD $01A9,PCR
    // TODO: Fix comma operator: ADDD $01A9,PCR
    ...
    // E97A: ADDD [$01B3,PCR]
    // TODO: Complex indexed addressing: [$01B3,PCR]

    // E97D: LDD $E405
    cpu.state_.d = cpu.read_memory_word(0xE405);
