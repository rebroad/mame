...
void routine_e7ad_impl(CPU6809& cpu) {
    ...
    // E7B0: BNE $000E
    if (!cpu.zero_flag()) cpu.m_pc = 0xE7C0;

    // E7B2: TST <$AD
    // TODO: Convert TST <$AD

    // E7B4: BLE $000E
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xE7C4;
    ...
    // E7C9: BCC $0025
    if (!cpu.carry_flag()) cpu.m_pc = 0xE7F0;
    ...
    // E7D5: BCC $002F
    if (!cpu.carry_flag()) cpu.m_pc = 0xE806;
    ...
    // E833: BCS $008F
    if (cpu.carry_flag()) cpu.m_pc = 0xE7C4;
    ...
    // E83E: BNE $0098
    if (!cpu.zero_flag()) cpu.m_pc = 0xE7D8;
    ...
    // E847: BNE $00A2
    if (!cpu.zero_flag()) cpu.m_pc = 0xE7EB;
    ...
    // E851: BCS $00AB
    if (cpu.carry_flag()) cpu.m_pc = 0xE7FE;
    ...
    // E85A: BNE $00B4
    if (!cpu.zero_flag()) cpu.m_pc = 0xE810;
    ...
    // E863: BNE $00BD
    if (!cpu.zero_flag()) cpu.m_pc = 0xE822;
    ...
    // E875: BNE $00CF
    if (!cpu.zero_flag()) cpu.m_pc = 0xE846;
    ...
    // E87E: BNE $00D8
    if (!cpu.zero_flag()) cpu.m_pc = 0xE858;
    ...
    // E891: BPL $007E
    if (!cpu.negative_flag()) cpu.m_pc = 0xE911;
    ...
    // E947: BNE $017E
    if (!cpu.zero_flag()) cpu.m_pc = 0xE9C7;
