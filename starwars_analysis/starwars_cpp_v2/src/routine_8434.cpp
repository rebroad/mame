#include "cpu_6809.h"

namespace StarWars {

void routine_8434_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_8434.md
    // Address: 0x8434

    // 8434: LDU $49B3
    cpu.state_.u = cpu.read_memory_word(0x49B3);

    // 8437: LDB ,U
    // TODO: Complex indexed addressing: ,U

    // 8439: CMPB #$03
    cpu.compare_b(0x03);

    // 843B: BNE $0013
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8450;

    // 843D: LDD $49B5
    cpu.state_.d = cpu.read_memory_word(0x49B5);

    // 8440: STD <$93
    cpu.write_memory(0x93, cpu.state_.d);

    // 8442: LDA #$FF
    cpu.state_.a = 0xFF;

    // 8444: STA <$92
    cpu.write_memory(0x92, cpu.state_.a);

    // 8447: LDB ,U
    // TODO: Complex indexed addressing: ,U

    // 8449: CMPB #$04
    cpu.compare_b(0x04);

    // 844B: BNE $0036
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8483;

    // 844D: LDD $49B5
    cpu.state_.d = cpu.read_memory_word(0x49B5);

    // 8450: STD <$96
    cpu.write_memory(0x96, cpu.state_.d);

    // 8452: LDA #$FF
    cpu.state_.a = 0xFF;

    // 8454: STA <$95
    cpu.write_memory(0x95, cpu.state_.a);

    // 8456: LDA #$00
    cpu.state_.a = 0x00;

    // 8458: STA <$44
    cpu.write_memory(0x44, cpu.state_.a);

    // 845A: STA <$45
    cpu.write_memory(0x45, cpu.state_.a);

    // 845C: LDA $4B36
    cpu.state_.a = cpu.read_memory(0x4B36);

    // 845F: BNE $0036
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8497;

    // 8461: INC $4B36
    // TODO: Convert INC $4B36

    // 8464: JSR $97E3
    cpu.call_function(0x97E3);

    // 8467: LDU $49B3
    cpu.state_.u = cpu.read_memory_word(0x49B3);

    // 846A: LDB ,U
    // TODO: Complex indexed addressing: ,U

    // 846C: CMPB #$01
    cpu.compare_b(0x01);

    // 846E: BNE $0041
    if (!cpu.zero_flag()) cpu.state_.pc = 0x84B1;

    // 8470: LDD #$0800
    cpu.state_.d = 0x0800;

    // 8473: BRA $0044
    cpu.state_.pc = 0x84B9;

    // 8475: LDD #$1000
    cpu.state_.d = 0x1000;

    // 8478: ADDD $49B5
    cpu.state_.d += 0x49B5;

    // 847B: STD $49B7
    cpu.write_memory(0x49B7, cpu.state_.d);

    // 847E: LDB $49B5
    cpu.state_.b = cpu.read_memory(0x49B5);

    // 8484: ANDB #$0F
    cpu.state_.b &= 0x0F;

    // 8486: LDX #$4989
    cpu.state_.x = 0x4989;

    // 8489: LDA $1,U
    // TODO: Fix comma operator: LDA $1,U

    // 848B: STA B,X
    // TODO: Handle indexed addressing: STA B,X

    // 848D: LDX #$4999
    cpu.state_.x = 0x4999;

    // 8490: LDA $2,U
    // TODO: Fix comma operator: LDA $2,U

    // 8492: STA B,X
    // TODO: Handle indexed addressing: STA B,X

    // 8495: LDA <$45
    cpu.state_.a = cpu.read_memory(0x45);

    // 8497: BNE $0081
    if (!cpu.zero_flag()) cpu.state_.pc = 0x841A;

    // 8499: LDB $4B19
    cpu.state_.b = cpu.read_memory(0x4B19);

    // 849C: CMPB #$07
    cpu.compare_b(0x07);

    // 849E: BLS $006E
    // TODO: Convert BLS $006E

    // 84A0: LDB #$07
    cpu.state_.b = 0x07;

    // 84A3: LDU #$84B6
    cpu.state_.u = 0x84B6;

    // 84A6: LEAU B,U
    // TODO: Fix comma operator: LEAU B,U

    // 84A8: LDA <$43
    cpu.state_.a = cpu.read_memory(0x43);

    // 84AA: ANDA ,U
    // TODO: Complex indexed addressing: ,U

    // 84AC: BNE $0081
    if (!cpu.zero_flag()) cpu.state_.pc = 0x842F;

    // 84AE: LDA $1,U
    // TODO: Fix comma operator: LDA $1,U

    // 84B0: STA <$9B
    cpu.write_memory(0x9B, cpu.state_.a);

    // 84B2: JSR $84C6
    cpu.call_function(0x84C6);

    // 84B6: CLR <$80
    cpu.write_memory(0x80, 0);

    // 84B8: CLR <$60
    cpu.write_memory(0x60, 0);

    // 84BA: CLR <$40
    cpu.write_memory(0x40, 0);

    // 84BC: CLR <$20
    cpu.write_memory(0x20, 0);

    // 84BE: ASR <$60
    // TODO: Convert ASR <$60

    // 84C0: ASR <$20
    // TODO: Convert ASR <$20

    // 84C2: COM <$60
    // TODO: Convert COM <$60

    // 84C4: COM <$20
    // TODO: Convert COM <$20

    // 84C6: LDD #$F800
    cpu.state_.d = 0xF800;

    // 84C9: ANDA $5098
    cpu.state_.a &= 0x5098;

    // 84CC: ADDA #$24
    cpu.state_.a += 0x24;

    // 84CE: STD $5078
    cpu.write_memory(0x5078, cpu.state_.d);

    // 84D1: LDA $5078
    cpu.state_.a = cpu.read_memory(0x5078);

    // 84D7: ANDA #$0F
    cpu.state_.a &= 0x0F;

    // 84D9: STA <$99
    cpu.write_memory(0x99, cpu.state_.a);

    // 84DB: LDD #$FE00
    cpu.state_.d = 0xFE00;

    // 84DE: STD $507C
    cpu.write_memory(0x507C, cpu.state_.d);

    // 84E1: LDD #$FC80
    cpu.state_.d = 0xFC80;

    // 84E4: STD $507A
    cpu.write_memory(0x507A, cpu.state_.d);

    // 84E7: LDX #$4989
    cpu.state_.x = 0x4989;

    // 84EA: LDB <$99
    cpu.state_.b = cpu.read_memory(0x99);

    // 84EC: LDA B,X
    // TODO: Fix comma operator: LDA B,X

    // 84EE: STA <$9A
    cpu.write_memory(0x9A, cpu.state_.a);

    // 84F0: ANDA #$C0
    cpu.state_.a &= 0xC0;

    // 84F2: CMPA #$C0
    cpu.compare_a(0xC0);

    // 84F4: BNE $00EE
    if (!cpu.zero_flag()) cpu.state_.pc = 0x84E4;

    // 84F6: LDD $509C
    cpu.state_.d = cpu.read_memory_word(0x509C);

    // 84F9: SUBD $507C
    cpu.state_.d -= 0x507C;

    // 84FC: BLT $00EE
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x84EC;

    // 84FE: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 8501: BGE $00DB
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x84DE;

    // 8503: LDA $4703
    cpu.state_.a = cpu.read_memory(0x4703);

    // 8506: CMPA <$9B
    cpu.compare_a(cpu.read_memory(0x9B));

    // 8508: BCS $00D9
    if (cpu.carry_flag()) cpu.state_.pc = 0x84E3;

    // 850A: JSR $A7F7
    cpu.call_function(0xA7F7);

    // 850D: BRA $00EE
    cpu.state_.pc = 0x84FD;

    // 850F: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 8512: BGE $00EE
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8502;

    // 8514: LDA $4703
    cpu.state_.a = cpu.read_memory(0x4703);

    // 8517: LDB $4703
    cpu.state_.b = cpu.read_memory(0x4703);

    // 851B: CMPA <$9B
    cpu.compare_a(cpu.read_memory(0x9B));

    // 851D: BCS $00EE
    if (cpu.carry_flag()) cpu.state_.pc = 0x850D;

    // 851F: JSR $A7F7
    cpu.call_function(0xA7F7);

    // 8522: LDD $507C
    cpu.state_.d = cpu.read_memory_word(0x507C);

    // 8525: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 8528: STD $507C
    cpu.write_memory(0x507C, cpu.state_.d);

    // 852B: LDA <$9A
    cpu.state_.a = cpu.read_memory(0x9A);

    // 852F: BNE $00BA
    if (!cpu.zero_flag()) cpu.state_.pc = 0x84EB;

    // 8531: LDD #$FE00
    cpu.state_.d = 0xFE00;

    // 8534: STD $507C
    cpu.write_memory(0x507C, cpu.state_.d);

    // 8537: LDD #$0380
    cpu.state_.d = 0x0380;

    // 853A: STD $507A
    cpu.write_memory(0x507A, cpu.state_.d);

    // 853D: LDX #$4999
    cpu.state_.x = 0x4999;

    // 8540: LDB <$99
    cpu.state_.b = cpu.read_memory(0x99);

    // 8542: LDA B,X
    // TODO: Fix comma operator: LDA B,X

    // 8544: STA <$9A
    cpu.write_memory(0x9A, cpu.state_.a);

    // 8546: ANDA #$C0
    cpu.state_.a &= 0xC0;

    // 8548: CMPA #$C0
    cpu.compare_a(0xC0);

    // 854A: BNE $0144
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8590;

    // 854C: LDD $509C
    cpu.state_.d = cpu.read_memory_word(0x509C);

    // 854F: SUBD $507C
    cpu.state_.d -= 0x507C;

    // 8552: BLT $0144
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8598;

    // 8554: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 8557: BGE $0131
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x858A;

    // 8559: LDA $4703
    cpu.state_.a = cpu.read_memory(0x4703);

    // 855C: CMPA <$9B
    cpu.compare_a(cpu.read_memory(0x9B));

    // 855E: BCS $012F
    if (cpu.carry_flag()) cpu.state_.pc = 0x858F;

    // 8560: JSR $A80B
    cpu.call_function(0xA80B);

    // 8563: BRA $0144
    cpu.state_.pc = 0x85A9;

    // 8565: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 8568: BGE $0144
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x85AE;

    // 856A: LDA $4703
    cpu.state_.a = cpu.read_memory(0x4703);

    // 856D: LDB $4703
    cpu.state_.b = cpu.read_memory(0x4703);

    // 8571: CMPA <$9B
    cpu.compare_a(cpu.read_memory(0x9B));

    // 8573: BCS $0144
    if (cpu.carry_flag()) cpu.state_.pc = 0x85B9;

    // 8575: JSR $A80B
    cpu.call_function(0xA80B);

    // 8578: LDD $507C
    cpu.state_.d = cpu.read_memory_word(0x507C);

    // 857B: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 857E: STD $507C
    cpu.write_memory(0x507C, cpu.state_.d);

    // 8581: LDA <$9A
    cpu.state_.a = cpu.read_memory(0x9A);

    // 8585: BNE $0110
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8597;

    // 8587: LDD $5078
    cpu.state_.d = cpu.read_memory_word(0x5078);

    // 858A: ADDD #$0800
    cpu.state_.d += 0x0800;

    // 858D: STD $5078
    cpu.write_memory(0x5078, cpu.state_.d);

    // 8590: SUBD $5098
    cpu.state_.d -= 0x5098;

    // 8593: SUBD #$6000
    cpu.state_.d -= 0x6000;

    // 8596: LBCS $009D
    // TODO: Convert LBCS $009D

    // 859B: LDA $5040
    cpu.state_.a = cpu.read_memory(0x5040);

    // 85A1: ANDA #$0F
    cpu.state_.a &= 0x0F;

    // 85A3: CMPA $49BF
    cpu.compare_a(cpu.read_memory(0x49BF));

    // 85A6: BEQ $01AA
    if (cpu.zero_flag()) cpu.state_.pc = 0x8552;

    // 85A8: LDB $49BF
    cpu.state_.b = cpu.read_memory(0x49BF);

    // 85AB: STA $49BF
    cpu.write_memory(0x49BF, cpu.state_.a);

    // 85AE: LDX #$4989
    cpu.state_.x = 0x4989;

    // 85B1: LDA B,X
    // TODO: Fix comma operator: LDA B,X

    // 85B4: BCC $0189
    if (!cpu.carry_flag()) cpu.state_.pc = 0x853F;

    // 85B6: BMI $0189
    if (cpu.negative_flag()) cpu.state_.pc = 0x8541;

    // 85B8: INC $49C0
    // TODO: Convert INC $49C0

    // 85BB: BRA $019E
    cpu.state_.pc = 0x855B;

    // 85BE: BNE $017F
    if (!cpu.zero_flag()) cpu.state_.pc = 0x863F;

    // 85C0: LDX #$4999
    cpu.state_.x = 0x4999;

    // 85C3: LDA B,X
    // TODO: Fix comma operator: LDA B,X

    // 85C6: BCC $019B
    if (!cpu.carry_flag()) cpu.state_.pc = 0x8563;

    // 85C8: BMI $019B
    if (cpu.negative_flag()) cpu.state_.pc = 0x8565;

    // 85CA: INC $49C0
    // TODO: Convert INC $49C0

    // 85CD: BRA $019E
    cpu.state_.pc = 0x856D;

    // 85D0: BNE $0191
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8563;

    // 85D2: LDA #$00
    cpu.state_.a = 0x00;

    // 85D4: LDX #$4989
    cpu.state_.x = 0x4989;

    // 85D7: STA B,X
    // TODO: Handle indexed addressing: STA B,X

    // 85D9: LDX #$4999
    cpu.state_.x = 0x4999;

    // 85DC: STA B,X
    // TODO: Handle indexed addressing: STA B,X

    // 85DE: JSR $B3E4
    cpu.call_function(0xB3E4);

    // 85E1: JSR $85F9
    cpu.call_function(0x85F9);

    // 85E4: JSR $8735
    cpu.call_function(0x8735);

    // 85E7: JSR $86AE
    cpu.call_function(0x86AE);

    // 85EA: LDA <$95
    cpu.state_.a = cpu.read_memory(0x95);

    // 85EC: BEQ $01BD
    if (cpu.zero_flag()) cpu.state_.pc = 0x85AB;

    // 85EE: JSR $889F
    cpu.call_function(0x889F);

    // 85F1: LDA <$92
    cpu.state_.a = cpu.read_memory(0x92);

    // 85F3: BEQ $01C4
    if (cpu.zero_flag()) cpu.state_.pc = 0x85B9;

    // 85F5: JSR $88F5
    cpu.call_function(0x88F5);

    // 85F9: LDD #$0000
    cpu.state_.d = 0x0000;

    // 85FC: STD $5040
    cpu.write_memory(0x5040, cpu.state_.d);

    // 85FF: LDD #$6270
    cpu.state_.d = 0x6270;

    // 8602: STD ,Y++
    cpu.write_memory(cpu.state_.y++, cpu.state_.d);

    // 8604: LDU #$8696
    cpu.state_.u = 0x8696;

    // 8607: LDA <$92
    cpu.state_.a = cpu.read_memory(0x92);

    // 8609: BEQ $01E4
    if (cpu.zero_flag()) cpu.state_.pc = 0x85EF;

    // 860B: LDD <$93
    cpu.state_.d = cpu.read_memory_word(0x93);

    // 860D: SUBD $5098
    cpu.state_.d -= 0x5098;

    // 8610: CMPD #$7000
    // TODO: Convert CMPD #$7000

    // 8614: BHI $01E4
    // TODO: Convert BHI $01E4

    // 8616: BRA $01E7
    cpu.state_.pc = 0x85FF;

    // 8618: LDD #$7000
    cpu.state_.d = 0x7000;

    // 861B: STD $5078
    cpu.write_memory(0x5078, cpu.state_.d);

    // 861E: LDD ,U
    // TODO: Complex indexed addressing: ,U

    // 8620: STD $507A
    cpu.write_memory(0x507A, cpu.state_.d);

    // 8623: LDD $2,U
    // TODO: Fix comma operator: LDD $2,U

    // 8625: STD $507C
    cpu.write_memory(0x507C, cpu.state_.d);

    // 8628: LDD #$000F
    cpu.state_.d = 0x000F;

    // 862B: STD $4701
    cpu.write_memory(0x4701, cpu.state_.d);

    // 862E: LDA #$67
    cpu.state_.a = 0x67;

    // 8630: JSR $CDBA
    cpu.call_function(0xCDBA);

    // 8633: LDD >$0000
    cpu.state_.d = cpu.read_memory_word(0x0000);

}

} // namespace StarWars