...
void routine_8434_impl(StarWarsCPU& cpu) {
    ...
    // 843B: BNE $0013
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0013;
    ...
    // 846E: BNE $0041
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0041;
    ...
    // 8497: BNE $0081
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0081;
    ...
    // 84F4: BNE $00EE
    if (!cpu.zero_flag()) cpu.state_.pc += 0x00EE;
    ...
    // 84FC: BLT $00EE
    // TODO: Convert BLT $00EE

    // 84FE: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 8501: BGE $00DB
    // TODO: Convert BGE $00DB
    ...
    // 850D: BRA $00EE
    cpu.state_.pc += 0x00EE;

    // 850F: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 8512: BGE $00EE
    // TODO: Convert BGE $00EE
    ...
    // 852F: BNE $00BA
    if (!cpu.zero_flag()) cpu.state_.pc += 0x00BA;
    ...
    // 8552: BLT $0144
    // TODO: Convert BLT $0144

    // 8554: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 8557: BGE $0131
    // TODO: Convert BGE $0131
    ...
    // 855E: BCS $012F
    if (cpu.carry_flag()) cpu.state_.pc += 0x012F;

    // 8560: JSR $A80B
    cpu.call_function(0xA80B);

    // 8563: BRA $0144
    cpu.state_.pc += 0x0144;

    // 8565: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // 8568: BGE $0144
    // TODO: Convert BGE $0144
    ...
    // 8573: BCS $0144
    if (cpu.carry_flag()) cpu.state_.pc += 0x0144;
    ...
    // 8585: BNE $0110
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0110;
    ...
    // 85B4: BCC $0189
    if (!cpu.carry_flag()) cpu.state_.pc += 0x0189;

    // 85B6: BMI $0189
    if (cpu.negative_flag()) cpu.state_.pc += 0x0189;
    ...
    // 85C8: BMI $019B
    if (cpu.negative_flag()) cpu.state_.pc += 0x019B;
    ...
    // 85D0: BNE $0191
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0191;
    ...
    // 85F3: BEQ $01C4
    if (cpu.zero_flag()) cpu.state_.pc += 0x01C4;
