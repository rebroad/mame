...
void routine_d8df_impl(CPU6809& cpu) {
    ...
    // D8E5: BCC $0012
    if (!cpu.carry_flag()) cpu.m_pc = 0xD8F9;

    // D8E7: CMPA ,X+
    // TODO: Complex indexed addressing: ,X+

    // D8E9: BNE $000D
    if (!cpu.zero_flag()) cpu.m_pc = 0xD8F8;
    ...
    // D8FC: BNE $0033
    if (!cpu.zero_flag()) cpu.m_pc = 0xD931;
    ...
    // D917: BCS $001B
    if (cpu.carry_flag()) cpu.m_pc = 0xD934;
    ...
    // D949: BCC $007E
    if (!cpu.carry_flag()) cpu.m_pc = 0xD9C9;
    ...
    // D976: BNE $009D
    if (!cpu.zero_flag()) cpu.m_pc = 0xD915;
    ...
    // D982: BCS $0095
    if (cpu.carry_flag()) cpu.m_pc = 0xD919;
    ...
    // D98C: BCC $00FC
    if (!cpu.carry_flag()) cpu.m_pc = 0xD98A;
    ...
    // D995: BEQ $00F4
    if (cpu.zero_flag()) cpu.m_pc = 0xD98B;
    ...
    // D9D9: BCS $00B4
    if (cpu.carry_flag()) cpu.m_pc = 0xD98F;
    ...
    // DA0F: BCC $013F
    if (!cpu.carry_flag()) cpu.m_pc = 0xDA50;
    ...
    // DAA9: BCS $01D2
    if (cpu.carry_flag()) cpu.m_pc = 0xDA7D;
    ...
    // DAB7: BCC $01E2
    if (!cpu.carry_flag()) cpu.m_pc = 0xDA9B;
    ...
    // DACB: BCS $020F
    if (cpu.carry_flag()) cpu.m_pc = 0xDADC;
    ...
    // DAD5: BNE $0200
    if (!cpu.zero_flag()) cpu.m_pc = 0xDAD7;
