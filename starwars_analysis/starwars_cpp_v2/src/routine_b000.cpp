...
void routine_b000_impl(CPU6809& cpu) {
    ...
    // B006: BGE $0044
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xB04C;
    ...
    // B019: BEQ $0020
    if (cpu.zero_flag()) cpu.m_pc = 0xB03B;
    ...
    // B025: BEQ $002C
    if (cpu.zero_flag()) cpu.m_pc = 0xB053;
    ...
    // B031: BEQ $0038
    if (cpu.zero_flag()) cpu.m_pc = 0xB06B;
    ...
    // B03D: BEQ $0044
    if (cpu.zero_flag()) cpu.m_pc = 0xB083;
    ...
    // B073: BEQ $0094
    if (cpu.zero_flag()) cpu.m_pc = 0xB009;
    ...
    // B080: BMI $0089
    if (cpu.negative_flag()) cpu.m_pc = 0xB00B;
    ...
    // B08B: BLE $0094
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xB021;
    ...
    // B0EF: BMI $00F9
    if (cpu.negative_flag()) cpu.m_pc = 0xB0EA;

    // B0F1: CMPD #$0400
    // TODO: Convert CMPD #$0400

    // B0F5: BGT $0158
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xB14F;
    ...
    // B124: BCS $0128
    if (cpu.carry_flag()) cpu.m_pc = 0xB14E;
    ...
    // B133: BEQ $0157
    if (cpu.zero_flag()) cpu.m_pc = 0xB18C;

    // B135: LDA <$44
    cpu.m_a = cpu.read_memory(0x44);

    // B137: BNE $0157
    if (!cpu.zero_flag()) cpu.m_pc = 0xB190;
    ...
    // B13E: BLT $0157
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xB197;

    // B140: SUBD #$0400
    cpu.m_d -= 0x0400;

    // B143: BGT $0157
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xB19C;
    ...
    // B14C: BLT $0157
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xB1A5;

    // B14E: SUBD #$0400
    cpu.m_d -= 0x0400;

    // B151: BGT $0157
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xB1AA;
    ...
    // B192: BMI $019E
    if (cpu.negative_flag()) cpu.m_pc = 0xB132;
    ...
    // B1D0: BCS $01D4
    if (cpu.carry_flag()) cpu.m_pc = 0xB1A6;
    ...
    // B1E6: BLT $0260
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xB248;
