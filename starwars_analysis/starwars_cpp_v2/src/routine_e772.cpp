...
void routine_e772_impl(StarWarsCPU& cpu) {
    ...
    // E797: BNE $0030
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0030;

    // E799: TST <$AD
    // TODO: Convert TST <$AD

    // E79B: BLE $0030
    // TODO: Convert BLE $0030

    // E79D: LDU $3002
    cpu.state_.u = cpu.read_memory_word(0x3002);

    // E7A0: BRA $0037
    cpu.state_.pc += 0x0037;
    ...
    // E7C9: BCC $0060
    if (!cpu.carry_flag()) cpu.state_.pc += 0x0060;
    ...
    // E833: BCS $00CA
    if (cpu.carry_flag()) cpu.state_.pc += 0x00CA;
    ...
    // E851: BCS $00E6
    if (cpu.carry_flag()) cpu.state_.pc += 0x00E6;
    ...
    // E86C: BNE $0101
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0101;
    ...
    // E87E: BNE $0113
    if (!cpu.zero_flag()) cpu.state_.pc += 0x0113;
