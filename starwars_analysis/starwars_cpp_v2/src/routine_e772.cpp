...
void routine_e772_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_e772.md
    // Address: 0xE772
    ...
    // E782: BGT $0017
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xE79B;
    ...
    // E797: BNE $0030
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE7C9;

    // E799: TST <$AD
    // TODO: Convert TST <$AD

    // E79B: BLE $0030
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xE7CD;

    // E79D: LDU $3002
    cpu.state_.u = cpu.read_memory_word(0x3002);

    // E7A0: BRA $0037
    cpu.state_.pc = 0xE7D9;
    ...
    // E7B0: BNE $0049
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE7FB;

    // E7B2: TST <$AD
    // TODO: Convert TST <$AD

    // E7B4: BLE $0049
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xE7FF;

    // E7B6: LDU $3002
    cpu.state_.u = cpu.read_memory_word(0x3002);

    // E7B9: BRA $0050
    cpu.state_.pc = 0xE80B;
    ...
    // E7C9: BCC $0060
    if (!cpu.carry_flag()) cpu.state_.pc = 0xE82B;

    // E7CB: STB $48AE
    cpu.write_memory(0x48AE, cpu.state_.b);

    // E7CE: BSR $006B
    // TODO: Convert BSR $006B

    // E7D0: BSR $0078
    // TODO: Convert BSR $0078

    // E7D3: CMPB #$D6
    cpu.compare_b(0xD6);

    // E7D5: BCC $006A
    if (!cpu.carry_flag()) cpu.state_.pc = 0xE841;
    ...
    // E833: BCS $00CA
    if (cpu.carry_flag()) cpu.state_.pc = 0xE8FF;
    ...
    // E83A: BRA $0116
    cpu.state_.pc = 0xE952;

    // E83C: CMPB #$74
    cpu.compare_b(0x74);

    // E83E: BNE $00D3
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE913;

    // E840: LDX #$2FDE
    cpu.state_.x = 0x2FDE;

    // E843: BRA $0116
    cpu.state_.pc = 0xE95B;

    // E845: CMPB #$80
    cpu.compare_b(0x80);

    // E847: BNE $00DD
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE926;

    // E849: LDX #$3058
    cpu.state_.x = 0x3058;

    // E84D: BRA $0116
    cpu.state_.pc = 0xE965;

    // E84F: CMPB #$60
    cpu.compare_b(0x60);

    // E851: BCS $00E6
    if (cpu.carry_flag()) cpu.state_.pc = 0xE939;

    // E853: LDX #$2FA4
    cpu.state_.x = 0x2FA4;

    // E856: BRA $0116
    cpu.state_.pc = 0xE96E;

    // E858: CMPB #$40
    cpu.compare_b(0x40);

    // E85A: BNE $00EF
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE94B;

    // E85C: LDX #$2FC2
    cpu.state_.x = 0x2FC2;

    // E85F: BRA $0116
    cpu.state_.pc = 0xE977;

    // E861: CMPB #$4E
    cpu.compare_b(0x4E);

    // E863: BNE $00F8
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE95D;

    // E865: LDX #$2FFE
    cpu.state_.x = 0x2FFE;

    // E868: BRA $0116
    cpu.state_.pc = 0xE980;

    // E86A: CMPB #$58
    cpu.compare_b(0x58);

    // E86C: BNE $0101
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE96F;

    // E86E: LDX #$2FF6
    cpu.state_.x = 0x2FF6;

    // E871: BRA $0116
    cpu.state_.pc = 0xE989;

    // E873: CMPB #$5A
    cpu.compare_b(0x5A);

    // E875: BNE $010A
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE981;

    // E877: LDX #$3000
    cpu.state_.x = 0x3000;

    // E87A: BRA $0116
    cpu.state_.pc = 0xE992;

    // E87C: CMPB #$4A
    cpu.compare_b(0x4A);

    // E87E: BNE $0113
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE993;

    // E880: LDX #$300C
    cpu.state_.x = 0x300C;

    // E883: BRA $0116
    cpu.state_.pc = 0xE99B;
    ...
    // E891: BPL $00B9
    if (!cpu.negative_flag()) cpu.state_.pc = 0xE94C;
    ...
    // E902: STU <$78
    cpu.write_memory(0x78, cpu.state_.u);

    // E904: STU <$80
    cpu.write_memory(0x80, cpu.state_.u);

    // E906: STU <$87
    cpu.write_memory(0x87, cpu.state_.u);

    // E908: STU <$8E
    cpu.write_memory(0x8E, cpu.state_.u);

    // E90A: STU <$AD
    cpu.write_memory(0xAD, cpu.state_.u);

    // E90C: STU <$C7
    cpu.write_memory(0xC7, cpu.state_.u);

    // E90E: STU <$DC
    cpu.write_memory(0xDC, cpu.state_.u);

    // E910: STU <$EF
    cpu.write_memory(0xEF, cpu.state_.u);
    ...
    // E945: BRA $01B7
    cpu.state_.pc = 0xEAFE;

    // E947: BNE $01B9
    if (!cpu.zero_flag()) cpu.state_.pc = 0xEB02;
    ...
    // E970: ADDD $0201,PCR
    // TODO: Fix comma operator: ADDD $0201,PCR
