...
void routine_e772_impl(StarWarsCPU& cpu) {
    ...
    // E833: BCS $00CA
    if (cpu.carry_flag()) cpu.state_.pc = 0xE7FF;
    ...
    // E83A: BRA $0116
    cpu.state_.pc = 0xE852;

    // E83C: CMPB #$74
    cpu.compare_b(0x74);

    // E83E: BNE $00D3
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE813;

    // E840: LDX #$2FDE
    cpu.state_.x = 0x2FDE;

    // E843: BRA $0116
    cpu.state_.pc = 0xE85B;

    // E845: CMPB #$80
    cpu.compare_b(0x80);

    // E847: BNE $00DD
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE826;

    // E849: LDX #$3058
    cpu.state_.x = 0x3058;

    // E84D: BRA $0116
    cpu.state_.pc = 0xE865;

    // E84F: CMPB #$60
    cpu.compare_b(0x60);

    // E851: BCS $00E6
    if (cpu.carry_flag()) cpu.state_.pc = 0xE839;

    // E853: LDX #$2FA4
    cpu.state_.x = 0x2FA4;

    // E856: BRA $0116
    cpu.state_.pc = 0xE86E;

    // E858: CMPB #$40
    cpu.compare_b(0x40);

    // E85A: BNE $00EF
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE84B;

    // E85C: LDX #$2FC2
    cpu.state_.x = 0x2FC2;

    // E85F: BRA $0116
    cpu.state_.pc = 0xE877;

    // E861: CMPB #$4E
    cpu.compare_b(0x4E);

    // E863: BNE $00F8
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE85D;

    // E865: LDX #$2FFE
    cpu.state_.x = 0x2FFE;

    // E868: BRA $0116
    cpu.state_.pc = 0xE880;

    // E86A: CMPB #$58
    cpu.compare_b(0x58);

    // E86C: BNE $0101
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE86F;

    // E86E: LDX #$2FF6
    cpu.state_.x = 0x2FF6;

    // E871: BRA $0116
    cpu.state_.pc = 0xE889;

    // E873: CMPB #$5A
    cpu.compare_b(0x5A);

    // E875: BNE $010A
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE881;

    // E877: LDX #$3000
    cpu.state_.x = 0x3000;

    // E87A: BRA $0116
    cpu.state_.pc = 0xE892;

    // E87C: CMPB #$4A
    cpu.compare_b(0x4A);

    // E87E: BNE $0113
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE893;
    ...
    // E891: BPL $00B9
    if (!cpu.negative_flag()) cpu.state_.pc = 0xE84C;
    ...
    // E945: BRA $01B7
    cpu.state_.pc = 0xE8FE;

    // E947: BNE $01B9
    if (!cpu.zero_flag()) cpu.state_.pc = 0xE902;
