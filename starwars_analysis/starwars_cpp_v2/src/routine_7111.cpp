...
void routine_7111_impl(StarWarsCPU& cpu) {
    ...
    // 7113: BPL $0005
    if (!cpu.negative_flag()) cpu.state_.pc = 0x711A;

    // 7116: CMPA #$4E
    cpu.compare_a(0x4E);

    // 7118: BCS $0029
    if (cpu.carry_flag()) cpu.state_.pc = 0x7143;
    ...
    // 7122: BPL $001E
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7142;
    ...
    // 7146: BPL $0042
    if (!cpu.negative_flag()) cpu.state_.pc = 0x718A;
    ...
    // 7163: BEQ $0070
    if (cpu.zero_flag()) cpu.state_.pc = 0x71D5;
    ...
    // 716B: BPL $0065
    if (!cpu.negative_flag()) cpu.state_.pc = 0x71D2;
    ...
    // 7184: BEQ $0091
    if (cpu.zero_flag()) cpu.state_.pc = 0x7117;
    ...
    // 718C: BPL $0086
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7114;
    ...
    // 71A5: BEQ $00B2
    if (cpu.zero_flag()) cpu.state_.pc = 0x7159;
    ...
    // 71AD: BPL $00A7
    if (!cpu.negative_flag()) cpu.state_.pc = 0x7156;
    ...
    // 71C4: BPL $00B6
    if (!cpu.negative_flag()) cpu.state_.pc = 0x717C;
    ...
    // 71E1: BCS $00D2
    if (cpu.carry_flag()) cpu.state_.pc = 0x71B5;
    ...
    // 725F: BCC $013C
    if (!cpu.carry_flag()) cpu.state_.pc = 0x729D;
    ...
    // 72D8: BNE $01DF
    if (!cpu.zero_flag()) cpu.state_.pc = 0x72B9;
    ...
    // 72E1: BCC $01DF
    if (!cpu.carry_flag()) cpu.state_.pc = 0x72C2;

    // 72E3: ANDB #$10
    cpu.state_.b &= 0x10;

    // 72E5: BNE $01DA
    if (!cpu.zero_flag()) cpu.state_.pc = 0x72C1;

    // 72E7: LDB #$4C
    cpu.state_.b = 0x4C;

    // 72E9: BRA $01DC
    cpu.state_.pc = 0x72C7;
