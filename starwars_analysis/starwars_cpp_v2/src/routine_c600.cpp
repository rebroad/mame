...
void routine_c600_impl(StarWarsCPU& cpu) {
    ...
    // C68D: BNE $008B
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC61A;

    // C690: LDA $4AF7
    cpu.state_.a = cpu.read_memory(0x4AF7);

    // C693: BEQ $00B7
    if (cpu.zero_flag()) cpu.state_.pc = 0xC64C;

    // C695: CMPA #$01
    cpu.compare_a(0x01);

    // C697: BNE $009D
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC636;

    // C699: LDB #$9C
    cpu.state_.b = 0x9C;

    // C69B: BRA $00B4
    cpu.state_.pc = 0xC651;
    ...
    // C6EF: BPL $00DC
    if (!cpu.negative_flag()) cpu.state_.pc = 0xC6CD;
    ...
    // C709: BPL $00FC
    if (!cpu.negative_flag()) cpu.state_.pc = 0xC707;
    ...
    // C71A: BCS $011E
    if (cpu.carry_flag()) cpu.state_.pc = 0xC73A;
    ...
    // C72C: BEQ $0134
    if (cpu.zero_flag()) cpu.state_.pc = 0xC762;
