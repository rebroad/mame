...
void routine_a7b6_impl(CPU6809& cpu) {
    ...
    // A7F9: BEQ $004A
    if (cpu.zero_flag()) cpu.m_pc = 0xA845;
    ...
    // A80D: BEQ $005E
    if (cpu.zero_flag()) cpu.m_pc = 0xA86D;
    ...
    // A83F: BLT $008D
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA7CE;
    ...
    // A850: BEQ $00AB
    if (cpu.zero_flag()) cpu.m_pc = 0xA7FD;
    ...
    // A857: BCC $00AA
    if (!cpu.carry_flag()) cpu.m_pc = 0xA803;
    ...
    // A868: BCS $0096
    if (cpu.carry_flag()) cpu.m_pc = 0xA800;

    // A86B: DEC $5,X
    // TODO: Convert DEC $5,X

    // A86D: BGT $00BE
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA82D;
    ...
    // A877: BGT $00CA
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA843;
    ...
    // A8A9: BGT $00FC
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA8A7;
    ...
    // A911: BMI $0169
    if (cpu.negative_flag()) cpu.m_pc = 0xA97C;
    ...
    // A92C: BPL $017C
    if (!cpu.negative_flag()) cpu.m_pc = 0xA9AA;
    ...
    // A93F: BMI $018F
    if (cpu.negative_flag()) cpu.m_pc = 0xA8D0;
    ...
    // A958: BNE $01A8
    if (!cpu.zero_flag()) cpu.m_pc = 0xA902;
    ...
    // A966: BMI $01C2
    if (cpu.negative_flag()) cpu.m_pc = 0xA92A;
    ...
    // A96F: BLE $01BE
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA92F;
    ...
    // A97F: BGE $01CE
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA94F;
    ...
    // A990: BMI $01EC
    if (cpu.negative_flag()) cpu.m_pc = 0xA97E;
    ...
    // A999: BLE $01E8
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA983;
    ...
    // A9B5: BEQ $0201
    if (cpu.zero_flag()) cpu.m_pc = 0xA9B8;
