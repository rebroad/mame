...
void routine_8979_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_8979.md
    // Address: 0x8979
    ...
    // 8AD9: BEQ $0163
    if (cpu.zero_flag()) cpu.state_.pc = 0x8B3E;
    ...
    // 8B30: BCS $01BA
    if (cpu.carry_flag()) cpu.state_.pc = 0x8AEC;
    ...
    // 8B42: BCS $01CC
    if (cpu.carry_flag()) cpu.state_.pc = 0x8B10;
    ...
    // 8B51: BPL $019F
    if (!cpu.negative_flag()) cpu.state_.pc = 0x8AF2;
    ...
    // 8B61: BNE $01ED
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8B50;
