...
void routine_8c44_impl(CPU6809& cpu) {
    ...
    // 8C66: BEQ $003D
    if (cpu.zero_flag()) cpu.m_pc = 0x8CA5;
    ...
    // 8C89: BMI $006A
    if (cpu.negative_flag()) cpu.m_pc = 0x8CF5;
    ...
    // 8C90: BGE $006A
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8CFC;
    ...
    // 8CB3: BEQ $00C1
    if (cpu.zero_flag()) cpu.m_pc = 0x8C76;
    ...
    // 8CBA: BNE $00C1
    if (!cpu.zero_flag()) cpu.m_pc = 0x8C7D;
    ...
    // 8CC2: BLE $00C1
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8C85;

    // 8CC4: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 8CC6: BNE $00C1
    if (!cpu.zero_flag()) cpu.m_pc = 0x8C89;
    ...
    // 8CCD: BCS $0090
    if (cpu.carry_flag()) cpu.m_pc = 0x8C5F;
    ...
    // 8CDF: BNE $00C1
    if (!cpu.zero_flag()) cpu.m_pc = 0x8CA2;
    ...
    // 8CEC: BNE $00B9
    if (!cpu.zero_flag()) cpu.m_pc = 0x8CA7;
    ...
    // 8D03: BCS $00A6
    if (cpu.carry_flag()) cpu.m_pc = 0x8CAB;
    ...
    // 8D0C: BEQ $0122
    if (cpu.zero_flag()) cpu.m_pc = 0x8D30;

    // 8D0E: LDA <$4F
    cpu.m_a = cpu.read_memory(0x4F);

    // 8D10: BNE $00F6
    if (!cpu.zero_flag()) cpu.m_pc = 0x8D08;

    // 8D12: LDA $5002
    cpu.m_a = cpu.read_memory(0x5002);

    // 8D15: BMI $00D8
    if (cpu.negative_flag()) cpu.m_pc = 0x8CEF;
    ...
    // 8D21: BNE $00F6
    if (!cpu.zero_flag()) cpu.m_pc = 0x8D19;
    ...
    // 8D30: BMI $00F3
    if (cpu.negative_flag()) cpu.m_pc = 0x8D25;
    ...
    // 8D3C: BNE $0122
    if (!cpu.zero_flag()) cpu.m_pc = 0x8D60;

    // 8D3E: LDA $5004
    cpu.m_a = cpu.read_memory(0x5004);

    // 8D41: BMI $0104
    if (cpu.negative_flag()) cpu.m_pc = 0x8D47;
    ...
    // 8D4D: BNE $0122
    if (!cpu.zero_flag()) cpu.m_pc = 0x8D71;
    ...
    // 8D5C: BMI $011F
    if (cpu.negative_flag()) cpu.m_pc = 0x8D7D;
    ...
    // 8DA3: BNE $019B
    if (!cpu.zero_flag()) cpu.m_pc = 0x8D40;
    ...
    // 8DB7: BCC $0178
    if (!cpu.carry_flag()) cpu.m_pc = 0x8E31;
    ...
    // 8DBE: BCC $017F
    if (!cpu.carry_flag()) cpu.m_pc = 0x8E3F;
    ...
    // 8DC5: BCC $0186
    if (!cpu.carry_flag()) cpu.m_pc = 0x8D4D;
    ...
    // 8DCC: BCC $018D
    if (!cpu.carry_flag()) cpu.m_pc = 0x8D5B;
    ...
    // 8DD3: BCC $0194
    if (!cpu.carry_flag()) cpu.m_pc = 0x8D69;
    ...
    // 8DDA: BCC $019B
    if (!cpu.carry_flag()) cpu.m_pc = 0x8D77;
    ...
    // 8DE9: BLT $01AA
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8D95;
    ...
    // 8DF0: BGT $01B1
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8DA3;
    ...
    // 8DFB: BLT $01BC
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8DB9;
    ...
    // 8E02: BGT $01C3
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8DC7;
    ...
    // 8E0D: BLT $01CE
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x8DDD;
    ...
    // 8E14: BGT $01D5
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8DEB;
    ...
    // 8E1E: BGT $01DE
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x8DFE;
    ...
    // 8E38: BEQ $020D
    if (cpu.zero_flag()) cpu.m_pc = 0x8E47;
    ...
    // 8E40: BNE $0218
    if (!cpu.zero_flag()) cpu.m_pc = 0x8E5A;
