...
void routine_a459_impl(CPU6809& cpu) {
    ...
    // A48F: BEQ $0046
    if (cpu.zero_flag()) cpu.m_pc = 0xA4D7;
    ...
    // A4C0: BPL $006C
    if (!cpu.negative_flag()) cpu.m_pc = 0xA52E;
    ...
    // A4EF: BMI $00F1
    if (cpu.negative_flag()) cpu.m_pc = 0xA4E2;

    // A4F1: SUBD $5004
    cpu.m_d -= 0x5004;

    // A4F4: BGT $00F1
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA4E7;
    ...
    // A4FA: BNE $00B1
    if (!cpu.zero_flag()) cpu.m_pc = 0xA4AD;
    ...
    // A502: BCC $00AF
    if (!cpu.carry_flag()) cpu.m_pc = 0xA4B3;
    ...
    // A521: BLT $00E5
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA508;
    ...
    // A52E: BNE $00F1
    if (!cpu.zero_flag()) cpu.m_pc = 0xA521;
    ...
    // A536: BCC $00E3
    if (!cpu.carry_flag()) cpu.m_pc = 0xA51B;
    ...
    // A544: BCC $00F1
    if (!cpu.carry_flag()) cpu.m_pc = 0xA537;
    ...
    // A570: BNE $0127
    if (!cpu.zero_flag()) cpu.m_pc = 0xA599;
    ...
    // A593: BLT $0159
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA5EE;
    ...
    // A5A2: BNE $0159
    if (!cpu.zero_flag()) cpu.m_pc = 0xA5FD;
    ...
    // A5D5: BGE $0185
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA55C;
    ...
    // A5E1: BLE $0190
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA573;
    ...
    // A5E7: BEQ $01AE
    if (cpu.zero_flag()) cpu.m_pc = 0xA597;
    ...
    // A5F2: BPL $019E
    if (!cpu.negative_flag()) cpu.m_pc = 0xA592;
    ...
    // A5FA: BPL $01A6
    if (!cpu.negative_flag()) cpu.m_pc = 0xA5A2;
    ...
    // A602: BPL $01AE
    if (!cpu.negative_flag()) cpu.m_pc = 0xA5B2;
    ...
    // A62A: BGE $01DA
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0xA606;
    ...
    // A636: BLE $01E5
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0xA61D;
    ...
    // A63C: BEQ $01FB
    if (cpu.zero_flag()) cpu.m_pc = 0xA639;
    ...
    // A647: BPL $01F3
    if (!cpu.negative_flag()) cpu.m_pc = 0xA63C;
