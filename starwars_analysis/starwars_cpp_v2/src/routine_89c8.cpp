...
void routine_89c8_impl(StarWarsCPU& cpu) {
    // Converted from rom_disasm_89c8.md
    // Address: 0x89C8
    ...
    // 8AD5: BNE $0113
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8BEA;

    // 8AD7: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 8AD9: BEQ $0114
    if (cpu.zero_flag()) cpu.state_.pc = 0x8BEF;
    ...
    // 8AE3: BNE $0125
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8C0A;
    ...
    // 8B30: BCS $016B
    if (cpu.carry_flag()) cpu.state_.pc = 0x8C9D;
    ...
    // 8B42: BCS $017D
    if (cpu.carry_flag()) cpu.state_.pc = 0x8CC1;
    ...
    // 8B51: BPL $0150
    if (!cpu.negative_flag()) cpu.state_.pc = 0x8CA3;
    ...
    // 8B61: BNE $019E
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8D01;
    ...
    // 8B76: BEQ $01B3
    if (cpu.zero_flag()) cpu.state_.pc = 0x8D2B;
    ...
    // 8B83: BCS $01A8
    if (cpu.carry_flag()) cpu.state_.pc = 0x8D2D;
    ...
    // 8B91: BNE $020E
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8DA1;
    ...
    // 8B97: BLT $01D3
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8D6C;
    ...
    // 8B9D: BGT $01D9
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8D78;
    ...
    // 8BA7: BLT $01E3
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8D8C;
    ...
    // 8BAD: BGT $01E9
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8D98;
    ...
    // 8BBC: BRA $020E
    cpu.state_.pc = 0x8DCC;

    // 8BBE: LDA $A,U
    // TODO: Fix comma operator: LDA $A,U

    // 8BC1: BPL $01FC
    if (!cpu.negative_flag()) cpu.state_.pc = 0x8DBF;

    // 8BC4: CMPA #$08
    cpu.compare_a(0x08);

    // 8BC6: BGT $020E
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8DD6;
