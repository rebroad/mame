...
void routine_8e23_impl(CPU6809& cpu) {
    // Converted from rom_disasm_8e23.md
    // Address: 0x8E23
    ...
    // 8E38: BEQ $002E
    if (cpu.zero_flag()) cpu.m_pc = 0x8E68;
    ...
    // 8E7D: BEQ $0071
    if (cpu.zero_flag()) cpu.m_pc = 0x8EF0;

    // 8E7F: ANDA $15,X
    // TODO: Fix comma operator: ANDA $15,X

    // 8E82: BNE $0071
    if (!cpu.zero_flag()) cpu.m_pc = 0x8EF5;

    // 8E84: ANDB $16,X
    // TODO: Fix comma operator: ANDB $16,X

    // 8E87: BNE $0071
    if (!cpu.zero_flag()) cpu.m_pc = 0x8EFA;
    ...
    // 8E8E: BNE $0066
    if (!cpu.zero_flag()) cpu.m_pc = 0x8EF6;

    // 8E90: BCC $0066
    if (!cpu.carry_flag()) cpu.m_pc = 0x8EF8;
    ...
    // 8EE4: BCS $00D1
    if (cpu.carry_flag()) cpu.m_pc = 0x8EB7;

    // 8EE6: LDA $4B14
    cpu.m_a = cpu.read_memory(0x4B14);

    // 8EEA: BCS $00CE
    if (cpu.carry_flag()) cpu.m_pc = 0x8EBA;
    ...
    // 8F0E: BEQ $0106
    if (cpu.zero_flag()) cpu.m_pc = 0x8F16;

    // 8F10: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8F12: BEQ $0106
    if (cpu.zero_flag()) cpu.m_pc = 0x8F1A;
    ...
    // 8F31: BCS $00E5
    if (cpu.carry_flag()) cpu.m_pc = 0x8F18;
    ...
    // 8F7D: BEQ $0160
    if (cpu.zero_flag()) cpu.m_pc = 0x8FDF;

    // 8F7F: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8F81: BNE $018E
    if (!cpu.zero_flag()) cpu.m_pc = 0x8F11;
    ...
    // 8F90: BCS $017D
    if (cpu.carry_flag()) cpu.m_pc = 0x900F;

    // 8F92: LDA $4B14
    cpu.m_a = cpu.read_memory(0x4B14);

    // 8F96: BCS $017A
    if (cpu.carry_flag()) cpu.m_pc = 0x9012;
    ...
    // 8FB8: BEQ $01A3
    if (cpu.zero_flag()) cpu.m_pc = 0x8F5D;
    ...
    // 8FC2: BCS $0191
    if (cpu.carry_flag()) cpu.m_pc = 0x8F55;
    ...
    // 8FC8: BEQ $0200
    if (cpu.zero_flag()) cpu.m_pc = 0x8FCA;

    // 8FCA: LDA ,U
    // TODO: Complex indexed addressing: ,U

    // 8FCC: BEQ $0200
    if (cpu.zero_flag()) cpu.m_pc = 0x8FCE;
