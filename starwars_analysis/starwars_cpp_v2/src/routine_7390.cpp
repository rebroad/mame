...
void routine_7390_impl(StarWarsCPU& cpu) {
    ...
    // 7453: BNE $00CA
    if (!cpu.zero_flag()) cpu.state_.pc = 0x741F;

    // 7455: JSR $76D3
    cpu.call_function(0x76D3);

    // 7458: BRA $00D4
    cpu.state_.pc = 0x742E;
    ...
    // 7467: BNE $0101
    if (!cpu.zero_flag()) cpu.state_.pc = 0x746A;
    ...
    // 7477: BNE $0101
    if (!cpu.zero_flag()) cpu.state_.pc = 0x747A;

    // 7479: LDA $4B12
    cpu.state_.a = cpu.read_memory(0x4B12);

    // 747C: BNE $00F2
    if (!cpu.zero_flag()) cpu.state_.pc = 0x7470;

    // 747E: LDB #$4C
    cpu.state_.b = 0x4C;

    // 7480: BRA $00FE
    cpu.state_.pc = 0x7480;
    ...
    // 7486: BNE $00FC
    if (!cpu.zero_flag()) cpu.state_.pc = 0x7484;

    // 7488: LDB #$4C
    cpu.state_.b = 0x4C;

    // 748A: BRA $00FE
    cpu.state_.pc = 0x748A;
    ...
    // 7494: BNE $0115
    if (!cpu.zero_flag()) cpu.state_.pc = 0x74AB;

    // 7496: LDA $4895
    cpu.state_.a = cpu.read_memory(0x4895);

    // 7499: BEQ $0115
    if (cpu.zero_flag()) cpu.state_.pc = 0x74B0;
    ...
    // 74A8: BLT $0126
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x74D0;

    // 74AA: BNE $0123
    if (!cpu.zero_flag()) cpu.state_.pc = 0x74CF;
    ...
    // 74E6: BEQ $0162
    if (cpu.zero_flag()) cpu.state_.pc = 0x754A;
    ...
    // 74FE: BNE $017F
    if (!cpu.zero_flag()) cpu.state_.pc = 0x757F;
    ...
    // 753A: BGT $01CA
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x7506;
    ...
    // 755F: BGT $020C
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x756D;

    // 7561: LDA $4845
    cpu.state_.a = cpu.read_memory(0x4845);

    // 7564: BEQ $020C
    if (cpu.zero_flag()) cpu.state_.pc = 0x7572;
    ...
    // 756B: BEQ $020C
    if (cpu.zero_flag()) cpu.state_.pc = 0x7579;
