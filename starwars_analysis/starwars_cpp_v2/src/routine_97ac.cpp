...
void routine_97ac_impl(StarWarsCPU& cpu) {
    ...
    // 97B1: BCS $000C
    if (cpu.carry_flag()) cpu.state_.pc = 0x97BF;

    // 97B3: LDU #$9847
    cpu.state_.u = 0x9847;

    // 97B6: BRA $0015
    cpu.state_.pc = 0x97CD;
    ...
    // 97E6: BRA $0064
    cpu.state_.pc = 0x984C;

    // 97E8: LDU #$984A
    cpu.state_.u = 0x984A;

    // 97EB: BRA $0064
    cpu.state_.pc = 0x9851;

    // 97ED: LDU #$984D
    cpu.state_.u = 0x984D;

    // 97F0: BRA $0064
    cpu.state_.pc = 0x9856;

    // 97F2: LDU #$9850
    cpu.state_.u = 0x9850;

    // 97F5: BRA $0064
    cpu.state_.pc = 0x985B;

    // 97F7: LDU #$9859
    cpu.state_.u = 0x9859;

    // 97FA: BRA $0064
    cpu.state_.pc = 0x9860;

    // 97FC: LDU #$9853
    cpu.state_.u = 0x9853;

    // 97FF: BRA $0064
    cpu.state_.pc = 0x9865;

    // 9801: LDU #$985C
    cpu.state_.u = 0x985C;

    // 9804: BRA $0064
    cpu.state_.pc = 0x986A;

    // 9806: LDU #$985F
    cpu.state_.u = 0x985F;

    // 9809: BRA $0064
    cpu.state_.pc = 0x986F;

    // 980B: LDU #$9862
    cpu.state_.u = 0x9862;

    // 980E: BRA $0064
    cpu.state_.pc = 0x9874;
    ...
    // 9876: BNE $00D2
    if (!cpu.zero_flag()) cpu.state_.pc = 0x984A;
    ...
    // 9892: BEQ $00EB
    if (cpu.zero_flag()) cpu.state_.pc = 0x987F;
    ...
    // 989C: BNE $0103
    if (!cpu.zero_flag()) cpu.state_.pc = 0x98A1;
    ...
    // 98A3: BPL $00FB
    if (!cpu.negative_flag()) cpu.state_.pc = 0x98A0;
    ...
    // 98AD: BCS $00F5
    if (cpu.carry_flag()) cpu.state_.pc = 0x98A4;
    ...
    // 98D6: BRA $0150
    cpu.state_.pc = 0x9928;

    // 98D8: BCC $0152
    if (!cpu.carry_flag()) cpu.state_.pc = 0x992C;

    // 98DA: BCS $0155
    if (cpu.carry_flag()) cpu.state_.pc = 0x9931;
