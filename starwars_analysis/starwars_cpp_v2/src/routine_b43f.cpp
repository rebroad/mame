...
void routine_b43f_impl(StarWarsCPU& cpu) {
    ...
    // B4BF: BGT $00D1
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xB592;
    ...
    // B4C5: BNE $00CC
    if (!cpu.zero_flag()) cpu.state_.pc = 0xB593;
    ...
    // B4CD: BLT $00D7
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB5A6;

    // B4CF: LDD $5042
    cpu.state_.d = cpu.read_memory_word(0x5042);

    // B4D2: BGT $00CC
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xB5A0;
    ...
    // B4DD: BLT $00CC
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB5AB;

    // B4DF: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // B4E2: BGT $00CC
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xB5B0;
    ...
    // B4F5: BLT $00BE
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB5B5;
    ...
    // B4FF: BNE $00CC
    if (!cpu.zero_flag()) cpu.state_.pc = 0xB5CD;
    ...
    // B506: BPL $00CA
    if (!cpu.negative_flag()) cpu.state_.pc = 0xB5D2;
    ...
    // B52B: BNE $0126
    if (!cpu.zero_flag()) cpu.state_.pc = 0xB653;
    ...
    // B534: BCC $00FE
    if (!cpu.carry_flag()) cpu.state_.pc = 0xB634;

    // B536: BMI $00FE
    if (cpu.negative_flag()) cpu.state_.pc = 0xB636;

    // B538: INC $49BD
    // TODO: Convert INC $49BD

    // B53B: BRA $0101
    cpu.state_.pc = 0xB63E;

    // B53E: BNE $00F4
    if (!cpu.zero_flag()) cpu.state_.pc = 0xB634;

    // B540: LDA $49BD
    cpu.state_.a = cpu.read_memory(0x49BD);

    // B543: BEQ $0126
    if (cpu.zero_flag()) cpu.state_.pc = 0xB66B;
    ...
    // B54B: BCS $0110
    if (cpu.carry_flag()) cpu.state_.pc = 0xB65D;
    ...
    // B559: BCC $011E
    if (!cpu.carry_flag()) cpu.state_.pc = 0xB679;
    ...
    // B5B4: BCS $017A
    if (cpu.carry_flag()) cpu.state_.pc = 0xB730;
    ...
    // B5DA: BNE $01B0
    if (!cpu.zero_flag()) cpu.state_.pc = 0xB78C;
    ...
    // B5F9: BGT $020B
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xB806;
    ...
    // B5FF: BNE $0206
    if (!cpu.zero_flag()) cpu.state_.pc = 0xB807;
    ...
    // B607: BLT $0211
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB81A;

    // B609: LDD $5042
    cpu.state_.d = cpu.read_memory_word(0x5042);

    // B60C: BLT $0206
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB814;
    ...
    // B617: BLT $0206
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB81F;

    // B619: SUBD #$0400
    cpu.state_.d -= 0x0400;

    // B61C: BGT $0206
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0xB824;
    ...
    // B62F: BLT $01F8
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0xB829;
    ...
    // B639: BNE $0206
    if (!cpu.zero_flag()) cpu.state_.pc = 0xB841;
