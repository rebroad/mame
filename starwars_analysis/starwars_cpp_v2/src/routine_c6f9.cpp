...
void routine_c6f9_impl(CPU6809& cpu) {
    ...
    // C709: BPL $0003
    if (!cpu.negative_flag()) cpu.m_pc = 0xC70E;
    ...
    // C71A: BCS $0025
    if (cpu.carry_flag()) cpu.m_pc = 0xC741;
    ...
    // C72C: BEQ $003B
    if (cpu.zero_flag()) cpu.m_pc = 0xC769;
    ...
    // C7AA: BCS $00DD
    if (cpu.carry_flag()) cpu.m_pc = 0xC789;
    ...
    // C840: BNE $014E
    if (!cpu.zero_flag()) cpu.m_pc = 0xC890;
    ...
    // C854: BMI $0178
    if (cpu.negative_flag()) cpu.m_pc = 0xC8CE;
    ...
    // C85B: BNE $0174
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8D1;
    ...
    // C862: BNE $0170
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8D4;
    ...
    // C873: BNE $018B
    if (!cpu.zero_flag()) cpu.m_pc = 0xC800;

    // C875: TST $4AEC
    // TODO: Convert TST $4AEC

    // C878: BMI $0186
    if (cpu.negative_flag()) cpu.m_pc = 0xC800;
    ...
    // C88C: BMI $01B0
    if (cpu.negative_flag()) cpu.m_pc = 0xC83E;
    ...
    // C893: BNE $01AC
    if (!cpu.zero_flag()) cpu.m_pc = 0xC841;
    ...
    // C89A: BNE $01A8
    if (!cpu.zero_flag()) cpu.m_pc = 0xC844;
    ...
    // C8AB: BNE $01C3
    if (!cpu.zero_flag()) cpu.m_pc = 0xC870;

    // C8AD: TST $4AEC
    // TODO: Convert TST $4AEC

    // C8B0: BMI $01BE
    if (cpu.negative_flag()) cpu.m_pc = 0xC870;
    ...
    // C8C4: BMI $01E8
    if (cpu.negative_flag()) cpu.m_pc = 0xC8AE;
    ...
    // C8CB: BNE $01E4
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8B1;
    ...
    // C8D2: BNE $01E0
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8B4;
    ...
    // C8E3: BNE $01FB
    if (!cpu.zero_flag()) cpu.m_pc = 0xC8E0;

    // C8E5: TST $4AEC
    // TODO: Convert TST $4AEC

    // C8E8: BMI $01F6
    if (cpu.negative_flag()) cpu.m_pc = 0xC8E0;
