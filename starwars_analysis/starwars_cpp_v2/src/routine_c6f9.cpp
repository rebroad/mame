...
void routine_c6f9_impl(StarWarsCPU& cpu) {
    ...
    // C7AA: BCS $00DD
    if (cpu.carry_flag()) cpu.state_.pc = 0xC789;
    ...
    // C840: BNE $014E
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC890;

    // C842: LDD #$6780
    cpu.state_.d = 0x6780;

    // C845: BRA $0151
    cpu.state_.pc = 0xC898;
    ...
    // C854: BMI $0178
    if (cpu.negative_flag()) cpu.state_.pc = 0xC8CE;
    ...
    // C85B: BNE $0174
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC8D1;
    ...
    // C862: BNE $0170
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC8D4;

    // C864: LDD $4B10
    cpu.state_.d = cpu.read_memory_word(0x4B10);

    // C867: BRA $0172
    cpu.state_.pc = 0xC8DB;

    // C869: LDD <$01
    cpu.state_.d = cpu.read_memory_word(0x01);

    // C86B: BRA $0176
    cpu.state_.pc = 0xC8E3;
    ...
    // C873: BNE $018B
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC800;

    // C875: TST $4AEC
    // TODO: Convert TST $4AEC

    // C878: BMI $0186
    if (cpu.negative_flag()) cpu.state_.pc = 0xC800;

    // C87A: LDD $3054
    cpu.state_.d = cpu.read_memory_word(0x3054);

    // C87D: BRA $0189
    cpu.state_.pc = 0xC808;

    // C87F: LDD $3002
    cpu.state_.d = cpu.read_memory_word(0x3002);

    // C882: BRA $018E
    cpu.state_.pc = 0xC812;
    ...
    // C88C: BMI $01B0
    if (cpu.negative_flag()) cpu.state_.pc = 0xC83E;
    ...
    // C893: BNE $01AC
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC841;
    ...
    // C89A: BNE $01A8
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC844;

    // C89C: LDD $4B10
    cpu.state_.d = cpu.read_memory_word(0x4B10);

    // C89F: BRA $01AA
    cpu.state_.pc = 0xC84B;

    // C8A1: LDD <$01
    cpu.state_.d = cpu.read_memory_word(0x01);

    // C8A3: BRA $01AE
    cpu.state_.pc = 0xC853;
    ...
    // C8AB: BNE $01C3
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC870;

    // C8AD: TST $4AEC
    // TODO: Convert TST $4AEC

    // C8B0: BMI $01BE
    if (cpu.negative_flag()) cpu.state_.pc = 0xC870;

    // C8B2: LDD $3054
    cpu.state_.d = cpu.read_memory_word(0x3054);

    // C8B5: BRA $01C1
    cpu.state_.pc = 0xC878;

    // C8B7: LDD $3002
    cpu.state_.d = cpu.read_memory_word(0x3002);

    // C8BA: BRA $01C6
    cpu.state_.pc = 0xC882;
    ...
    // C8C4: BMI $01E8
    if (cpu.negative_flag()) cpu.state_.pc = 0xC8AE;
    ...
    // C8CB: BNE $01E4
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC8B1;
    ...
    // C8D2: BNE $01E0
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC8B4;

    // C8D4: LDD $4B10
    cpu.state_.d = cpu.read_memory_word(0x4B10);

    // C8D7: BRA $01E2
    cpu.state_.pc = 0xC8BB;

    // C8D9: LDD <$01
    cpu.state_.d = cpu.read_memory_word(0x01);

    // C8DB: BRA $01E6
    cpu.state_.pc = 0xC8C3;
    ...
    // C8E3: BNE $01FB
    if (!cpu.zero_flag()) cpu.state_.pc = 0xC8E0;

    // C8E5: TST $4AEC
    // TODO: Convert TST $4AEC

    // C8E8: BMI $01F6
    if (cpu.negative_flag()) cpu.state_.pc = 0xC8E0;
