...
void routine_6db6_impl(CPU6809& cpu) {
    ...
    // 6DD4: BEQ $003A
    if (cpu.zero_flag()) cpu.m_pc = 0x6E10;

    // 6DD6: BLE $0029
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E01;
    ...
    // 6DFC: BEQ $0062
    if (cpu.zero_flag()) cpu.m_pc = 0x6E60;

    // 6DFE: BLE $0051
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E51;
    ...
    // 6E24: BEQ $0078
    if (cpu.zero_flag()) cpu.m_pc = 0x6E9E;
    ...
    // 6E30: BPL $007D
    if (!cpu.negative_flag()) cpu.m_pc = 0x6EAF;
    ...
    // 6E38: BPL $0088
    if (!cpu.negative_flag()) cpu.m_pc = 0x6DC2;
    ...
    // 6E4A: BPL $009A
    if (!cpu.negative_flag()) cpu.m_pc = 0x6DE6;
    ...
    // 6E58: BLE $00AF
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E09;
    ...
    // 6E69: BGE $00B8
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6E23;
    ...
    // 6E78: BLE $00C7
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E41;
    ...
    // 6E83: BGE $00D2
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6E57;
    ...
    // 6EAE: BNE $0103
    if (!cpu.zero_flag()) cpu.m_pc = 0x6EB3;

    // 6EB0: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 6EB2: BNE $0103
    if (!cpu.zero_flag()) cpu.m_pc = 0x6EB7;
    ...
    // 6EBC: BLE $010D
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6ECB;
    ...
    // 6EC9: BCS $00F4
    if (cpu.carry_flag()) cpu.m_pc = 0x6EBF;
    ...
    // 6ED4: BMI $0125
    if (cpu.negative_flag()) cpu.m_pc = 0x6EFB;
    ...
    // 6EE6: BMI $0137
    if (cpu.negative_flag()) cpu.m_pc = 0x6F1F;
    ...
    // 6F14: BLE $0183
    if (cpu.zero_flag() || cpu.negative_flag() != cpu.overflow_flag()) cpu.m_pc = 0x6E99;
    ...
    // 6F8B: BPL $01D8
    if (!cpu.negative_flag()) cpu.m_pc = 0x6F65;

    // 6F90: TST <$7D
    // TODO: Convert TST <$7D

    // 6F92: BGE $01E2
    if (cpu.negative_flag() == cpu.overflow_flag()) cpu.m_pc = 0x6F76;
    ...
    // 6FAF: BPL $01FC
    if (!cpu.negative_flag()) cpu.m_pc = 0x6FAD;
