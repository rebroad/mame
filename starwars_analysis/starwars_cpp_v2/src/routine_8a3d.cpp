...
void routine_8a3d_impl(StarWarsCPU& cpu) {
    ...
    // 8AD5: BNE $009E
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8A75;

    // 8AD7: LDA $6,X
    // TODO: Fix comma operator: LDA $6,X

    // 8AD9: BEQ $009F
    if (cpu.zero_flag()) cpu.state_.pc = 0x8A7A;
    ...
    // 8AE3: BNE $00B0
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8A95;
    ...
    // 8B30: BCS $00F6
    if (cpu.carry_flag()) cpu.state_.pc = 0x8B28;
    ...
    // 8B42: BCS $0108
    if (cpu.carry_flag()) cpu.state_.pc = 0x8B4C;
    ...
    // 8B51: BPL $00DB
    if (!cpu.negative_flag()) cpu.state_.pc = 0x8B2E;
    ...
    // 8B61: BNE $0129
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8B8C;
    ...
    // 8B76: BEQ $013E
    if (cpu.zero_flag()) cpu.state_.pc = 0x8BB6;
    ...
    // 8B83: BCS $0133
    if (cpu.carry_flag()) cpu.state_.pc = 0x8BB8;
    ...
    // 8B91: BNE $0199
    if (!cpu.zero_flag()) cpu.state_.pc = 0x8B2C;
    ...
    // 8B97: BLT $015E
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8BF7;
    ...
    // 8B9D: BGT $0164
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8C03;
    ...
    // 8BA7: BLT $016E
    if (cpu.negative_flag() != cpu.overflow_flag()) cpu.state_.pc = 0x8C17;
    ...
    // 8BAD: BGT $0174
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8C23;
    ...
    // 8BBC: BRA $0199
    cpu.state_.pc = 0x8B57;

    // 8BBE: LDA $A,U
    // TODO: Fix comma operator: LDA $A,U

    // 8BC1: BPL $0187
    if (!cpu.negative_flag()) cpu.state_.pc = 0x8B4A;

    // 8BC4: CMPA #$08
    cpu.compare_a(0x08);

    // 8BC6: BGT $0199
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8B61;

    // 8BC8: LDA $C,U
    // TODO: Fix comma operator: LDA $C,U

    // 8BCB: BPL $0191
    if (!cpu.negative_flag()) cpu.state_.pc = 0x8B5E;

    // 8BCE: CMPA #$08
    cpu.compare_a(0x08);

    // 8BD0: BGT $0199
    if (!cpu.zero_flag() && cpu.negative_flag() == cpu.overflow_flag()) cpu.state_.pc = 0x8B6B;
    ...
    // 8BFF: BMI $01D8
    if (cpu.negative_flag()) cpu.state_.pc = 0x8BD9;
    ...
    // 8C38: BCC $0200
    if (!cpu.carry_flag()) cpu.state_.pc = 0x8C3A;
