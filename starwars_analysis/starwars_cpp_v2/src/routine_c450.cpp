...
void routine_c450_impl(StarWarsCPU& cpu) {
    ...
    // C458: BCS $0002
    if (cpu.carry_flag()) cpu.state_.pc += 0x0002;
    ...
    // C472: BPL $0019
    if (!cpu.negative_flag()) cpu.state_.pc += 0x0019;
    ...
    // C4B6: BEQ $008A
    if (cpu.zero_flag()) cpu.state_.pc += 0x008A;
    ...
    // C64D: BNE $020A
    if (!cpu.zero_flag()) cpu.state_.pc += 0x020A;
