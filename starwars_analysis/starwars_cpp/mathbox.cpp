...
void Mathbox::load_prom_data() {
    // In a real implementation, this would load from the actual PROM files
    // For now, we'll initialize with some basic patterns
    
    // Initialize PROM_STR (strobe bits)
    for (int i = 0; i < 1024; i++) {
        prom_str[i] = (i % 256) & 0xFF;
    }
    
    // Initialize PROM_MAS (address bits)
    for (int i = 0; i < 1024; i++) {
        prom_mas[i] = i & 0x7F;
    }
    
    // Initialize PROM_AM (addressing mode)
    for (int i = 0; i < 1024; i++) {
        prom_am[i] = (i >> 7) & 0x01;
    }
}
