...
def update_cmake_lists(project_path: Path, routine_files: List[str]) -> bool:
    """Update CMakeLists.txt to include all routine source files"""
    ...
    # Extract existing routine files to avoid duplicates
    ...
    for line in lines[sources_start+1:sources_end]:
        if 'src/routine_' in line:
            ...
            if match:
                existing_routines.add(match.group(1))
    
    # Add new routine files
    added_count = 0
    for routine in routine_files:
        if routine not in existing_routines:
    ...
    print(f"    📊 Total routines in build: {len(existing_routines) + added_count}")
    return True

def update_routine_map(project_path: Path, routine_files: List[str]) -> bool:
    """Update rom_routines_wrapper.cpp to add all routines to routine_map"""
    ...
    # Extract existing routine entries to avoid duplicates
    existing_routines = set()
    ...
    print(f"    📊 Total routines in map: {len(existing_routines) + added_count}")
    return True
